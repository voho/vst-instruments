...
| --- | --- | ---: | ---: | ---: |
| `01-dual-filter-vocal.wav` | The signature dual filter: the lower parametric boost slid against the upper lowpass | 10.5 s | +4.0 dBFS | −7.0 dB |
| `02-formant-brass.wav` | FORMANT tracking: a frozen lower-filter peak under articulated brass stabs | 10.2 s | −1.6 dBFS | −1.4 dB |
| `03-overdrive-growl.wav` | The soft clipper between the filters on a two-oscillator bass riff | 8.4 s | +6.8 dBFS | −9.8 dB |
| `04-sync-sweep.wav` | Hard sync: Shaper Y sweeps Osc B through the lock on every note | 8.4 s | −9.5 dBFS | +6.5 dB |
| `05-ring-bells.wav` | The triangle-cross ring modulator's clangorous register | 10.5 s | −6.1 dBFS | +3.1 dB |
| `06-leap-arpeggio.wav` | The LEAP arpeggiator cycling unison, up an octave, down an octave per note | 7.5 s | −11.4 dBFS | +8.4 dB |
...
| `10-red-noise-drift.wav` | RED NOISE wander on both oscillators' pitch | 8.3 s | −5.0 dBFS | +2.0 dB |
...
| `12-program-tour.wav` | Six of the Ghostar Programs in turn, each with a phrase that suits it | 20.1 s | −1.8 dBFS | −1.2 dB |
<!-- peaks-table-end -->
