...
- **CEM3360 preliminary datasheet** (dual linear VCA), Curtis Electromusic —
  current-output and control architecture; it does not specify exact top gain,
  saturation voltage or per-device feedthrough.
...
R38=10 kΩ; R40=5.6 kΩ returns it to ground and R41=1 MΩ to −12 V. An
always-biased BC173 emitter follower reaches the same node through R39=3.6 kΩ.
For instantaneous emitter voltage `Q`, the external network is exactly
`V_C=0.1794207274·S+0.4983909094·Q−0.0215304873 V`. TR2's base is fed from
+12 V through R29=100 kΩ, so `Q` requires a coupled β/V_BE/load solve; the
original CEM3360 input clamp, full-gain voltage and feedthrough are not
specified. SHAPE's reversal levels are parametrically half the SG op-amp
rails, whose loaded swing is also unrecorded. Ghostar therefore keeps its
clearly labelled behavioral `max(0,Y)` VCA law until simultaneous `S`, `Q`
and `V_C` captures close OQ-26; no guessed rail substitution is encoded.
...
pre-charge, plus the Upper SW4/C40 state network and tied VIF+VIV drive;
TL082 dynamics and internal CEM3350 headroom also require completion or
hardware measurement (OQ-09/OQ-10/OQ-12/OQ-20).
...
| SLOPE | Upper is 12 dB (controlled section) or 24 dB (controlled plus fixed-Q=0.5 section). SW4 also moves C40=1 nF between their 22 nF LP nodes with retained charge, leaves a 1 MΩ cross-coupling only in 12 dB, and changes the output-buffer gain by exact ratio `101/201`; those state/gain quirks remain to be integrated | topology/components **derived**, OM pp.30/32 + SM DWG 2; engine completion **open**, OQ-09 |
...
| OVERDRIVE clipper | a separate IC12A/BA130/RS7 network between Lower and Upper, so the Upper re-filters its distortion products. The engine solves the ideal-op-amp scalar from physical Lower VLP and drives C34 from the exact functional Thevenin source `Vth=(33·o+47·VLP)/80`, `Rth=47k||33k`: C10 makes R167 a clean-VLP feed, not a ground shunt. A3+B7+C10 is functionally identifiable as OVERDRIVE; the unmarked three-deck rotor phase and other named detents still need hardware continuity | placement/components anchored OM p.32 + SM; local transfer and Lower drive **derived/modelled**; remaining switch mapping **open**, OQ-10/OQ-20 |
...
| Shaper audio VCA | P1013's R38/R40/R41 divider is loaded through R39 by an always-biased BC173 emitter follower. Its external KCL is closed, but control voltage needs TR2 β/V_BE, original-CEM3360 input-clamp/full-scale behavior and loaded SHAPE op-amp swing. Ghostar's current `max(0,Y)` half-wave law is explicitly behavioral, not a claimed transistor reduction | topology/KCL anchored and **derived**, SM DWG 2 P1013; transfer **voiced/open**, OQ-26 |
...
RS7 rotor phase/C34 pre-charge outside OVERDRIVE, the Upper SW4/C40 and tied-input model, original
...
OSC B) uses the selected post-IC10 waveform at the internal rate: current for
the filters and, with SYNC off, A/PWM; causal prior B for its own frequency
feedback and for A/PWM when SYNC closes the B→A→A-reset→B loop.
