...
- **CEM3360 production datasheet** (dual linear VCA, CES © 1984;
  ...
  (−0.5…−4.0 µA), 80 dB typical / 70 dB minimum zero-control attenuation,
  and the exponential-control scale of 3.0 mV/dB typical (2.7–3.3). The
  latter closes P1015's nominal MOD RATE span from its 132 mV swing. The
  one-point linear-pin bias is not a complete `i5(Vc)` law; the sheet does
  not specify out-of-range behavior or the sign of a device's feedthrough
  residual. The earlier preliminary sheet remains period corroboration only.
...
conditioner supplies the derived 5 V/unit normalization. The remaining Lower
seam is RS7's unlabelled OUT/BANDPASS/HIGHPASS rotor mapping and corresponding
C34 pre-charge. Until continuity closes it, Ghostar keeps the owner's-manual
behaviour explicit with output-referred dry, `dry + 11·VBP` parametric-boost
and `dry − 8·VLP` high-pass bridges; those two numeric gains are voiced, not
presented as component derivations. TL082 dynamics and internal CEM3350
headroom also require hardware measurement (OQ-10/OQ-12/OQ-20).

| Control | Law | Provenance |
|---|---|---|
| MASTER (cutoff) | sets both filters' cutoff, always. P6=100 kΩ LIN across ±12 V is loaded by R50||R51=110.5 kΩ, then IC15's 100/221 gain and the Dynamic CEM node gain 0.021233778 V/V produce a curved ±5.88248-octave law about panel 5 (11.76495 octaves total). Absolute placement remains the undocumented 100 kΩ trim: Ghostar retains 565.685 Hz at panel 5, yielding nominal 9.589 Hz–33.372 kHz before modulation | routing anchored OM p.31; taper/span **derived**, SM DWG 2 + CEM3350 datasheet; centre **voiced**, OQ-02 |
| LOWER ONLY | P5=100 kΩ LIN from 0 to −12 V is loaded by R48=150 kΩ. Relative to the manual's panel-8 coincidence, Dynamic spans −7.10055 to +1.56630 octaves. In Formant, corrected R188=22 kΩ changes the node gain by 1.389%, widening that to −7.19917/+1.58805 and making coincidence drift ±0.08170 octave across MASTER. P5 remains active in both modes | coincidence/direction anchored OM p.31; taper/span/FORMANT quirk **derived**, SM DWG 2 + errata + CEM3350 datasheet, OQ-02 |
...
| SLOPE | Upper is 12 dB (controlled section) or 24 dB (controlled plus fixed-Q=0.5 section). Both halves tie VIF+VIV, so drive is `u·(1+1/Qcommanded)`. SW4 moves C40=1 nF between their 22 nF LP nodes with retained charge, leaves a 1 MΩ cross-coupling only in 12 dB, and changes IC14B's absolute output gain from 201 to 101. Ghostar solves all four states together and charge-projects the newly selected node; unresolved output-referred RS7 seams are referred back through 201 before the physical states, while C34's OVERDRIVE voltage enters them directly | **derived/modelled**, OM pp.30/32 + SM DWG 2 + CEM3350 datasheet, OQ-09 |
| Lower mode | OUT / OVERDRIVE / BANDPASS / HIGHPASS. BANDPASS is **parametric boost** — "a peak … without attenuation of frequencies far from this cutoff" — not a true band-pass. OVERDRIVE is the manual's distorted parametric register *between* the filters, but its production throw is a distinct Lower-VLP-fed IC12A/BA130 path rather than `dry + BP` followed by a generic clipper. HIGHPASS gives the documented "double-peak, highpass-lowpass" response. Pending RS7 continuity, the anchored behaviours use explicit output-referred `dry + 11·VBP` and `dry − 8·VLP` bridges; their gains are voiced and regression-tested, not claimed as traced contacts | behaviour anchored, OM pp.30–32; OVERDRIVE topology SM DWG 2; BP/HP output gains **voiced/open**, OQ-20 |
...
| Item | Law | Provenance |
|---|---|---|
| Two ADSRs | Each envelope has its own 4.7 µF cap and 2 MΩ log A/D/R sliders. R23/R24=100 Ω lie between the common segment/556-threshold node and the actual cap, so all currents cross them: the nominal range is 5.17 ms to 9.40047 s and fast Attack trips with the cap at `1−100/R_attack·(1.3−1)=0.97` of the 7.5 V threshold. The two 100 kΩ sustain tracks share one D15-biased bottom rail (`f=1/15` nominal), and D11/D14 give release its nonlinear knee. Selected X/Y rising edges use the separate 10 nF/470 kΩ nominal 5 ms reset lane; raw MULTIPLE KT and accepted arpeggiator AA meet at R10=1 MΩ/C7=10 nF for 10 ms. Either releases both caps before Attack resumes from their retained levels; a 5 ms edge cannot shorten a coincident 10 ms notch. SINGLE has no KT pulse branch | topology/components and trigger state machine **derived/modelled**, SM DWG 3 + Signetics 1973 databook / AN170; nominal pulse widths, fast residual, aim and diode calibration **modelled**, OQ-04 |
...
| TRIGGER | MULTIPLE routes every raw new-key KT, tapped before KBD gate selection, through the nominal 10 ms reset lane; Attack resumes only if a selected gate remains high. SINGLE has no KT branch: only a genuine selected-bus low→high attacks, so legato or a keyboard rise hidden under X/Y does not | anchored/derived, OM p.34 + SM DWG 3 |
...
| LFO/S+H RATE | nominal 0.3154787–50 Hz; also the S&H and arpeggiator clock; no effect on RED NOISE or OSC B. P2=100 kΩ LIN is loaded by R33=200 kΩ, giving `w(x)=200x/(200+100x(1−x))` and `w(0.5)=4/9`; P1015's 132 mV span against the original CEM3360's 3.0 mV/dB typical scale gives 44 dB/158.489319:1 and a 2.9974213 Hz midpoint. The datasheet's 2.7–3.3 limits bound the nominal slow end to about 0.1797–0.5000 Hz before Spirit tolerances | fast end anchored OM p.34; travel/span **derived**, SM MOD drawing + CEM3360 production datasheet, OQ-21 |
