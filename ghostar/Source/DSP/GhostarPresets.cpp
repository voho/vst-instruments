...
namespace
{
    ...
    const std::array<Preset, 29>& presets()
    {
        static const std::array<Preset, 29> table {{
            ...
            { "Bypass Pad",
              "VCA BYPASS holds the path open; the X wheel wanders the "
              "cutoff", PresetBank::Programs, bypassPad() },
            { "Leap Sequence",
              "LEAP cycles each note through unison, up and down an octave",
              PresetBank::Programs, leapSequence() },
            { "Patterned Steps",
              "S+H sampling the Shaper: raise the X wheel for a repeating "
              "figure", PresetBank::Programs, patternedSteps() },
            { "Glide Lead",
              "AUTO glide, legato only when two keys overlap; X wheel "
              "vibrato", PresetBank::Programs, glideLead() },
            { "Hollow Fifth",
              "A fifth apart: X wheel PWM on Osc A, Y wheel PWM on Osc B",
