...
if [[ -z "${VERSION:-}" ]]; then
    VERSION="$(sed -n 's/^project(Ghostar VERSION \([0-9][0-9.]*\).*/\1/p' \
...
ARTIFACT_DIR="${BUILD_DIR}/Ghostar_artefacts/${CONFIG}"
...
VST3="${ARTIFACT_DIR}/VST3/Ghostar.vst3"
AU="${ARTIFACT_DIR}/AU/Ghostar.component"
APP="${ARTIFACT_DIR}/Standalone/Ghostar.app"
...
ditto "${VST3}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Ghostar.vst3"
ditto "${AU}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Ghostar.component"
ditto "${APP}" "${PACKAGE_ROOT}/Applications/Ghostar.app"
...
# binaries; the installer lands them at /Library/Documentation/Ghostar).
DOC_DIR="${PACKAGE_ROOT}/Library/Documentation/Ghostar"
mkdir -p "${DOC_DIR}"
...
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Ghostar.vst3"
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Ghostar.component"
sign_bundle "${PACKAGE_ROOT}/Applications/Ghostar.app"

ZIP_PATH="${DIST_DIR}/Ghostar-${VERSION}-macOS-universal.zip"
PKG_UNSIGNED="${DIST_DIR}/Ghostar-${VERSION}-unsigned.pkg"
PKG_FINAL="${DIST_DIR}/Ghostar-${VERSION}-macOS-universal.pkg"
...
pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier audio.ghostar.synth.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
