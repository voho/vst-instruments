...
ARTIFACT_DIR="${BUILD_DIR}/Neuramar_artefacts/${CONFIG}"
...
for tool in codesign ditto lipo pkgbuild pkgutil xattr; do
...
VST3="${ARTIFACT_DIR}/VST3/Neuramar.vst3"
AU="${ARTIFACT_DIR}/AU/Neuramar.component"
APP="${ARTIFACT_DIR}/Standalone/Neuramar.app"
...
VST3_ARCHS="$(lipo -archs "${VST3}/Contents/MacOS/Neuramar")"
AU_ARCHS="$(lipo -archs "${AU}/Contents/MacOS/Neuramar")"
APP_ARCHS="$(lipo -archs "${APP}/Contents/MacOS/Neuramar")"
...
ditto "${VST3}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Neuramar.vst3"
ditto "${AU}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Neuramar.component"
ditto "${APP}" "${PACKAGE_ROOT}/Applications/Neuramar.app"

NOTICE_ROOT="${PACKAGE_ROOT}/Library/Application Support/Neuramar/Documentation"
...
mkdir -p "${NOTICE_ROOT}"
ditto "${PROJECT_DIR}/LICENSE" "${NOTICE_ROOT}/Neuramar-LICENSE.txt"
...
for bundle in \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Neuramar.vst3" \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Neuramar.component" \
    "${PACKAGE_ROOT}/Applications/Neuramar.app"; do
    bundle_notice_root="${bundle}/Contents/Resources/Documentation"
    mkdir -p "${bundle_notice_root}"
    ditto "${PROJECT_DIR}/LICENSE" "${bundle_notice_root}/Neuramar-LICENSE.txt"
    ditto "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
        "${bundle_notice_root}/THIRD_PARTY_NOTICES.md"
    ditto "${JUCE_NOTICE}" "${bundle_notice_root}/JUCE-LICENSE.md"
done

# Build outputs can inherit provenance, quarantine, Finder metadata, or other
# local extended attributes. Strip them before signing so pkgbuild cannot turn
# those machine-local attributes into AppleDouble `._*` payload entries.
...
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Neuramar.vst3"
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Neuramar.component"
sign_bundle "${PACKAGE_ROOT}/Applications/Neuramar.app"

ZIP_PATH="${DIST_DIR}/Neuramar-${VERSION}-macOS-${ARTIFACT_ARCH}.zip"
PKG_UNSIGNED="${DIST_DIR}/Neuramar-${VERSION}-unsigned.pkg"
PKG_FINAL="${DIST_DIR}/Neuramar-${VERSION}-macOS-${ARTIFACT_ARCH}.pkg"

# Leave exactly one Neuramar release set in dist, avoiding stale version or
# architecture names in wildcard-driven CI publication.
rm -f "${DIST_DIR}/Neuramar-"*-macOS-*.zip
rm -f "${DIST_DIR}/Neuramar-"*-macOS-*.pkg
rm -f "${DIST_DIR}/Neuramar-"*-unsigned.pkg

COPYFILE_DISABLE=1 pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier audio.neuramar.synth.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
...
if [[ -n "${INSTALLER_SIGN_IDENTITY}" ]]; then
    ...
    echo "note: installer container is unsigned; its embedded bundles are signed with '${APP_SIGN_IDENTITY}'"
fi

if pkgutil --payload-files "${PKG_FINAL}" | grep -E '(^|/)\._' >/dev/null; then
    echo "error: installer payload contains AppleDouble metadata entries" >&2
    exit 1
fi
