...
cmake_args=(
    ...
    "-DNEURAMAR_BUILD_UNIVERSAL=${BUILD_UNIVERSAL}"
...
if [[ -n "${JUCE_PATH:-}" ]]; then
    cmake_args+=("-DNEURAMAR_JUCE_PATH=${JUCE_PATH}")
fi
...
artifacts=(
    "${ARTIFACT_DIR}/VST3/Neuramar.vst3"
    "${ARTIFACT_DIR}/AU/Neuramar.component"
    "${ARTIFACT_DIR}/Standalone/Neuramar.app"
)
