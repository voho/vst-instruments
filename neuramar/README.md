...
A **neural instrument**: VST3, Audio Unit and Standalone for macOS, plus Linux
and Windows builds.

![Neuramar](Docs/screenshots/neuramar-standalone.png)

Drop in one sound, let Neuramar infer its root and fit a compact local
synthesis model, then play the result across the keyboard — or start without a
recording and grow a playable neural seed from the **Randomize** controls. Its
**Core**, **Air** and **Bone** layers preserve or invent different parts of a
sound's identity while the front panel invites the memory to stay faithful,
drift, breathe, or become something new.

Neuramar is not a conventional sampler. The imported recording is analysed in
the background but is never repitched or played back when notes sound.
Completed learning produces a small neural controller plus explicit harmonic,
stochastic and modal synthesizers. Rendering is local and requires no cloud
service, GPU, pretrained model or source file.
...
> the Stretch, Formant and Touch controls.

## Audio demos

Eight takes, rendered by [`Tools/RenderDemos.cpp`](Tools/RenderDemos.cpp),
covering the whole learning loop: a procedurally synthesized training tone, the
fitted model replaying it, pitch and velocity generalisation, the Core/Air/Bone
anatomy, formant and stretch control, and a polyphonic pad. Even the training
input is code — no recording is involved anywhere.
...
possible and keep inference bounded. The rationale and the primary papers each
branch follows are recorded in `Source/DSP/NeuralModel.h`, beside the code that
implements them.

A preregistered comparison against named competitor systems — with a held-out
corpus, objective targets and a blind listening acceptance rule — has **not**
been run. "Industry-leading" would have to mean something narrower than
universally better: that in a version-pinned comparison Neuramar achieved the
highest reference-similarity result among the named systems on a stated corpus
while meeting stated pitch, aliasing and real-time gates. No such claim is made
here.
...
library are included. The demonstration audio in `Docs/audio/` is generated
entirely by this repository's own code, training input included.

### Measured resynthesis quality

These are numbers printed by the repository's own regression binaries
(`Tests/ResynthesisQualityTests.cpp`, `Tests/NeuramarEngineTests.cpp`, run
through `ctest`) on deterministic analytic fixtures, reproducible from a clean
checkout. **Read the boundary carefully:** this is a self-comparison across
Neuramar versions on synthetic material. It is not a competitor comparison, not
a listening test, and not evidence about real acoustic instruments.

Root-note reconstruction measures how close the fitted model's render is to the
sound that was dropped in, at the pitch it was analysed at. Two fixtures are
learned and rendered at the source pitch under a frozen `Match` state — full
Imprint, Core, Air and Bone; Mutation, Orbit, Noise and spread off; neutral
tone controls; zero added attack; Body Lock at 0.65. Onsets are aligned at
sample zero, the reference is conditioned the way `learn()` conditions it, and
one RMS scalar is applied to the render.

Across the four successive engine changes the benchmark tracks — a sequential
solve, the joint solve, the joint solve with a halved onset aperture, and the
widened body representation — mean spectral convergence over the three
resolutions improved from 0.0412 to 0.0364 on the source/filter fixture. The
noise-plus-transient fixture moved the other way, from 0.0571 to 0.0754: the
joint solve buys accuracy on tonal material and spends some on transient
material, and that trade is recorded rather than hidden.

A single source note contains no evidence about unrecorded registers,
velocities or articulations, so root-note reconstruction and cross-register
instrument resemblance are reported as separate questions and never averaged
together.
...
measured on a twelve-item analytic corpus by the regression suite and is not a
claim about real recordings. The displayed confidence is an
...
- Added root-note reconstruction measurement — the benchmark the project had
  specified but never reported — along with the joint harmonic solve, the
  halved onset aperture and the widened body representation it scores. See
  [measured resynthesis quality](#measured-resynthesis-quality).

### 1.2

- Core-targeted register normalisation, fixed note-on voice placement, forward
  golden-ratio Orbit looping with level normalisation, frequency-dependent
  Dissolve damping, key-tracked model clocking and velocity-varied strike
  dynamics.

### 1.1

- Added the model-anatomy display and the Stretch, Formant and Touch controls;
  the body grew to 16 Air bands and 12 Bone modes across 16 voices.

### 1.0

- First release: drop in a monophonic sound, infer its root, and fit a compact
  local DDSP-inspired model with fitted stiff-string inharmonicity, whose
  harmonic Core, noisy Air and resonant Bone stay playable across pitches.
  Sample-free Randomize grows a playable seed with no recording at all.
...
cmake -S . -B build-dsp -DCMAKE_BUILD_TYPE=Release \
  -DNEURAMAR_BUILD_PLUGIN=OFF -DBUILD_TESTING=ON
cmake --build build-dsp --parallel
ctest --test-dir build-dsp --output-on-failure
./build-dsp/NeuramarRenderDemos Docs/audio
```

`ctest` also runs `ResynthesisQualityTests`, which prints the figures quoted
under [measured resynthesis quality](#measured-resynthesis-quality).

The full plug-in (JUCE 8.0.14 is fetched pinned at configure time, or pass
`-DNEURAMAR_JUCE_PATH=/path/to/JUCE`):
...
terms — see `THIRD_PARTY_NOTICES.md`. No pretrained weights, datasets, samples
or third-party model data are included.
