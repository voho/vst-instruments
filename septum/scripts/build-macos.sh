...
cmake_args=(
    ...
    "-DSEPTUM_BUILD_UNIVERSAL=${BUILD_UNIVERSAL}"
    -DSEPTUM_BUILD_PLUGIN=ON
    -DBUILD_TESTING=ON
)
...
if [[ -n "${JUCE_PATH:-}" ]]; then
    cmake_args+=("-DSEPTUM_JUCE_PATH=${JUCE_PATH}")
fi
...
echo "  ${BUILD_DIR}/Septum_artefacts/${CONFIG}/VST3/Septum.vst3"
echo "  ${BUILD_DIR}/Septum_artefacts/${CONFIG}/AU/Septum.component"
echo "  ${BUILD_DIR}/Septum_artefacts/${CONFIG}/Standalone/Septum.app"
