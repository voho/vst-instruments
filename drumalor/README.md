...
- 2026-08-17: Deduplicated the pan-clamp-then-constant-power-pair assignment that a triggered voice's `initialiseVoice()`, a sympathetic bed's initial configuration and its per-block pan refresh, and the per-block mixer target update each spelled out separately - `const float pan = std::clamp (values.pan, -1.0f, 1.0f);` followed by a `constantPowerLeft`/`constantPowerRight` pair - into a single `applyPan()` helper that all four now call, matching the pattern the `pushMetallicSubstep()` and `centerPanGain` cleanups already established for this file; `constantPowerLeft`/`constantPowerRight` themselves are unchanged, only now called exclusively from `applyPan()` with the value it just clamped. No change to any pan actually applied. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a full demo re-render leaving `git status drumalor/Docs/audio` clean (byte-identical to the seven committed WAVs).
- 2026-08-17: Added regression coverage for `setInstrumentParameters()`'s own out-of-range instrument guard, which every other call in the suite only ever drove with one of `Instrument`'s own enumerators, by exposing a direct `DrumEngineTestAccess::instrumentValid()` seam onto `validInstrument()` (since proving the guard only through a render side effect would not reliably catch it missing, as the out-of-bounds write it exists to prevent is undefined behaviour) plus a render comparison confirming a real instrument is unaffected by a rejected call; verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a demo re-render showing `git status drumalor/Docs/audio` clean (test-only change, no engine code touched).
- 2026-08-17: Added regression coverage for `setInstrumentParameters()`'s own characterA/characterB/decay guards, each an independent `clampUnit (value, fallback)` - `std::isfinite (value) ? std::clamp (value, 0.0f, 1.0f) : fallback` - that the broad invalid-values stress test only ever drove far enough to confirm the render stayed finite, the same gap the pitch/level/pan guards next to them and the kit's humanise/bleed/busDrive/busCompression guards had before their own passes. A new test compares a Kick struck after each field is set to NaN, +infinity or -infinity against the same Kick struck after the field's own documented default, and an out-of-range finite value against its 0.0/1.0 floor and ceiling, asserting bit-identical output in every case; temporarily widening the shared clamp's ceiling to 2.0 made the new characterA/characterB/decay assertions fail as expected (along with the existing kit-parameter guard tests, since they share the same helper). Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a demo re-render showing `git status drumalor/Docs/audio` clean (test-only change, no engine code touched).
- 2026-08-17: Added regression coverage for `HighResolutionVelocityPrefix::clear()`'s own channel guard, `if (valid (channel)) pending_[...].reset();`. Every existing `clear()` call in the CC 88 bookkeeping test only ever named a channel already inside `[0, 15]`; the out-of-range case was previously driven only through `set()` and `take()`, which happen to share the same `valid()` guard but do not exercise `clear()`'s own copy of it at all. The new assertion sets a real channel's pending prefix, calls `clear()` with channel `-1` and channel `16`, and confirms that channel's prefix is still there afterwards, matching the guard's documented contract that an out-of-range channel is ignored rather than acted on. Review caught that this indirect check alone is not a reliable regression: `pending_[static_cast<std::size_t> (channel)]` for an out-of-range channel is an out-of-bounds array write, undefined behaviour that is not guaranteed to land on the sibling channel's own slot even with the guard removed, so the assertion could still pass with the guard missing. Added a `friend`-exposed `DrumEngineTestAccess::prefixChannelValid()` seam and a direct assertion that `valid()` itself rejects `-1` and `16` while accepting `0` and `15`, pinning the guard's boundary independently of any write. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a demo re-render showing `git status drumalor/Docs/audio` clean (test-only change, no engine code touched).
- 2026-08-17: Deduplicated the ring-buffer write that `resetMetallicOscillatorBanks()`, `wakeMetallicOscillatorBank()` and `renderMetallicOscillatorBanks()` each spelled out separately - render one substep, write it into `decimatorHistory` at `decimatorWriteIndex`, then advance and wrap that index - into a single `pushMetallicSubstep()` helper that all three now call, matching the pattern the `forEachVoice()` and `sanitizeAdaaPair()` cleanups already established for this file. No change to which substeps are rendered, in what order, or what state they leave behind. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a full demo re-render byte-identical (`cmp`) to the seven committed WAVs under `drumalor/Docs/audio`.
- 2026-08-17: Added regression coverage for `getOutputLevel()`'s own channel selector, `channel <= 0 ? outputLevelLeft_ : outputLevelRight_`, which `testMetering()` only ever drove with the two in-range indices 0 and 1, one call each. A new test strikes a hard-left-panned Snare so the two meters read visibly apart, then asserts a negative channel (`-1` and `-100`) collapses onto the same value as the explicit left channel and an out-of-range channel (`2` and `255`) collapses onto the same value as the explicit right channel, rather than the ternary's comparison happening to land on whichever branch by accident. Temporarily narrowing the guard to `channel == 0` made the new negative-channel assertions fail as expected. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a demo re-render showing `git status drumalor/Docs/audio` clean (test-only change, no engine code touched).
- 2026-08-17: Added regression coverage for `setInstrumentParameters()`'s own chokeGroup guard, `std::clamp (values.chokeGroup, 0, chokeGroupCount)`, which every existing choke-group test only ever drove with values already inside `[0, chokeGroupCount]` and the broad invalid-values stress test only drove far enough to confirm the render stayed finite. chokeGroup is private state with no public accessor, so the new test proves the clamp's ceiling the way `testChokeGroups()` proves the group mechanism itself: it sets one voice ringing in the top group, triggers a second voice with `chokeGroup` set to `INT_MAX`, and asserts the ringing voice is still cut - which only happens if the out-of-range value collapsed onto the real top group rather than being passed through as its own group that nothing else ever joins. Temporarily removing the clamp made the new assertion fail as expected. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a demo re-render showing `git status drumalor/Docs/audio` clean (test-only change, no engine code touched).
- 2026-08-16: Unified `Voice`, `SympatheticBed` and `MixerTarget`'s three independently-duplicated centre-pan default literals (two spelled `0.70710678f`, one spelled the less precise `0.7071f`) into a single `centerPanGain` constant, with no change to engine output since every one of those defaults is overwritten before it is ever read.
- 2026-08-16: Shrank `Voice`'s tonal-core `oscillatorCount` from eight slots to two, the actual number `oscillator()` is ever called with (`renderSnare`'s body fundamental/shell and `renderTom`'s head fundamental/shell, and no other voice or index); every trigger of every voice was previously drawing and discarding six unread phase/asymmetry hashes per hit. Slots 0 and 1 keep their exact seed formula, so the change is invisible to `oscillator()`'s only two call sites. Verified with `ctest --test-dir drumalor/build-dsp` (all tests pass) and a full demo re-render showing `git status drumalor/Docs/audio` clean.
