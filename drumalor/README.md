...
A real-time **procedural drum instrument**: VST3, Audio Unit and Standalone for
macOS, plus Linux and Windows builds.

![Drumalor](Docs/screenshots/drumalor-standalone.png)

Drumalor takes classic analogue-drum synthesis techniques as a starting point,
then generates every hit locally from oscillators, noise, filters, envelopes,
resonators and circuit-inspired nonlinear shaping. It loads no samples, copies
no ROM, emulates no particular branded machine, and contacts no service while
rendering audio.

**Thirteen separately playable synthesized voices** — Kick, Snare, Clap, Closed
and Open Hats, Ride, Crash, Low/Mid/High Toms, Shaker and two percussion
voices — each with its own MIDI mapping, synthesis character, pitch and decay
controls, plus a per-voice mixer and a shared drive-and-compression bus.

> The screenshot is the actual Standalone application, captured from the
> version 1.0 build; the 1.1 editor adds the kit mixer, kit bus deck and
> metering described below and has not been re-captured, because that requires
> a macOS build.

## Audio demos

Seven takes, rendered by [`Tools/RenderDemos.cpp`](Tools/RenderDemos.cpp) from
the shipping signal path rather than drawn by hand, and re-rendered whenever
the engine moves: the full thirteen-voice kit one hit at a time, programmed
grooves with ghost notes and a bar played on the hi-hat pedal, snare velocity
and its three articulations, toms and cymbals, the Humanise control, and the
kit bus.
...
| `02-rock-groove.wav` | A plain rock beat at 96 BPM, then the same beat played on the hi-hat pedal | 21.9 s | −9.6 dBFS | +6.6 dB |
| `03-breakbeat-ghost-notes.wav` | A busier break at 104 BPM with ghost-note snare | 10.8 s | −12.0 dBFS | +9.0 dB |
| `04-snare-velocity.wav` | One snare from a ghost note to a full accent, then its rimshot and cross-stick | 9.8 s | −14.2 dBFS | +11.2 dB |
| `05-toms-and-cymbals.wav` | Ride time, crashes and a descending tom fill | 13.1 s | −8.8 dBFS | +5.8 dB |
| `06-humanise.wav` | The same bar machine-tight, at the factory Humanise, then doubled | 13.4 s | −11.9 dBFS | +8.9 dB |
| `07-kit-bus.wav` | One loop dry, through Bus Drive, then glued by Bus Compression | 13.6 s | −9.9 dBFS | +6.9 dB |
<!-- peaks-table-end -->
...
Drumalor deliberately does not use a learned model. Esqueda and Murai's
[2025 antialiased recurrent study](https://dafx25.dii.univpm.it/wp-content/uploads/2025/09/DAFx25_paper_61.pdf)
shows compact learned state-space models can run in real time, but without
measurements from a defined target circuit the weights would be an
uncalibrated black box rather than a more defensible analog model.

Not modelled: any room or ambience, any recorded cymbal (the 909 channel's
tables are generated, not embedded), and any calibrated correspondence to a
particular unit of the machines whose published circuit analyses the cymbal
channels follow.
...
- Added the **kit mixer**: per-voice level, constant-power pan and choke/mute
  groups, with a shared **kit bus** carrying drive and compression, plus
  metering. The version 1.0 parameter block is unchanged and still occupies
  host parameter indices 0–52, so existing sessions and automation lanes keep
  working; every 1.1 control is appended after it and defaults to preserving
  the earlier sound — unity channel level, the original kit pan positions, the
  original hi-hat choke pair and a bypassed bus.
- Added **Kit Bleed**, coupling the kit through sympathetic snare and tom beds.
- Added the two-machine cymbal channel: the 1980 analogue channel and the 1983
  digital one, selected by the Ride's and Crash's Machine control.
- Added membrane-bank tension modulation, split degenerate mode pairs, strike
  position and rimshot/cross-stick articulation on the snare, a continuous
  CC 4 hi-hat pedal, and Hertzian contact dynamics on the cymbals.
- The overdrive is evaluated at a fixed internal rate rather than the host's,
  so its character no longer follows the session's sample rate.
- **Two 1.1 changes are not recoverable by leaving a control alone.** The
  Ride's and Crash's first character control is now Machine rather than Bell
  and Spread, so those two voices play differently from a saved position that
  meant something else — the parameter block is unchanged, so the session still
  loads and every other voice is untouched. And the per-hit analogue variation
  is wider than the depth 1.0 used to mean, having been sized to be measured
  rather than heard, so a kit at the default Humanise no longer reproduces the
  older rendering sample for sample. Setting Humanise to 0 % removes the
  variation entirely if a session needs the tighter behaviour back.
- Fixed `updateSympatheticBeds()` retuning every one of the four sympathetic
  beds when only one needed it.
- Two MIDI aliases earlier versions accepted were retired; see
  [the voice map](#voices-midi-notes-and-controls).

### 1.0.0

- First release: the thirteen procedural voices, their MIDI mapping and
  per-voice pitch, level, pan, character and decay controls, in one JUCE
  codebase producing VST3, Audio Unit and Standalone.
...
cmake -S . -B build-dsp -DCMAKE_BUILD_TYPE=Release \
  -DDRUMALOR_BUILD_PLUGIN=OFF -DBUILD_TESTING=ON
...
`-DDRUMALOR_JUCE_PATH=/path/to/JUCE`):
...
terms — check the publisher and use case before shipping; see
`THIRD_PARTY_NOTICES.md` and JUCE's official licence. No drum samples, impulse
responses, neural model weights, factory ROMs or third-party presets are
included.
