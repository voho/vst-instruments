...
private:
    ...
    // table either way.
    //
    // The TR-909 service notes document the digital channel this models:
    // cymbal memories addressed by a counter at a tunable, divided-down sample
    // clock, read as six-bit companded codes whose envelope is restored after
    // the converter, with that envelope generated by a second converter on the
    // address lines through an anti-log stage so it tracks the clock rather
    // than the wall.
    ...
    //
    // The analogue channel follows Werner, Abel and Smith's TR-808 cymbal
    // circuit analysis for the measured six-oscillator frequencies and pulse
    // duty cycle, the fixed-versus-trimmed split across those six, the two
    // band-pass centres, and the block order - oscillators, band-passes,
    // trigger logic and attack smoother, envelope generators, swing-type VCAs,
    // Sallen-Key high-passes, tone-control mixer, output buffer:
    ...
    //
    // Combining relaxation-circuit state, accurate edge timing and BLEP
    // correction - rather than choosing between physical modelling and
    // antialiasing - follows Olsen, Werner and Germain's network-variable-
    // preserving oscillator study:
