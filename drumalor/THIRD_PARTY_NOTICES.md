...
Drumalor's original source code is covered by the project's `LICENSE` file.
The build downloads or consumes the separate dependency below; that dependency
is not relicensed by Drumalor.
...
distributing Drumalor with JUCE therefore requires either compliance with the
