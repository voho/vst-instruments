...
The rendered peak before normalisation, for the record — this table is
rewritten in place by the renderer itself on every full render, so it cannot
drift away from the committed files:
...
| `14-whammy-and-feedback.wav` | −14.3 dBFS | +11.3 dB |
<!-- peaks-table-end -->
...
(the renderer also rewrites the marked level table in this README, so a model
change shows up in the documentation in the same commit).
