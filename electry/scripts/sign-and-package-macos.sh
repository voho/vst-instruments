...
VST3="${ARTIFACT_DIR}/VST3/Electry.vst3"
AU="${ARTIFACT_DIR}/AU/Electry.component"
APP="${ARTIFACT_DIR}/Standalone/Electry.app"
...
VST3_ARCHS="$(lipo -archs "${VST3}/Contents/MacOS/Electry")"
AU_ARCHS="$(lipo -archs "${AU}/Contents/MacOS/Electry")"
APP_ARCHS="$(lipo -archs "${APP}/Contents/MacOS/Electry")"
...
ditto "${VST3}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Electry.vst3"
ditto "${AU}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Electry.component"
ditto "${APP}" "${PACKAGE_ROOT}/Applications/Electry.app"

NOTICE_ROOT="${PACKAGE_ROOT}/Library/Application Support/Electry/Documentation"
...
mkdir -p "${NOTICE_ROOT}"
ditto "${PROJECT_DIR}/LICENSE" "${NOTICE_ROOT}/Electry-LICENSE.txt"
...
for bundle in \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Electry.vst3" \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Electry.component" \
    "${PACKAGE_ROOT}/Applications/Electry.app"; do
    bundle_notice_root="${bundle}/Contents/Resources/Documentation"
    mkdir -p "${bundle_notice_root}"
    ditto "${PROJECT_DIR}/LICENSE" "${bundle_notice_root}/Electry-LICENSE.txt"
    ditto "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
...
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Electry.vst3"
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Electry.component"
...
rm -f "${DIST_DIR}/Electry-"*-macOS-*.zip
rm -f "${DIST_DIR}/Electry-"*-macOS-*.pkg
rm -f "${DIST_DIR}/Electry-"*-unsigned.pkg

COPYFILE_DISABLE=1 pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier audio.electry.synth.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
