...
std::uint32_t packStringVisual(const StringVisualState& state) noexcept
{
    ...
    const auto articulation = static_cast<std::uint32_t>(
        std::clamp(static_cast<int>(state.articulation), 0,
                   ElectryEngine::keyswitchCount - 1));
...
StringVisualState unpackStringVisual(std::uint32_t packed) noexcept
{
    ...
    state.articulation = static_cast<Articulation>((packed >> 14) & 0x0fu);
