...
NOTICE_ROOT="${PACKAGE_ROOT}/Library/Application Support/YouKnow106/Documentation"
...
mkdir -p "${NOTICE_ROOT}"
ditto "${PROJECT_DIR}/LICENSE" "${NOTICE_ROOT}/YouKnow106-LICENSE.txt"
...
for bundle in \
    ...
    ditto "${PROJECT_DIR}/LICENSE" "${bundle_notice_root}/YouKnow106-LICENSE.txt"
    ditto "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
...
ZIP_PATH="${DIST_DIR}/YouKnow106-${VERSION}-macOS-${ARTIFACT_ARCH}.zip"
PKG_UNSIGNED="${DIST_DIR}/YouKnow106-${VERSION}-unsigned.pkg"
PKG_FINAL="${DIST_DIR}/YouKnow106-${VERSION}-macOS-${ARTIFACT_ARCH}.pkg"

# Leave exactly one YouKnow106 release set in dist, avoiding stale version or
...
COPYFILE_DISABLE=1 pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier audio.youknow106.synth.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
