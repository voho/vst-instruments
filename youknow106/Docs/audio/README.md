...
volume. The output stage's rails sit *before* the volume control, exactly as
on the hardware, so headroom is set where a player would set it — at the VCA
LEVEL slider — and the renderer refuses to write any take whose peak lands
above the rails' provable linear region, because such a take would document
the rails rather than the circuit behind them. The one deliberately
nonlinear voice in the set is the self-oscillating filter, whose sine-like
statistics are the oscillation's own, not clipping.
...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −17.7 dBFS | +14.7 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −16.4 dBFS | +13.4 dB |
| `03-sixteen-foot-bass.wav` | A 16' bassline: the exponential envelope segments doing the punch | 13.8 s | −24.9 dBFS | +21.9 dB |
| `04-filter-brass.wav` | Resonant filter-envelope stabs, ending on a full bender push | 10.3 s | −21.2 dBFS | +18.2 dB |
| `05-self-oscillation.wav` | The filter played as a voice at full resonance and key follow | 12.9 s | −13.1 dBFS | +10.1 dB |
| `06-chorus-modes.wav` | The same pad with the effect off, in mode I, then in mode II | 15.8 s | −19.7 dBFS | +16.7 dB |
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −20.7 dBFS | +17.7 dB |
| `08-delayed-vibrato.wav` | The modulator's two-stage delay fading vibrato onto a held chord | 9.6 s | −19.0 dBFS | +16.0 dB |
| `09-high-pass-ladder.wav` | One bright chord through all four high-pass switch positions | 10.6 s | −19.6 dBFS | +16.6 dB |
| `10-calibration-drift.wav` | A six-voice chord on a perfectly matched unit, then on a worn one | 12.9 s | −24.9 dBFS | +21.9 dB |
<!-- peaks-table-end -->
