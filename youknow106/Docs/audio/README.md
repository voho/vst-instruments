...
volume. Signal order is voice-module VCAs, voice sum, shared high-pass, the
stored VCA LEVEL trim, chorus/IC6 output summers, the final host-rate stereo
output coupling and then the VOLUME potentiometer. That coupling models each
IC6 output's 10 µF capacitor and 1.5 kΩ series resistor loaded by its 10 kΩ
VOLUME track: a 1.384 Hz high-pass with 0.8696 (−1.214 dB) high-frequency
gain before the still-provisional pot law. VCA LEVEL can therefore match
...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −16.1 dBFS | +13.1 dB |
...
| `04-filter-brass.wav` | Resonant filter-envelope stabs, ending on a full bender push | 10.3 s | −19.8 dBFS | +16.8 dB |
| `05-self-oscillation.wav` | The filter played as a voice at full resonance and key follow | 12.9 s | −19.0 dBFS | +16.0 dB |
| `06-chorus-modes.wav` | The same pad with the effect off, in mode I, then in mode II | 15.8 s | −16.2 dBFS | +13.2 dB |
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −14.4 dBFS | +11.4 dB |
...
| `10-unit-character.wav` | A six-voice chord at nominal zero Unit Character, then at full amount | 12.9 s | −19.3 dBFS | +16.3 dB |
<!-- peaks-table-end -->
