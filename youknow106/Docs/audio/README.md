...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −5.2 dBFS | +2.2 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −2.8 dBFS | −0.2 dB |
| `03-sixteen-foot-bass.wav` | A 16' bassline: the exponential envelope segments doing the punch | 13.8 s | −15.3 dBFS | +12.3 dB |
| `04-filter-brass.wav` | Resonant filter-envelope stabs, ending on a full bender push | 10.3 s | −9.8 dBFS | +6.8 dB |
| `05-self-oscillation.wav` | The filter played as a voice at full resonance and key follow | 12.9 s | −13.8 dBFS | +10.8 dB |
| `06-chorus-modes.wav` | The same pad with the effect off, in mode I, then in mode II | 15.8 s | −5.0 dBFS | +2.0 dB |
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −1.3 dBFS | −1.7 dB |
| `08-delayed-vibrato.wav` | The modulator's two-stage delay fading vibrato onto a held chord | 9.6 s | −8.4 dBFS | +5.4 dB |
| `09-high-pass-ladder.wav` | One bright chord through all four high-pass switch positions | 10.6 s | −1.2 dBFS | −1.8 dB |
| `10-unit-character.wav` | A six-voice chord at nominal zero Unit Character, then at full amount | 12.9 s | −8.7 dBFS | +5.7 dB |
<!-- peaks-table-end -->
