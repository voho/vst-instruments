...
| Filter core | A photographed A1QH80017A teardown identifies one IR3109 quad OTA/filter plus two BA662s; the service circuit gives a 68 kΩ input resistor, 560 Ω shunt and 240 pF integrator per stage | The engine advances the continuous four-stage equations directly. Each stage evaluates `omega·gScale·Early·H·tanh((previous − V + offset)/H)` and stage 1 receives the circuit-shaped nonlinear resonance return from stage 4. Two fixed half-interval, five-evaluation Merson RK4 steps advance four double-precision capacitor voltages. A causal current-plus-three-past polynomial supplies seven unique input phases; cutoff, feedback and thermal headroom interpolate between their previous/current endpoints at the same nodes. There is no nonlinear convergence loop, runtime selector, lookahead or extra latency | Device identity and topology/component values remain **anchored**; the Merson realization is an **engine-validated numerical product mechanism**, not hardware evidence. Against an independent full-mechanism RK96 trajectory it reads −162.551 dB / 4.21471e-8 V; all six standard HQ selector cells clear the predeclared −40 dB dynamic gate at −48.293 dB or better, with converged references and zero recovery/count mismatch. The 8 kHz/4× engine-bound extension still rejects at −33.245 dB because converter events are snapped to that coarse internal grid; Step 11 owns fractional event-aware hold evaluation |
...
| Oversampling work attribution | No analogue-hardware claim: this row characterizes only how the numerical product realizes the circuit | The shipping library is timed uninstrumented with a thread-CPU clock. A separate Engine/Chorus build enables compile-time semantic counters for scan, DCO, fixed Merson VCF integration, BBD/BLEP, exact/legacy support and decimation work; it is not timed or linked into the plug-in. CTest requires raw-float fingerprints to equal the shipping build and fences the exact `q·H`, six-card, fixed-VCF, sixteen-slot, two-line, support-transition and 49-nonzero-tap algebra over every production 4×/2×/1× branch | **Model-internal measurement, not a rate-change decision.** The 48 kHz resonant 2,048-frame HQ/HQ-off windows contain 49,152/12,288 VCF steps, 98,304/24,576 Merson substeps, 491,520/122,880 RHS and feedback evaluations, 1,966,080/491,520 stage and full-Early evaluations, 344,064/86,016 input-phase reconstructions and zero recovery. Step 9's BBD counts remain: HQ has 16,384 exact advances per side and 1,966,080 transition MACs; HQ-off has 4,096 legacy input plus 4,096 exact output frames and 245,760 MACs. Three alternating seven-repetition M1 Max audits put current 4×/1× CPU/audio at 0.677/0.170 idle, 0.690/0.178 plain, 0.850/0.228 resonant and 0.736/0.191 full-mixer Chorus II. Step 9 baselines were 0.546/0.152, 0.509/0.159, 0.670/0.192 and 0.786/0.296 respectively. These paired observations are informational and patch-dependent; the unchanged `<5×` Release test is the only coarse performance gate. No split rate is admitted |
| Common-host numerical-quality qualification | No analogue-hardware claim: independent analytic/RK/closed-form references judge only the fidelity with which the product realizes its declared equations | `YouKnow106DcoScanQualityAudit` compares the isolated pre-VCF DCO spectrum, normalized 23-write schedule and DCO/PWM/SUB hold laws with analytic references. `YouKnow106VcfBbdQualityAudit` compares the nominal Character-0 input-to-fourth-pole cascade with a factor-independent fixed-16× RK4 reference and one deterministic BBD line with a closed-form component/128-edge/loss-pole/ZOH-phasor reference. `YouKnow106VcfDynamicQualityAudit` independently evaluates the exact 23-write trajectory, 522 µs holds, all six physical cards and cold/warm Unit Character profiles at actual product selectors. VCF/BBD references cross an independent 4,097-tap FIR to the same host boundary; candidates are never treated as truth | **Numerical qualification only; no hardware or split-rate claim.** DCO passes all six common-host cells. Current VCF 1×/2×/4× hot NRMS is −12.538/−30.414/−50.351 dB at 44.1 kHz and −14.269/−33.028/−50.064 dB at 48 kHz; driven NRMS is −145.593/−113.526/−112.144 and −144.364/−114.710/−113.339 dB; hot residual off-mask is −44.602/−85.968/−133.278 and −48.081/−88.898/−140.552 dBc. The unchanged classification is REJECT/REJECT/PASS per host. All six standard dynamic HQ cells pass from −48.724 to −48.293 dB with convergence ≤−150.9 dB and no recovery/count mismatch; 768 kHz/1× passes −61.360 dB, while 8 kHz/4× remains an expected −33.245 dB REJECT assigned to fractional event timing. Step 9 BBD q4 still passes at both common hosts and every actual HQ selector; lower rates remain as recorded. Hardware constants and fixed 41-sample latency are unchanged |
