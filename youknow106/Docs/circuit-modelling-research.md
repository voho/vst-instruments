...
| Control scan | Service Notes pp. 5, 8 and 13: one 12-bit converter and three 8-way muxes, with 18 per-card holds (DCO, VCF and ENV/GATE VCA for six cards), five shared holds (SUB, stored VCA LEVEL, PWM, RESONANCE and NOISE), one unused channel, and a 4.2 ms pass. The p. 8 chart orders shared RES/VCA/SUB, DCO 1–6, PWM, interleaved VCF/VCA 1–6, then NOISE and depicts them sequentially across the pass | A fractional scheduler preserves the nominal 4.2 ms average without host-rate truncation. Each pass executes the exact 23-write logical queue. The default `NormalizedServiceChart` compatibility profile spaces ordinal `n` at `n/23` of the pass so six DCO resets are not collapsed onto one sample; `PhaseZeroDiagnostic` retains only order for comparison. For VCF and shared RESONANCE only, the renderer purely peeks for an event in `(phase, phase + delta]`, latches its event-time payload, evaluates the 522 µs hold at the exact segmented endpoint and seven Merson nodes, and leaves the normal scheduler to commit that payload once at its next poll. The peek neither consumes nor reorders the official queue; pass-wrap resonance and intervening automation retain the event-time payload. Each destination has a separately named hold constant; VCF 522 µs and voice VCA 687 µs are evidence-backed, the common VCA LEVEL path's post-S/H C7 network derives 9.08249 ms, the PWM hold crosses its derived R117/C62 then R116/C63 cascade (4.7 ms, 2.632 ms) and the SUB hold its derived R11/C1 10 ms pole; equal 522 µs values on the DCO, RESONANCE and NOISE nodes remain compatibility defaults | Topology, nominal pass, ordinal order and qualitative non-simultaneity are **anchored** by the Service Notes. The supplied hash-matched B-2's `b<<7` aligned work word and `b<<5` physical 12-bit DAC code are **ROM-resolved**; the two domains must not be conflated. Fractional VCF/resonance evaluation makes the engine realize its existing normalized policy without host-grid event snapping; it does not upgrade that policy to hardware evidence. Normalized offsets, first-host-snapshot priming, continuous exponential holds and their event-time interpretation are explicit **product/compatibility policy**, not measured timing. Absolute offsets, jitter, acquisition/droop, branches and other destination constants remain OQ-07/OQ-08 |
...
| Oversampling work attribution | No analogue-hardware claim: this row characterizes only how the numerical product realizes the circuit | The shipping library is timed uninstrumented with a thread-CPU clock. A separate Engine/Chorus build enables compile-time semantic counters for scan, DCO, fixed Merson VCF integration, fractional VCF/resonance event handling, BBD/BLEP, exact/legacy support and decimation work; it is not timed or linked into the plug-in. CTest requires raw-float fingerprints to equal the shipping build and fences the exact `q·H`, six-card, fixed-VCF, event-node, sixteen-slot, two-line, support-transition and 49-nonzero-tap algebra over every production 4×/2×/1× branch | **Model-internal measurement, not a rate-change decision.** The 48 kHz resonant 2,048-frame HQ window retains 49,152 VCF steps, 98,304 Merson substeps, 491,520 RHS/feedback evaluations and 1,966,080 stage/full-Early evaluations. Fractional event handling adds 70 peeks and 70 commits across 120 affected voice intervals, with 840 exact control nodes and 720 extra nonlinear maps; it adds no RHS evaluation. Step 9's BBD counts remain as recorded. In a fresh three-pair/seven-repetition 48 kHz thread-CPU cohort, Step 10→11 4× meta-medians are 0.653→0.666 idle, 0.670→0.682 plain, 0.823→0.832 resonant and 0.706→0.719 full-mixer; 1× is 0.164→0.169, 0.172→0.176, 0.221→0.225 and 0.184→0.188. Changes are +1.096…+3.072%; the worst current row is 0.832×, all remain below realtime, and the hard Engine CPU gate passes. These paired values are distinct from Step 10's dated standalone medians and remain machine/patch-specific. No split rate is admitted |
| Common-host numerical-quality qualification | No analogue-hardware claim: independent analytic/RK/closed-form references judge only the fidelity with which the product realizes its declared equations | `YouKnow106DcoScanQualityAudit` compares the isolated pre-VCF DCO spectrum, normalized 23-write schedule and DCO/PWM/SUB hold laws with analytic references. `YouKnow106VcfBbdQualityAudit` compares the nominal Character-0 input-to-fourth-pole cascade with a factor-independent fixed-16× RK4 reference and one deterministic BBD line with a closed-form component/128-edge/loss-pole/ZOH-phasor reference. `YouKnow106VcfDynamicQualityAudit` independently evaluates the exact 23-write trajectory, fractional VCF/shared-resonance 522 µs holds, all six physical cards and cold/warm Unit Character profiles at actual product selectors. It also probes the real scheduler's pure peek, payload latch, pass-wrap and once-only commit semantics, then replays the shipping `renderVoice` path both connected and under a rejected `nullptr` trajectory mutation. VCF/BBD references cross an independent 4,097-tap FIR to the same host boundary; candidates are never treated as truth | **Numerical qualification only; no hardware or split-rate claim.** DCO and the dated common-host VCF/BBD matrices retain their classifications. The current event-aware dynamic matrix covers 19 physical/24 logical profiles and clears the unchanged −40 dB gate: 8 kHz/4× is −84.881 dB, the six standard HQ cells have a −112.406 dB worst and −116.317 dB best result, and 768 kHz/1× is −119.340 dB, with exact schedule/count and finite/zero-recovery contracts. Late/ceil and early/floor snap mutations remain expected rejections at −33.245 and −32.007 dB. Step 9 BBD q4 still passes at both common hosts and every actual HQ selector; lower rates remain as recorded. No physical constant, selector, split, or fixed 41-sample latency changed |
...
- **BBD host-grid aliasing is now separated from physical BBD aliasing.** A
  ...
  selector paths; lower factors remain absolute REJECT. This does not qualify a
  nonlinear whole-line domain, and OQ-01/OQ-03/OQ-04/OQ-20 remain open.
