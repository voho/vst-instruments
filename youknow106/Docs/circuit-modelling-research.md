...
| Oversampling | Standard practice for nonlinear audio | The complete voice, filter, amplifier and both delay lines run at 4x for host rates below 88.2 kHz, 2x below 176.4 kHz, and natively above, followed by a 95-tap Kaiser (β = 7.857, the standard 80 dB design) half-band per stage. The longer boundary is required by the expanded DCO matrix at 44.1 kHz; 63 taps leaked a legitimate 25.1 kHz sixth pulse harmonic back near 19.0 kHz. Filter/VCA audio coefficients update at every internal sample, so their wall-clock bandwidth does not change with HQ. A requested live rate change waits for voices and musical tails, then a block-size-independent 5 ms fade brackets rebuilding sample-grid histories. Converter/LFO/DCO phases, BBD buckets/clock/RNG state and C14/C12/C17/C20 coupling states survive; OTA carries are retimed, while exact or legacy chorus support state, BBD input history and output polyBLEP slots deliberately clear at zero gain | Genuine internal oversampling with filtered decimation, not a quality label. Raw numerical centres are 24 host samples at 1×, 35.5 at 2× and 41.25 at 4×. Integer pads of 17/6/0 make them 41/41.5/41.25, so every path reports 41 host samples and remains within 0.5 sample of that coordinate. The report is 0.930 ms at 44.1 kHz, 0.854 ms at 48 kHz, 0.427 ms at 96 kHz and 0.214 ms at 192 kHz. It covers numerical oscillator reconstruction/decimation delay only, not converter scan, envelope/VCA holds, host/device buffers or BBD wet delay. Without HQ, physical BGA can fold according to the modeled BBD/support chain while polyBLEP reduces the additional SGA; HQ moves that numerical boundary and the filtered decimator defines what reaches the host. Neither mechanism is described as deleting all physical BGA. The exact support coordinates are physical voltages rather than timestep-embedded carries, but preserving or reseeding them has not been qualified. The transition fade and selective numerical/support reset are click-prevention product policies, not reference-unit behavior |
| Oversampling work attribution | No analogue-hardware claim: this row characterizes only how the numerical product realizes the circuit | The shipping library is timed uninstrumented with a thread-CPU clock. A separate Engine/Chorus build enables compile-time semantic counters for scan, DCO, VCF solver, BBD/BLEP, exact/legacy support and decimation work; it is not timed or linked into the plug-in. CTest requires its raw-float fingerprints to equal the shipping build and fences the exact `q·H`, six-card, sixteen-slot, two-line, support-transition and 49-nonzero-tap algebra over all production 4×/2×/1× branches | **Model-internal measurement, not a rate-change decision.** At 48 kHz the resonant 2,048-frame HQ window counts 16,384 exact input and 16,384 exact output advances, 196,608 physical-coordinate updates and 1,966,080 transition MACs. HQ-off counts 4,096 legacy input frames, 4,096 exact output advances, 24,576 coordinate updates and 245,760 MACs. A 2× window counts 8,192 of each exact advance, 98,304 updates and 983,040 MACs; a high-rate 1× window counts 4,096 of each, 49,152 and 491,520. Every exact advance is six coordinates/60 MACs. The current Step-9 48 kHz M1 Max observations are 0.578/0.167× realtime idle, 0.547/0.178 plain, 0.719/0.205 resonant and 0.830/0.315 full-mixer Chorus II at 4×/1×; they are non-gating and alternating Step-8/current runs do not support a paired regression claim. Heterogeneous counters are not cycle weights, and coupled-domain split behavior remains unqualified. No split rate is admitted |
| Common-host numerical-quality qualification | No analogue-hardware claim: independent analytic/RK/closed-form references judge only the fidelity with which the product realizes its declared equations | `YouKnow106DcoScanQualityAudit` compares the isolated pre-VCF DCO spectrum, normalized 23-write schedule and DCO/PWM/SUB hold laws with analytic references. `YouKnow106VcfBbdQualityAudit` compares the nominal Character-0 input-to-fourth-pole cascade with a factor-independent fixed-16× RK4 reference and one deterministic BBD line with a closed-form component/128-edge/loss-pole/ZOH-phasor reference. VCF and BBD references cross an independently designed 4,097-tap FIR to the same 44.1/48 kHz host boundary; 1×/2×/4× are candidates rather than truth. Its BBD scope is four deterministic low-drive cases with one qualifying BGA line per cell, not a nonlinear whole-line oracle. A second BBD matrix follows the engine's actual 44.1–192 kHz HQ/HQ-off selector | **Numerical qualification only; no rate split admitted.** DCO still passes all six common-host cells and the production-compensated hot VCF still rejects every factor. With Step 9's exact continuous support, BBD common-host 1×/2×/4× analytic NRMS is −3.511/−18.390/−53.442 dB and −5.263/−20.051/−56.101 dB at 44.1/48 kHz; BGA error is 4.764/0.070/0.011 dB and 3.406/0.016/0.008 dB; SGA is −26.934/−41.304/−71.831 dBc and −30.746/−46.044/−65.381 dBc. Both q4 cells pass all absolute fixture gates; all lower cells remain overall REJECT. The bounded fixture passes on all six actual HQ paths at 44.1/48/88.2/96/176.4/192 kHz. The four HQ-off paths pass only predeclared Step-8 nonregression allowances of +0.75/+0.25/+1.5 dB for NRMS/BGA/SGA and remain absolute failures. The BBD oracle evaluates the same declared anchors and is not a second hardware fit. Character 1, nonlinear whole-line behavior, inter-domain reconstruction and whole-engine output remain unqualified; production factor selection, hardware constants and 41-sample latency are unchanged |
