...
- **OQ-18 gains a measured comparison curve**, which confirms the exponential law,
  ...
  write overwrote.
- **Implementation measurements.** The exploratory alias calculation put the
  oscillator alone at −111.5 dB and a bright resonant VCF at −55.5 dB, then
  projected −87.7 dB with the filter on a doubled internal grid. That last
  figure is a projection, not the built result below. `testAliasFloor` cannot observe this, since it runs
