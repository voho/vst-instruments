...
| Filter core | A photographed A1QH80017A teardown identifies one IR3109 quad OTA/filter plus two BA662s; the 68 kΩ input resistor, 560 Ω shunt and 240 pF integrator per stage descend from the JUNO-60's printed circuit (sibling) and the reconstruction lineage — the 106's own p. 9 module drawing prints the identical per-stage topology at junction level but no component values (2026-08-20 read, twice-verified) | The engine advances the continuous four-stage equations directly. Each stage evaluates `omega·gScale·Early·H·tanh((previous − V + offset)/H)` and stage 1 receives the circuit-shaped nonlinear resonance return from stage 4. Two fixed half-interval, five-evaluation Merson RK4 steps advance four double-precision capacitor voltages. A causal current-plus-three-past polynomial supplies seven unique input phases. Ordinary cutoff/feedback/headroom trajectories remain endpoint-linear; an interval containing a VCF or shared-resonance event instead receives exact 522 µs cutoff and feedback values at those seven nodes, segmented at whichever event occurs. This adds six nonlinear control maps per affected voice interval without splitting the solver or changing its fixed ten RHS evaluations. There is no nonlinear convergence loop, runtime solver/discretization selector, audio-sample lookahead or extra latency; the separate numerical-transfer selector leaves that topology fixed | Device identity and topology remain **anchored** — the 2026-08-20 junction-level read of the p. 9 drawing confirms the four-stage series/feedback/shunt/integrator pattern, the shared EXP CONV distribution to all four stages, and the two-ground split as Roland-printed. The component values are **sibling-printed** (JUNO-60 p. 9 prints 68K/560/240P; the 106's manual prints none), corroborated by the AS3109 datasheet's 240 pF/68 kΩ test condition, with one value under live contradiction: the 240 pF integrator, which the Open80017a reconstruction reads as 270 pF in the same position (traceable 2026-08-20 to clone-parts substitution with no claimed hardware measurement), is carried **voiced pending OQ-18** rather than as an unqualified anchor; the Merson and fractional-event realization is an **engine-validated numerical product mechanism**, not hardware evidence. Against an independent full-mechanism RK96 trajectory the focused integrator reads −162.551 dB / 4.21471e-8 V. The dated Step-11 HQ matrix remains intact: 8 kHz/4× passes at −84.881 dB, the six standard HQ paths span −112.406 to −116.317 dB, 768 kHz/1× reads −119.340 dB, and its late/early snap mutations reject at −33.245/−32.007 dB. Step 13 adds audit-only HQ-off evidence. Across the same 19 physical/24 logical moving-control coverage, q1 passes at −53.279/−84.738/−86.568/−97.893/−99.618 dB for 8/44.1/48/88.2/96 kHz, but the 8 kHz result is a separately labelled moving-only endpoint. The static nominal production-hot saw reads −12.538/−14.269/−30.417/−33.080 dB at the four standard rates, so every standard HQ-off row remains **REJECT** under the combined smooth-plus-hot rule. This identifies lower-grid input-density/nonlinear pre-grid foldback as a numerical limitation; it changes no production DSP or hardware claim. Exact normalized offsets and the 522 µs event law remain compatibility policy; OQ-07/OQ-08 remain open |
| Filter drive level | The 68 kΩ/560 Ω divider attenuates each stage's differential input by `560/(68000+560) = 0.00816803`, or 122.43:1 | That attenuator, not a user-facing "drive" control, sets the differential pair's nonlinear span; the engine currently uses `+/-6 V` saw/pulse, `+/-5 V` sub (briefly 2.0 on 2026-08-19; reverted the same day, see OQ-15) and `+/-2 V` noise coordinates followed by a 0.40 scale | The component attenuator and OTA span are **derived**. A centered `+/-6 V` interpretation is merely compatible with a 12 Vpp reading at the same source node; it does not establish the loaded transfer. The sub/noise coordinates and 0.40 mapping are **voiced compatibility** pending OQ-15 |
...
| Voice tolerance | The service procedure calibrates each card, but no qualifying repeated six-card/multi-unit data set fixes the residual population or thermal process; the digital envelope generator is shared | The calibrated nominal profile has zero inter-voice spread and zero drift. Existing deterministic seeded offsets/wander are available only as optional `Unit Character` compatibility/sound-design behavior, attached to physical voice slots. As of 2026-08-20 the VCF trim residual is bounded by Roland's printed acceptance (p. 19 procedures 7/8, "within ±10 cents", checked jointly): one ±10-cent draw at the code-6272 FREQ check point, one at the WIDTH point two octaves up, the line through them elsewhere (`vcfTrimResidualOctaves`/`vcfFreqTrimAnchorCounts`/`vcfWidthTrimSpanCounts`), replacing former voiced ±0.07-octave/±5%-of-total-counts magnitudes no source bounded; the PWM comparator's ±0.24 V was already the printed 48–52 % window. A separate engine-level `aging` extension (default 0, not a host parameter) applies the one documented recalibration lead — up to a quarter tone of flatward per-card VCF drift on a seeded unit weight plus the account's +3.52 dB noise-trim drift | Zero nominal is **product policy** in the absence of measurements; the optional distribution remains **voiced** pending OQ-10, with the two VCF trim check points now **anchored acceptance bounds** rather than voiced spans (the untrimmed VCA/resonance/level magnitudes stay voiced — p. 19 prints no tolerance for them). The `aging` magnitudes are **voiced, single-unit lineage** (the 2026-08-20 pass's recalibration lead); OQ-10's population data owns any promotion. A fixed seed must reproduce exactly and Character amount zero must collapse all cards to nominal. Shared CV ownership permits downstream card error but rules out independent envelope laws or six independent sub/noise controls |
| Portamento | Hash-identified B-2 reads an 8-bit raw ADC value; zero is direct/immediate, nonzero selects an 8-bit coefficient by `raw>>1`, and index zero is also immediate. Six 8.8-semitone slot states advance by constant add/subtract and clamp, including while inactive | Raw 0/1 are immediate; paired active raw codes share a coefficient; `octave_passes=ceil(12*256/c)`. The coefficient region `0x0A00–0x0A7F` hashes to `06d1c862622b5aaa2b7e42d561dbdf2cd424620a8e46cfa0c2c9deb5c484984e`. As of 2026-08-20 the knob reaches those raw codes through the bender board's own loaded divider (`portamentoTravelAdcFraction`): the 50KB linear track's wiper against R16 47 kΩ, `x·R_L/(R_L + x(1−x)·R_T)`, 0.395 of full scale at half travel, exact at both ends, applied at the knob boundary in the engine and the parameter's text/value functions with an exact closed-form inverse; the raw-code-addressed laws stay byte-exact | Digital mapping, state width, direction, clamp and inactive-slot behavior are **ROM-resolved** for the stated B-2; the knob-to-ADC-fraction transfer is **derived** from the p. 16 network (2026-08-20 read). Pot taper tolerance, noise/hysteresis, sampling cadence, physical pass timing and revision comparison remain OQ-14 |
...
| Output stage | Service-note signal order: voice VCAs, 0.1-per-voice summer, C14, shared HPF, C12/common VCA LEVEL and chorus/final summer IC6, identified on p. 15 as TA75558S. Each IC6 output then crosses C17/C20 10 µF and R54/R57 1.5 kΩ into one 10 kΩ track of the dual VR1 VOLUME control, marked `10KB×2`. Each wiper sees the complete 41.3 kΩ selector ladder in parallel with the 101 kΩ IC7/headphone input, or 29.313 kΩ, before any external load | C17/C20, R54/R57 and the nominal-linear tracks run as one position-dependent host-rate network. For shaft position `x`, `Z=(10kx)||29.313k` and `Vw/VIC6=Z/[1.5k+10k(1−x)+Z]`; gain is 0.39655 at half and 0.83252 at full (normalized midpoint 0.4763), while the same resistance moves the coupling corner. A 5 ms shaft glide prevents automation zippering. The fixed pre-jack High-tap product boundary then applies `digital=analogue*10^(-18/20)/Vref_rms`, permits floating samples beyond `+/-1`, and adds no limiter | IC6 identity, placement, named parts, fixed internal loading and linear transfer are **anchored/derived**. Panasonic's later JIS/EIAJ table maps plain B to the nominal-linear 1B resistance law, replacing the unsupported squared taper. A TA75558S identity and rail labels do not establish its loaded in-circuit swing; the part's datasheet comparison column is on file as of 2026-08-20 (V_OM min ±12/typ ±14 V at R_L = 10 kΩ — the row the computed ≈ 9–11.5 kΩ wiper-network load selects — V_OMR min ±10/typ ±13 V at 2 kΩ, SR typ 1.0 V/µs against a curve-implied 1.6–2.5 V/µs knee; the modelled 13.5 V bound sits inside the 12–14 V bracket and the 1.7 V/µs slew inside the curve band), population values that do not substitute for the board measurement. Real dual-gang tracking/tolerance, selected-tap loading, R64/R65, C21/C22, jack normaling, one-versus-two-plug transfer, external loads, loaded IC6 clipping (OQ-05), absolute `Vref_rms` and driven headphone output remain open |
| Velocity and MIDI modulation | The keyboard is not velocity sensitive and Note On uses a fixed value; the owner's MIDI implementation chart recognizes CC 1 Modulation and CC 64 Hold. The chart itself prints "velocity ignored" on both Note Off and Note On (`vvvvvvv = 1 - 127`) | Velocity is an explicit extension defaulting to zero. When turned up it scales both the amplifier control and the ENV amount into the VCF by the same `1 − velocityDepth·(1 − velocity)` gain, so it rides the two paths the panel already has rather than a curve of its own. CC 1 drives the bender lever's forward/LFO modulation axis and is scaled by the panel BENDER LFO depth, matching the documented MIDI path | Velocity response is a plug-in extension inert at zero, and the chart's own "velocity ignored" is what makes it an extension rather than a gap; CC 1/64 reception is **anchored**, not an extension |
| Hold (CC 64) threshold | The chart's two receive rows split at **zero**, not at the MIDI 1.0 convention's 64: `0100 0000 \| 0` is "hold OFF" and `0100 0000 \| 0vvv vvvv` is "hold ON  vvvvvvv = 1 - 127" | Any non-zero value latches the hold on; only an explicit zero lifts it | **Anchored** on the printed rows, and fenced by `testHoldLatchesOnAnyNonZeroValue`, which fails on the `>= 64` convention. Read 2026-08-23 off the rendered page-35 scan of the owner's manual (SHA-256 `e9315efe…8902efe`); the PDF has no text layer |
| Channel-mode messages | Under RECOGNIZED RECEIVE DATA the chart prints ALL NOTES OFF (123), OMNI OFF (124), OMNI ON (125), MONO ON (126) and POLY ON (127), then notes "Mode messages (123 - 127) are also recognized as ALL NOTES OFF" and "The JUNO-106 does not respond to MONO ON message". Its mode-resolution table resolves every OMNI OFF/OMNI ON × POLY/MONO combination to **POLY**. Power-up receiver mode is "OMNI ON, POLY" | All five numbers release the keyboard through the normal envelope path. No mode state is kept: the plug-in is unconditionally omni, which *is* the chart's power-up mode | Reception of 123–127 as All Notes Off is **anchored** and fenced by `testChannelModeMessagesReleaseLikeAllNotesOff`. Omni-always is defensible rather than absent, but the chart's OMNI OFF basic-channel filtering is a real printed receiver behaviour this product does not implement — **declared scope**, and it would need a basic-channel parameter. CC 120 All Sound Off and CC 121 Reset All Controllers are host-hygiene **extensions**, absent from this 1984 chart |
...
14. **CD4051 multiplexer charge injection — removed, as unreachable and unquantifiable**:
    ...
    **The stated blocker went stale, and the derivation now closes (2026-08-19).** This entry said $\Delta Q / C_{\text{hold}}$ "needs the hold capacitance and the mux on-resistance — neither of which is established". The hold capacitance *is* established: OQ-07's 2026-08-07 designator read puts **all 23 holds at 0.01 µF** (C65–C72, C73–C79, C80–C87, p. 13 ".01×7/.01×8"), and this entry was not updated with it.

    With that value the size follows. The anchored cutoff CV divider (OQ-10: a $\approx 1/38$ network turning the 10 V DAC span into $\approx 266$ mV at pin 6, at 19 mV/octave, so 14 octaves) puts **714 mV per octave at the hold node**. A CD4051-class injection of 1–10 pC into 0.01 µF is 0.1–1.0 mV, hence **0.24–2.42 cents of cutoff**. For scale, the R-2R ladder's major-carry INL that this model *does* carry is $-4.64/+23.31/-4.48$ cents — an order of magnitude larger.

    It stays unimplemented, but for a better reason than "unquantifiable". $\Delta Q = C_{gd}\,\Delta V_{\text{gate}}$ is set by a fixed logic swing, so the dominant term is a **constant offset per destination, not a modulation** — precisely what the service procedure's per-card cutoff trim nulls. Only the weak channel-voltage dependence of $C_{gd}$ survives calibration, and that residual is second-order on top of a figure already ten times under a mechanism the engine models explicitly. The mux on-resistance still belongs to **OQ-07** for the acquisition question, but it is not what stands between this entry and a number.
...
22. **R-2R converter major-carry glitch — removed as an impulse, reinstated as an offset**:
    ...
    The separate zipper texture the impulse was meant to supply would come from modelling each hold capacitor as tracking only while its multiplexer window is open and holding afterwards, which follows from the anchored 23-writes-per-4.2 ms scan. **[Superseded 2026-08-20 — it is bounded to inertness, not blocked for want of a source.]** With the 0.01 µF holds read at designator level and the HD14051B on-resistance rows, the in-window acquisition constant is 0.8–10.5 µs across the candidate supply rows — 8-bit settling in 4.4–66 µs, far inside even the drawn half-slot — so the physical hold is a per-pass staircase into the downstream smoothing networks, and the engine's exponential-toward-target realisation from the event time is equivalent to that staircase-plus-RC at nominal precision. No DSP change follows. **OQ-07** stays open for installed-unit values, jitter and the charge-injection budget.
...
temperature, so the onset of the OTA's compression shifts over the warm-up.
That is where the modelled thermal behaviour lives — and *only* there.
`thermalWarmupFraction_` has exactly three readers in tree, and the one that
reaches audio is `dynamicOtaHeadroomVolts`; `refreshVoiceCardThermalScales`
builds each card's cutoff tempco from the **time-invariant** spatial gradient
alone. **The modelled cutoff reference carries no warm-up term at all**, and
that absence is deliberate: R111's 560 Ω positor is fitted precisely to cancel
the 0.33 %/°C tempco, and how much it leaves is OQ-10's unmeasured residual, so
a time-varying cutoff drift would be an invented trajectory. Roland's printed
mandatory 10-minute VCF warm-up sits in tension with the voiced τ = 900 s
(48.7 % complete at 600 s); that is recorded as an observation and is **not** a
licence to retune τ.
...
The 2026-08-20 evidence search added: the two Roland Service Information
bulletins appended to the synfo.nl scan (100222, 100229); pixel-measured reads
of the Service Notes p. 8 D/A & S/H timing chart and p. 9 module-internal
drawing (twice-derived, independently); the Intel 82C54/8254 datasheets (edu
mirrors — closing the recorded 8253-retrieval negative); Toshiba TA75558
(1998-02-20), Toshiba TC4051/4052/4053B (2016), Hitachi HD14051B, Toshiba
2SK30A, NEC µPD7810/7811 databook pages, TI TL072/TL082, Rohm BA6110 and Alfa
AS3109/AS662 sheets; the JUNO-60 Service Notes (sibling, provenance
adjudication only); MAME's `juno106` catalogue metadata (hashes only); and
targeted reads of KR-106 v2.5.13, the Alpes Machines One-O-Six kit guide, the
atosynth reconstruction blog and the Cornutt workbench pages. Findings,
corrections and negatives are recorded in `Docs/open-questions.md` §
