...
| Noise | One shared generator mixed into every voice through a per-voice level CV | A single shared source added to each voice before the high-pass, its level a scanned, slewed, 7-bit per-voice control voltage | **Anchored** topology and control path (sysex byte 4); the 4 Vpp mix weight is **anchored in 1.3** — the service procedure sets 4.0 Vpp at TP8 via VR32, which is the target an earlier revision had to voice. The value is unchanged: it was voiced correctly |
| High-pass | A four-position switched RC network ahead of the filter, one leg per voice, selected by a CMOS switch; the setting is a patch bit | Position 0 is the measured boost shelf: +10.5 dB at DC falling to +1.41 dB in the high band across a corner near 59 Hz. Position 1 passes the band untouched. Positions 2 and 3 are single-pole high-passes at 225.8 Hz and 707.4 Hz, each leg's own series capacitor (47 nF, 15 nF) against one shared 15 kOhm shunt, switched by a CMOS multiplexer. **Corrected in 1.3**: an earlier revision read 44.9 kΩ against 15 nF and 4.7 nF produce | **Anchored** — the boost was verified against a hardware noise sweep and is three times the +3 dB an earlier account reported; the cut corners follow from the part values. Modelled at first order; the measured shelf's second, nearly-cancelling pole-zero pair is dropped |
...
| Voice tolerance | Per-voice analogue dispersion: two filter trimmers per voice (offset and scale of the cutoff CV, the ~±5% module figure), regeneration-amplifier gain spread, comparator offset, ramp-current spread, amplifier offset and gain spread, per-voice level-CV and high-pass part spread — and *no* envelope dispersion, because the envelopes are computed digitally in one processor and are identical across voices | One deterministic draw per mechanism, scaled by one Calibration control; slow thermal wander on the analogue chain only | **Anchored in 1.4** for two, and one mechanism deleted. The comparator holds at +/-2 duty points, which the Service Notes state twice (48-52% at the 50% setting, 93-97% at PWM 10). The ramp is +/-3% from its parts' own classes -- charging resistors marked FX, 1% metal-oxide film, against 1 nF capacitors coded G, 2% -- which is legitimate only because the ramp has no trimmer. Resonance went back to **voiced**: 1.3 anchored 5% to a component class, but that loop *is* trimmed, and a trimmed mechanism's residual is not its parts' tolerance. Per-voice high-pass dispersion is **removed**: the schematic carries one set of high-pass parts, not six. An earlier revision drew a per-voice envelope-rate error, which the digital generator makes impossible. Calibration at 0 gives a perfectly matched instrument |
...
| Chorus modes | Two independent latching buttons on the panel; the patch memory stores the effect as one on/off bit and one mode bit | Four panel states -- off, I, II and I+II -- of which the first three are storable | **Reclassified in 1.4 as an addition, not a model.** The Service Notes and the owner's manual both settle this against the model: the manual says outright that I and II cannot be used together, and the board carries one enable bit plus one binary I/II bit rather than two independent switches. I+II is kept as a deliberate feature of this plug-in -- it is what owners fit a board modification to obtain -- and its *behaviour* is still derived from the circuit, the two timing resistors in parallel so the rates add. What is no longer claimed is that the instrument does it. The 1.376 Hz is also not the Juno-60's 9.75 Hz both-down setting: that is a different chorus circuit. *Superseded:* 1.1 had argued the buttons were independent and both-down a documented audibly distinct setting, and it is simply not recallable from a patch. The SysEx writer reports the loss rather than hiding it |
| Chorus I+II rate | Each button switches its own resistor into the modulation oscillator's timing network | Closing both puts the two in parallel, so the conductances and with them the rate add: 1.376 Hz, with depth, centre delay and line gain untouched | **Derived** from the stated topology, now independently corroborated: a second account describes the same parallel-conductance switching into an op-amp integrator and reaches the same 1.376 Hz. Neither gives the resistor values, so the topology is still a description rather than a part list. What is anchored is the direction: both-down is faster than either alone, not a repeat of II |
| Chorus nonlinearity | The delay line is the first thing in the wet path to overload: rated 0.3% distortion at 0.78 Vrms with the surrounding op-amps on ±15 V rails, and the sibling measurement observed wet-only distortion when driven | The line's write saturates at the bias window's ≈2.9 V swing, so a hot bus grits the wet while the dry stays clean — the driven-chorus signature | **Anchored** mechanism and rated point; the pre-line divider is **removed in 1.4 because it does not exist**. 1.3 added one on a misreading: the 100 kOhm at each line input injects adjustable DC bias from VR1/VR2 rather than forming a divider's lower leg, and the 10 kOhm against 2.2 nF beside it is a low-pass pole. Roland sets the drive by calibration instead -- VR1/VR2 trimmed for symmetrical clipping with 10 Vpp at 1 kHz on module-board TP2 |
| Chorus charge transfer | Per-stage charge-transfer loss condensed to one pole at the clock rate. The MN3009 datasheet states −3 dB at 12 kHz on a 40 kHz clock, putting the half-power point at 0.3 of the clock rate; the per-stage-loss literature had only bounded the aggregate Nyquist droop to a 1–4 dB band | Per-edge retention 0.7725, the value that lands the half-power point where the datasheet puts it, i.e. 4.0 dB at the clock Nyquist | **Anchored in 1.3** to the part's own datasheet, replacing a value voiced from the middle of the inferred band. The dark end of that band, not its middle: the 2 dB it replaces left the line essentially flat to Nyquist, passing treble the hardware loses in the line. An earlier revision's retention of 0.34 erred the other way, parking a modulation-swept lowpass at 1.6–5.2 kHz |
| Chorus noise | No compander anywhere in the circuit, giving roughly 55–65 dB in-circuit signal-to-noise against the part's own 88 dB datasheet figure | A modelled per-line noise floor, defeatable by a control the hardware does not have | **Voiced, and back to voiced in 1.4.** 1.3 briefly anchored 60 dB to a stated measurement; the Service Notes round found the underlying capture reports noise level alone -- no reference tone, calibrated level, weighting or bandwidth -- so it cannot be converted into a signal-to-noise figure at all. The 55-65 dB band still has no citation. The value is unchanged either way. The missing compander is what the effect is known for |
| Chorus support filters | A published fifth-order model of this circuit puts the input filter at 9.9 kHz and the output at 9.5 kHz; a direct measurement of a sibling's wet path fits a *single* pole at 14 kHz across the audio band | One pole at each published corner | **Anchored in 1.4, from the Service Notes schematic.** Five poles ahead of each line -- two emitter-follower Sallen-Key sections on equal 22 kOhm pairs (820 pF/680 pF, then 1.8 nF/270 pF, giving 9.69 kHz at Q 0.549 and 10.38 kHz at Q 1.291) plus one passive pole at R122 10 kOhm against C52 2.2 nF, 7.23 kHz -- and the same two sections again after it. The single pole at 9.9/9.5 kHz this replaces was a guess at a fifth-order response rather than the response itself, and was far too bright: four poles near 10 kHz reach -12 dB at 15 kHz where one reaches -4. The sibling's single-pole 14 kHz fit is now explained as not describing this circuit. The output's own fifth pole is the tap-summing node, whose corner depends on the part's unspecified output impedance, so it is left out rather than voiced at a number doing no work. *Superseded note:* 1.3 rejected a candidate answer whose A transcription arrived giving second-order Sallen-Key stages at 10.22 kHz and 9.48 kHz -- corners that sit right on the published 9.9/9.5 pair and explain the 14 kHz outlier as test-gear loading -- but its component values do not produce its own corners: 33k/12k/2.2n/1.0n gives 5.4 kHz against a claimed 10.22, and 39k/12k/47n/82n gives 118 Hz against a claimed 9.48, an eighty-fold miss. The same designators are also given two incompatible roles, as Sallen-Key series resistors and as a series/shunt divider. The corner is better supported than it was, since two accounts now put it near 9.5-10 kHz; the topology and the values are not. Where the corner *lands* is not voiced: the suite bisects the realised half-power point of the coefficient and the recursion together, at both rates the engine runs |
