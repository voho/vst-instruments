...
| Oversampling work attribution | No analogue-hardware claim: this row characterizes only how the numerical product realizes the circuit | The shipping library is timed uninstrumented with a thread-CPU clock. A separate Engine/Chorus build enables compile-time semantic counters for scan, DCO, VCF solver, BBD/BLEP and decimation work; it is not timed or linked into the plug-in. CTest requires its raw-float fingerprints to equal the shipping build and fences the exact `q·H`, six-card, sixteen-slot, two-line and 33-nonzero-tap algebra over 48 kHz 4×/1×, 96 kHz 2×/1× and 192 kHz 1× | **Model-internal measurement, not a rate-change decision.** On the declared M1 Max/arm64 Release fixture, whole-engine 4×/1× thread-CPU ratios span 2.589–3.684. At 48 kHz the resonant 2,048-frame window counts the same 10 converter passes, 61 DCO wraps and 3,162 BBD shifts on both grids, while internal scan/chorus work, six-card DCO/VCF work and sixteen-slot hold/PWM work scale exactly 4:1; 4× alone adds 405,504 stereo nonzero-tap MACs. Heterogeneous counters are not cycle weights, and coupled-domain output error is unmeasured here. No domain is admitted to a lower grid; the common-host qualification below now supplies the isolated reference matrix, while this attribution row itself still admits nothing |
| Common-host numerical-quality qualification | No analogue-hardware claim: independent analytic/RK/closed-form references judge only the fidelity with which the product realizes its declared equations | `YouKnow106DcoScanQualityAudit` compares the isolated pre-VCF DCO spectrum, normalized 23-write schedule and DCO/PWM/SUB hold laws with analytic references. `YouKnow106VcfBbdQualityAudit` compares the nominal Character-0 input-to-fourth-pole cascade with a factor-independent fixed-16× RK4 reference and one deterministic BBD line with a closed-form component/128-edge/loss-pole/ZOH-phasor reference. VCF and BBD references cross an independently designed 4,097-tap FIR to the same 44.1/48 kHz host boundary; 1×/2×/4×, including shipping 4×, are candidates | **Numerical baseline only; no factor admitted.** At 1×/2×/4×, DCO worst off-mask bins are −12.780565/−36.596878/−42.618000 dBc at 44.1 kHz and −16.741087/−36.344575/−41.452375 dBc at 48 kHz: all reject −70 dBc despite passing analytic controls, scan and declared holds; the cause is **unattributed**. Production-compensated hot VCF NRMS is −1.110/−12.232/−24.343 dB and −1.062/−13.751/−25.810 dB: all nominal cells reject −40 dB although narrower RK/self-oscillation fixtures and the 2×/4× exhaustive 20 Hz–20 kHz residual off-mask gate pass. At 44.1/48 kHz respectively, BBD analytic NRMS is −3.099/−14.910/−27.043 dB and −4.640/−16.427/−28.183 dB; its one qualifying wanted-image gain error is 34.389/4.090/0.869 dB and 22.893/3.257/0.708 dB; exhaustive 20 Hz–20 kHz unmasked SGA is −24.854/−28.762/−47.635 dBc and −28.871/−31.329/−38.189 dBc. Only analytically validated wanted physical lines at or below 20 kHz are masked, and all BBD cells reject. Its oracle evaluates the same declared anchors and is not a second hardware fit. Character 1, inter-domain reconstruction, whole-engine output and latency remain unqualified; production rates and DSP are unchanged |
...
- **Implementation measurements.** A historical narrow C6-saw projection put
  ...
  the expanded audit now rejects every DCO factor, with 4× worst bins of
  −42.618 dBc at 44.1 kHz and −41.452 dBc at 48 kHz. Its fold cause remains
  unattributed, so the historical oscillator figure must not be used as a
