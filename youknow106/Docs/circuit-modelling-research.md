...
| Chorus nonlinearity | The delay line is the first thing in the wet path to overload: rated 0.3% distortion at 0.78 Vrms with the surrounding op-amps on ±15 V rails, and the sibling measurement observed wet-only distortion when driven | The line's write saturates at the bias window's ≈2.9 V swing, so a hot bus grits the wet while the dry stays clean — the driven-chorus signature | **Anchored in 1.3** throughout: the pre-line divider is 33 kOhm in series against 12 kOhm to ground, 11.5 dB, applied explicitly with a matching restoring gain after the line. That sets both the drive the line sees -- the chorus now needs 11.5 dB more before it grits -- and the reason its noise is audible, since the restore lifts the floor with the signal |
