...
struct YouKnow106TestAccess
{
    ...
    {
        constexpr double half = (YouKnow106Engine::halfbandTaps - 1) / 2.0;
        if (factor == 2)
            return half / 2.0;
        if (factor == 4)
...
public:
    ...
    void observe(ChorusMode mode, float noiseScale,
                 const YouKnow106TestAccess::State& state)
    {
...
private:
    ...
    YouKnow106TestAccess::State previous_;
};
...
StructuralAudit auditFullCycle(float sampleRate, ChorusMode mode)
{
    ...
    for (std::size_t frame = 0u; frame < frames; ++frame)
    {
        ...
        checker.observe(mode, 1.0f, YouKnow106TestAccess::state(chorus));
    }
...
struct ProductionRender
{
    ...
    YouKnow106TestAccess::State state;
    YouKnow106TestAccess::ProcessingRate processingRate;
                                  ...
                                  bool alignToReference = true)
{
    const auto processingRate =
        YouKnow106TestAccess::shippingProcessingRate(hostRate, hqEnabled);
    ...
    for (std::size_t frame = 0; frame < frames; ++frame)
    {
        ...
        checker.observe(productionMode(intervalStart),
                        ...
                        YouKnow106TestAccess::state(chorus));
    ...
    if (!alignToReference)
    {
        ...
        result.state = YouKnow106TestAccess::state(chorus);
        result.scheduleLedger = checker.result();
        return result;
    }
    const double delay =
        YouKnow106TestAccess::shippingBoundaryDelayHostFrames(factor);
...
int main(int argc, char** argv)
{
    try
    {
        ...
        std::cerr << "Usage: YouKnow106BbdDynamicQualityAudit "
