...
struct YouKnow106TestAccess
{
    ...
    {
        return YouKnow106Engine::hardwareVoices;
    }
    ...
    static Destination translate(
        YouKnow106Engine::ConverterDestination destination) noexcept
    {
        switch (destination)
        {
    ...
    {
        return YouKnow106Engine::isPassiveHoldWrite(
            YouKnow106Engine::converterWriteOrder()[ordinal]);
    }
    ...
    {
        return YouKnow106Engine::commonVcaHoldTimeConstantSeconds();
    }
    ...
    {
        YouKnow106Engine::PwmHoldState production { state[0], state[1] };
        production = YouKnow106Engine::exactPwmHoldEndpoint(
            production, target, hasEvent, position, eventTarget, seconds,
            YouKnow106Engine::pwmHoldCoefficients(seconds));
    ...
    static YouKnow106Engine fixture(
        ...
        VcaMode vcaMode = VcaMode::Envelope)
    {
        ...
        for (int voiceIndex = 0; voiceIndex < YouKnow106Engine::maxVoices;
             ++voiceIndex)
        {
            auto& voice = engine.voices_[static_cast<std::size_t>(voiceIndex)];
            voice.active = false;
            voice.cardIndex = voiceIndex < YouKnow106Engine::hardwareVoices
                ? voiceIndex : 0;
    ...
    static Coordinates coordinates(const YouKnow106Engine& engine) noexcept
    {
        Coordinates result;
    ...
    static Targets targets(const YouKnow106Engine& engine) noexcept
    {
        Targets result;
        for (std::size_t voice = 0; voice < result.voiceVca.size(); ++voice)
    ...
    static Consumers consumers(const YouKnow106Engine& engine) noexcept
    {
        Consumers result;
    ...
    static Consumers expectedConsumers(const Coordinates& coordinates) noexcept
    {
        ...
        for (std::size_t voice = 0; voice < result.voiceVcaGain.size(); ++voice)
        {
            result.voiceVcaGain[voice] =
                YouKnow106Engine::VoiceVcaControlLaw::gain(
                    static_cast<float>(coordinates.voiceVca[voice]));
            result.pulseDuty[voice] = YouKnow106Engine::pwmDutyCycle(
                static_cast<float>(coordinates.pwmSecond), 1.0f);
        }
        return result;
    }

    static float eventTarget(const YouKnow106Engine& engine,
                             std::size_t ordinal,
    ...
    static float heldTarget(const YouKnow106Engine& engine,
                            std::size_t ordinal) noexcept
    {
        const auto& write = YouKnow106Engine::converterWriteOrder()[ordinal];
    ...
    static double geometricEventPosition(const YouKnow106Engine& engine,
                                         std::size_t ordinal,
    ...
    static void processOne(YouKnow106Engine& engine)
    {
        float left = 0.0f;
        float right = 0.0f;
    ...
    static Latch latch(const YouKnow106Engine& engine) noexcept
    {
    ...
    static bool peek(YouKnow106Engine& engine, double phase, double delta,
                     const EngineParameters& parameters) noexcept
    {
    ...
    static void setSchedulerCursor(YouKnow106Engine& engine,
                                   std::size_t ordinal) noexcept
    {
        engine.nextConverterWrite_ = ordinal;
        engine.passiveHoldEventLatch_ = {};
    ...
    static std::size_t schedulerCursor(const YouKnow106Engine& engine) noexcept
    {
        return engine.nextConverterWrite_;
    }

    static void setAutomationAfterPeek(YouKnow106Engine& engine,
                                       EngineParameters& parameters,
                                       float& lfoGated) noexcept
    {
        ...
        lfoGated = 0.79f;
        for (int voice = 0; voice < YouKnow106Engine::hardwareVoices; ++voice)
        {
    ...
    static void commitOverride(YouKnow106Engine& engine,
                               std::size_t ordinal,
                               const EngineParameters& parameters,
    ...
    static constexpr bool scalarLatchIsAllocationFree() noexcept
    {
        return std::is_trivially_copyable_v<
                   YouKnow106Engine::PassiveHoldEventLatch>
            && std::is_trivially_destructible_v<
                   YouKnow106Engine::PassiveHoldEventLatch>
            && sizeof(YouKnow106Engine::PassiveHoldEventLatch) <= 64u;
    }

    static CommonConsumerProbe commonConsumerProbe()
    {
        constexpr int frames = 512;
        YouKnow106Engine base;
        ...
        YouKnow106Engine low = base;
        YouKnow106Engine high = base;
        ...
        const double ratio = static_cast<double>(
                                 YouKnow106Engine::patchLevelGain(highControl))
                           / static_cast<double>(
                                 YouKnow106Engine::patchLevelGain(lowControl));
...
using PwmMode = youknow106::PassiveHoldAuditPwmMode;
        ...
        for (const std::size_t ordinal : newPassiveOrdinals)
        {
            ...
            for (int modeIndex = 0; modeIndex < modeCount; ++modeIndex)
            {
                ...
                const auto vcaMode = vcaModeIndex == 0
                    ? youknow106::VcaMode::Envelope
                    : youknow106::VcaMode::Gate;
                for (const double requestedPosition : processPositions)
                {
                    ...
                    const std::string label = std::string(cell.name) + "/"
                        ...
                        + (vcaMode == youknow106::VcaMode::Gate
                               ? "/gate" : "/envelope")
...
int main(int argc, char** argv)
{
    ...
    {
        std::cout << "usage: YouKnow106PassiveHoldTimingAudit [--self-test]\n";
        return 0;
    }
    else if (argc != 1)
    {
        std::cerr << "usage: YouKnow106PassiveHoldTimingAudit [--self-test]\n";
        return 2;
    }
