...
public:
    ...
    {
        // Factory product controls currently request HQ. Treat a future change
        // as a contract error rather than silently auditing another quality.
        hqRequested = preset.controls.hq;
        engine.prepare (sampleRate, renderBlockSize, hqRequested);
        engine.setParameters (parametersFor (preset, muteChorusNoise));
    }

    bool isHq() const noexcept
    {
        return hqRequested && engine.getOversamplingFactor() > 1;
    }
                     ...
                     bool includeCorpusLevel)
{
    ...
    csv << "slot,name,tone_bytes_hex,volume,bender_dco,bender_vcf,bender_lfo,"
           ...
           "product_chorus_noise,polyphony,hq,frames,sample_peak,sample_peak_dbfs,"
    ...
    for (const auto& row : rows)
    {
        ...
        csv << preset.number << ',' << csvField (preset.name) << ','
            ...
            << (controls.hq ? 1 : 0) << ',' << metrics.frames << ','
                        ...
                        double commonPreviewGain, double corpusMedianDb)
{
    ...
    report << ownershipMarker << "\n\n# YouKnow106 factory-preset gain audit\n\n"
           << "Generated by `YouKnow106AuditFactoryPresets` from the immutable "
              "factory tone-memory states and their product controls. This is a "
              "measurement report, not an equal-loudness target: overloads and "
              "level differences are warnings only, and no preset bytes or "
              "per-preset gains are changed.\n\n"
...
int run (bool smoke, const std::filesystem::path& directory)
{
    ...
    for (const auto& preset : bank)
        if (! preset.controls.hq)
        {
            std::fprintf (stderr, "%s no longer requests HQ; audit aborted\n",
                          preset.number);
            return 1;
        }
    ...
    return numericalFailure.load (std::memory_order_relaxed) ? 1 : 0;
}
} // namespace
