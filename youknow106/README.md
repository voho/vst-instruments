...
snapshots put 4× / 1× at **0.520 / 0.147×
realtime idle**, **0.493 / 0.154×** for six plain voices, **0.636 / 0.183×**
for the high-resonance fixture and **0.753 / 0.284×** for the full-mixer
Chorus-II fixture; every median absolute deviation is below 1.2%. These are
...
VCF and deterministic BBD still reject every factor; at 4× their respective
full-waveform errors are −24.35/−25.81 and −30.39/−31.85 dB RMS for
44.1/48 kHz. Step 8's causal four-point Lagrange reconstruction of the BBD
input at each clock edge moves the 4× unmasked SGA maxima to
−72.04/−65.60 dBc, passing that submetric at both hosts, but support-filter
warping and full-waveform NRMS still reject both cells; the 44.1 kHz cell also
misses the wanted-image BGA gate. The independent-reference method and complete
before→current matrix are in the comparative assessment. The production DCO
reconstruction and its reported latency changed in Step 7; Step 8 changes only
the numerical BBD edge-input interpolation. The global rate selector and
41-sample latency remain unchanged, and no domain split is admitted.
...
that earlier protocol-v1 comparison; Step 8's controlling current evidence is
the common-host numerical audit above. Neither set of deterministic numerical
...
0.542974 (−5.30 dB) common gain so their relative levels survive 16-bit
...
- **`YouKnow106.VcfBbdQualityContract`** compares the shipping VCF and one
  ...
  four-point edge-input history and exact clock/bucket/transfer state, and locks
  the current metrics and classifications; a rejected cell is evidence, not a
