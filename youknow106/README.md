...
- 2026-08-17: Added regression coverage for `Chorus::correlatedRandomStep`'s own rho guard -- `std::isfinite(correlation) ? clamp(correlation, -1, 1) : 0.0f` -- whose non-finite and out-of-range branches had never fired outside a test, since the function's only production call site always hands it `OptionalNoiseComponents::commonRandomCorrelation`, a "voiced placeholder for deterministic tests only" field with no host/panel control. The new test drives NaN and +/-infinity through it and asserts each matches an explicit rho=0.0 call bit-for-bit, including the noise-state advance, then drives 4.0 and -4.0 and asserts each matches the literal +/-1.0 endpoint it clamps to, rather than merely checking the result stays finite. Test-only change, so `Chorus` behaviour is unaffected by construction; verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render (`git status youknow106/Docs/audio` stays clean).
