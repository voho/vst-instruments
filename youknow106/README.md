...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −16.8 dBFS | +13.8 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −14.7 dBFS | +11.7 dB |
...
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −12.6 dBFS | +9.6 dB |
...
| `10-unit-character.wav` | A six-voice chord at nominal zero Unit Character, then at full amount | 12.9 s | −23.1 dBFS | +20.1 dB |
<!-- peaks-table-end -->
...
| OQ-08 | Exact intra-pass timing and DCO pitch-write staging. The 23-write ordinal order is settled; the normalised `ordinal/23` offsets are compatibility policy, not timestamps. Roland names IC33/IC34 as M82C53 and ties their GATE inputs high. The relevant ranges of the pinned partial IC29 listing match the recovered B-2 image: changed pitch on a non-running voice writes a control word plus count, while a running voice writes only the count. Matching OKI Mode-3 documentation says a running count enters CE after the next OUT transition, gives an odd count's extra clock to OUT high, and forces OUT high on a mode write. Roland's p. 9 description maps only a positive-going OUT edge to C54 discharge and the sub clock. The engine now keeps explicit OUT polarity, half-cycle countdown and one replaceable pending count, including conditional control-word edges and next-clock initial load. Exact CPU-instruction/PIT-clock phase, divided-clock phase during a range switch, and C54's off-phase transistor waveform remain open; the renderer retains its documented finite-linear discharge as analogue policy rather than a measurement claim. Delayed hybrid cycles use the MC5534A's ideal +15 V supply as a scale-aware compatibility ceiling; its installed output swing below that supply is unmeasured | P1 |
