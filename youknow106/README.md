...
- 2026-08-19: Re-voiced the sub oscillator's mixer coordinate, `subMixVolts`, from 5.0 to 2.0 -- the first change in this pass that alters what the instrument sounds like. Measured on the shipping engine with both legs at full slider, no saw or pulse, a flat envelope and the filter wide open, the noise leg rendered **13.3 dB below** the sub leg, a figure that moves less than 0.1 dB across MIDI 36/48/60 and under 1 dB from cutoff 1.00 to 0.85, so it is the mix constants rather than any patch or filter artefact. Against the hardware recording of A64 Snare Drum -- saw and pulse off, key follow 0, so its snare character can only come from the noise leg, and the reference is noise-dominated -- the model rendered noise 10.10 dB *under* sub at the preset's own slider values. The correction goes on the sub leg because it is the one coordinate with no end-to-end anchor, while the noise leg is tied to the 4 Vpp TP8 adjustment within about 4 dB and is derived rather than chosen; closing the gap from the noise side would break an anchor to fix an unanchored number. 2.0 takes 7.96 dB of the 10.10 and leaves A64 at -2.26 dB, deliberately short of the full inversion the recording implies because the magnitude inherits an undocumented MP3 chain's uncertainty even though its direction does not. Bank cost was measured rather than assumed: A83 Drum Booms, a sub-feature patch, loses 7.80 dB, Brass Swell 1.40 dB and Organ I 0.16 dB, while every program with `sub` at 0 or with its sub masked by saw and pulse is untouched. A65 Tom Toms and A66 Timpani stay sub-dominant afterwards and that residual is explicitly not this constant's -- no single scalar reconciles the three, and both run far more closed filters than A64 -- so it remains with OQ-15, which also still owns the measured source-to-VCF budget that would replace this voiced value. The coordinate stays labelled **voiced**; nothing here is promoted to anchored or measured. Verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing). Two suites had to move with it and both are recorded rather than relaxed: `DcoScanQualityContract` rejected at exactly 7.96 dB on `strict_waveform=sub` because its analytic Fourier reference states the source amplitudes independently of the engine -- that reference moved to the same coordinate; and the OQ06 output corpus was re-pinned, with its self-oscillation row unchanged because that fixture runs `subLevel = 0`, which is the control confirming only sub-carrying fixtures moved. The committed demo audio is re-rendered by the nightly job rather than by this change.
