...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −15.7 dBFS | +12.7 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −15.3 dBFS | +12.3 dB |
...
| `06-chorus-modes.wav` | The same pad with the effect off, in mode I, mode II, then I+II | 20.5 s | −17.0 dBFS | +14.0 dB |
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −11.4 dBFS | +8.4 dB |
| `08-delayed-vibrato.wav` | The modulator's two-stage delay fading vibrato onto a held chord | 9.6 s | −22.8 dBFS | +19.8 dB |
| `09-high-pass-ladder.wav` | One bright chord through all four high-pass switch positions | 10.6 s | −15.9 dBFS | +12.9 dB |
...
- One 12-bit converter scans 23 sample-and-hold destinations — 18 per-card,
  ...
  within their slot; the intra-pass offsets are compatibility policy, with a
...
| OQ-09 | Resonance byte-to-loop-gain law. Topology and mechanism are settled, including the Roland-printed input-side compensation from the p. 9 module drawing. The 106's own drawing prints no component values, but two sibling readings of the same network now do, and they bracket the compensation coefficient rather than fixing it: Roland's JUNO-6/JUNO-60 CPU BOARD p. 9 gives (10/68)·(101.5/48.5) = 0.3078, the published Open80017a reconstruction gives (4.7/68)·(101.5/25.5) = 0.2751, and they disagree 2.1× on the stage-1 input resistor. A technician's ohmmeter reading of a **de-potted original 80017A** ([Sound Doctorin](https://sounddoctorin.com/synthtec/roland/juno106.htm)) now supplies a third reading, and it is the first measured-on-an-original datum this project has: the resonance OTA's non-inverting leg reads 5.1 kΩ against 1.5 kΩ, and the stage-1 series input reads 3.9 kΩ, giving c = (3.9/68)·(101.5/6.6) = 0.882. That is 6.9 dB of extra passband compensation at full resonance over the shipped floor, so the bracket has widened rather than closed, and its top is now a measurement rather than a drawing. It is not adopted, because the same reading puts the stage-1 gain at 68/3.9 = 17.4 where the drive coordinate assumes 6.8, so it moves OQ-15 by the same act; the two have to be re-derived together and reconciled against the anchored 6 Vp-p TP8 trim. Caveats that keep it from being decisive on its own: one sample, in-circuit and hand-held, self-described as "roughly", against two independent 900 dpi reads of the sibling factory drawing. The shipped value stays at the floor. The onset coordinate now includes the anchored +0.26 V VR34/TP7 standoff (p. 18 section 3) that the p. 8 IC27b branch carries into the RES CV hold; the 0.6 V junction drop above it is still a nominal prior awaiting the measured response family | P1 |
