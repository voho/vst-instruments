...
| Numerical cost | The VCF's default rung retains fixed solver work per internal sample: two Merson halfsteps, 10 right-hand-side and feedback evaluations, 40 stage and full-Early evaluations when Character is enabled, seven input reconstructions and no normal-path recovery. The persisted `VCF Solver` property selects two cheaper Runge-Kutta tableaux beside it — `RK4 x2` at 8 evaluations and five reconstructions, and `RK4 x1` at 4 evaluations and three — whose abscissae are all drawn from the same seven control-node positions, so no rung moves a control node, changes the converter hold trajectory or needs a second reconstruction grid. Event-containing VCF intervals additionally perform six control mappings; ordinary intervals do not. Settled per-card constants remain outside the sample loops. A compile-time-only work audit counts scan, passive holds, DCO, VCF, BBD/BLEP, exact/legacy support and decimator events on every 4×/2×/1× production branch; a separate executable times the uninstrumented shipping library with a thread-CPU clock. Normal and active-counter renders must remain raw-float identical, and preprocessing plus symbol/string scans keep audit instrumentation out of the shipping library. | None of it is a hardware claim, and no constant, level, corner or law moves. In the 2,048-host-frame 48 kHz fixture, HQ and HQ-off each see 160 passive fractional peeks/commits, while the retained VCF subset stays 70/70 and 120 exact affected card intervals. The fixed VCF and BBD work algebra is unchanged. Counters make no cycle-cost claim; the paired uninstrumented Step 11 → Step 12 audit ranges from −0.126874% to +1.342855%, with a worst Step 12 result of 0.853898× realtime. BBD support is unchanged. Counters prove structural work, not cycle cost or a split-rate saving; completed paired CPU measurements are in the [comparative assessment](Docs/comparative-assessment.md). The cheaper rungs are a cost ceiling rather than a promise about a particular interval: each interval is planned against the largest stage pole and loop gain it presents, a single full-interval step is taken only inside a measured accuracy bound, and both RK4 rungs fall back to Merson where classic RK4's own stability region ends — which is reachable at the product grid's 0.9·π cap. The measured error against an independent 96-substep reference solve, the false-limit-cycle onset that sets the stability bound, and the paired whole-engine CPU results are in [Docs/vcf-solver-optimization.md](Docs/vcf-solver-optimization.md). |
...
The persisted, non-automatable host parameter `VCF Tanh` selects `Exact` or
experimental `Fast`. Exact is the default for new and historical sessions;
Fast currently uses the zoned cubic-Hermite kernel with per-Merson-node
reciprocal normalization documented below. Neither setting belongs to a patch,
Program Change, SysEx dump or randomization. The persisted ordinals remain
Exact=0/Fast=1, so an explicitly saved experimental Fast state selects the
current Fast implementation. The approximation research, measured CPU results
and blind-audition protocol are recorded in
[Docs/vcf-tanh-optimization.md](Docs/vcf-tanh-optimization.md).
...
existing Exact and Fast session; opt-in `Cubic` replaces only the four small
Character/Early transfers while leaving feedback and the four main-stage
transfers on ZonedHermite. It is likewise outside patches, Program Change,
SysEx and randomization. Old states restore Hermite rather than inheriting a
live Cubic selection.
...
- 2026-08-23: Examined the *number* of VCF right-hand-side evaluations rather than their cost for the first time. The four-pole cascade spent ten per card per internal sample on a fixed two-half-step Merson pair, unconditionally, whatever step size the interval presented; Merson's fifth stage buys an error estimate a fixed step never uses. A new persisted, non-automatable `VCF Solver` property adds two classic-RK4 rungs beside it — `RK4 x2` at 8 evaluations and 5 input reconstructions, `RK4 x1` at 4 and 3 — whose abscissae are all drawn from the existing seven control-node positions, so no rung moves a control node, changes the converter hold trajectory or needs a second reconstruction grid. `Merson x2` stays ordinal zero and reproduces every frozen raw-float fingerprint at 1×, 2× and 4×. Each interval is planned against the largest stage pole and loop gain it presents: a full-interval step is taken only below a normalized step of 1.25, measured against an independent 96-substep reference solve that holds `−87 dB` out to 2.35 and departs near 2.97, and *both* RK4 rungs fall back to Merson above 2.0 per half step. That last tier is not conservatism — the suite's zero-input cap fixture found classic RK4 sustaining a false limit cycle of 0.538 V at `k = 2.0` and 1.181 V at `k = 3.8` at the product grid's `0.9·π` cap, with the onset exactly at RK4's own stability radius where Merson's five-stage region still reaches. Live solve accuracy is `−162.6/−147.1/−123.1 dB` at a 192 kHz internal grid and `−141.6/−125.7/−101.6 dB` at 48 kHz. The service ADJUSTMENT's self-oscillation anchor is unmoved: identical amplitude to four decimal places and 0.14 cents of pitch at every rung and every quality factor, against a procedure that itself accepts ±10 cents. Paired whole-engine x86-64 runs at 48 kHz/4× measure 52.7–54.4% less CPU for `RK4 x1` alone with the exact libm `tanh` untouched, 17.3–20.2% for `RK4 x2`, and 67.8–76.5% composed with the existing `VCF Tanh`/`VCF Fast Early` selectors; the wide-open full-mixer fixture returns 17.2% for the solver column because it escalates, which is the mechanism working. A three-stage Kutta rung was implemented, measured at `−64.7 dB` and losing the limit cycle outright, and removed. Blind A/B sets null 138–147 dBc for `RK4 x2` — below what a 16-bit file carries — and 88–110 dBc for the cheapest rung. The player then reported no audible difference between any rung, which is what promoted the cheapest one to the shipped default; `EngineParameters` still defaults to the Merson kernel so every frozen fingerprint and work-counter contract keeps testing the reference. Separately, this pass found `main` already red on macOS: `VST3BundleSmokeTests` locates the standard Bypass and Program parameters immediately after the public ones, and `VCF Tanh` and `VCF Fast Early` had been added to the layout without being added to that test's list, so the two were pushed out of position. Because the test is macOS-only nothing on Linux or Windows could see it. The list now lives in `Tests/PublicParameterOrder.h`, shared with a new `PluginProcessorTests` check that compares it against the live processor's order and ids on every platform. See the [research and evidence note](Docs/vcf-solver-optimization.md).
- 2026-08-23: Re-profiled the optimized sound generator on an Apple M1 Max with native arm64 `-O3`/ThinLTO at 48 kHz, 4× and block 256, then admitted five byte-identical specializations: one Fast state validation replaces its repeated nonlinear-helper guards; the private production table path trusts that invariant for subnormals while the checked public helper retains its exact behavior; the four-double Merson state is passed as an arm64 `d0`–`d3` HFA; 99.944885% of ordinary VCA/sub/voice-VCA holds take their unchanged no-event recurrence directly; and chorus exact-support no longer repeats sanitation already enforced at both ownership boundaries. A strict six-scenario cohort measures **9.188726%** combined median thread-CPU reduction (`0.422565` percentage-point MAD, 90/90 wins, both order splits agreeing, no fingerprint mismatch). Final matched Fast ratios are `0.363275×`/`0.356117×` realtime for one/six plain notes and `0.361359×`/`0.355058×` for one/six resonant notes. A fresh seven-repetition selector audit measures Exact at `0.602×`/`0.609×` plain and `0.753×`/`0.768×` resonant versus Fast at `0.361×`/`0.363×` and `0.360×`/`0.358×`. Single/chord work stays nearly equal because all six physical cards remain powered. Four current six-second stack captures plus arm64 return-PC mapping prove a conservative VCF numerical-core floor of `68.433–70.861%`; no exact total is inferred from the mixed caller tail. The rebuilt 15-contract DSP suite passes in full. See the [measured profile](Docs/comparative-assessment.md#current-single-note-versus-chord-profile-2026-08-23).
- 2026-08-23: Replaced the BBD write transfer's two ordinary-path fractional powers with an 8,208-byte, 512-interval cubic-Hermite table sampled from the unchanged fitted law; inputs at or beyond four rails retain the original defensive double-power expression. A 34,000,014-input scan found at most one float ULP of error and zero monotonicity failures. The strict one-note/chord cohort saves `0.696225%` median whole-engine thread CPU (`0.267472` percentage-point MAD, 53/60 wins); every scenario is positive at `0.604493–0.882118%`, both executable-order splits agree, and dry-path fingerprints remain identical. Separate full-Chorus and hot wet-Chorus renders null at `−162.34` and `−173.85 dBc`, with maxima `2.98023e-8` and `1.19209e-7`; a 45-pair chorus/bypass cohort saves `0.792028%` median with 44/45 wins. Two plausible call-count optimizations were rejected on whole-engine evidence: caching fixed VCF node decays removed 36,667 double `exp` calls/s but slowed every scenario (`−0.198951%` aggregate), while a thermal exponential recurrence measured only `+0.087097%` against `0.271421` pp MAD.
- 2026-08-23: Admitted four independently profiled hot-path reductions after native Apple M1 Max whole-engine runs at 48 kHz/4×. The fixed exponent-eight output clip now uses multiplies and three square roots instead of two general powers (`1.872%` isolated aggregate median saving, `0.287` percentage-point MAD, 81/90 wins); the chorus exact-support transition is stored in its consumption order and uses the equivalent single-comparison state guard (`0.472%`, `0.214` pp, 47/60); the half-band decimator pairs bit-equal mirrored coefficients (`0.607%`, `0.230` pp, 52/60); and each card's spatial-thermal cutoff scale is cached until calibration or the gradient switch changes (`1.010%`, `0.224` pp, 51/60). A corrected combined cohort alternated executable order independently for every scenario over 15 repetitions: the six-fixture aggregate saves `4.010%` median thread CPU (`0.319` pp MAD, 89/90 wins), with baseline-first and candidate-first medians agreeing at `3.994%` and `4.021%`; scenario medians span `3.666%` for a single plain note to `4.517%` for Chorus II. Clip, chorus and thermal were bit-identical in their isolated full-engine renders; the clip additionally matched all `92,699,209` numerical checks, and the cached thermal law now has a live-edit/reset/rate-change regression against the retained direct calculation. Half-band float reassociation is the combined build's only numerical difference: response and group delay are unchanged, 1× remains byte-identical, and the six 2×/4× full renders null from `−135.67` to `−135.09 dBc`, with maximum absolute error `7.15256e-7` engine units in the deliberately hot unison fixture. The four isolated medians describe attribution cohorts and are not additive; the combined result is the measured shipping effect.
- 2026-08-23: Measured two C1 inner-zone polynomial endpoints against the current reciprocal-normalized Fast VCF in complete one-note and six-note plain/resonant renders. Final Poly9 has `0.000725672%` maximum full-scale transfer error and saves `13.465%` aggregate median CPU; Poly5 deliberately approaches the requested ceiling at `0.214236%` maximum error and saves `20.893%`. Both won all 60 paired runs. Deterministic Exact/Poly9 and Exact/Poly5 blind sets now cover 1×/2×/4×; the user's hearing verdict remains the promotion gate. A separate sound-identical cleanup removed a redundant `isfinite` state predicate after two independent 60-pair rotations measured `0.513%` combined median gain, `0.289%` MAD, 100/120 wins, and bit-identical fingerprints. Exact-output history restructuring was rejected at `0.029%` versus `0.287%` MAD. Native vector `tanh` was 67–76% slower and not strictly monotone; explicit two-lane Poly9 was bit-identical but averaged 8.20% slower than scalar Poly9. Two other exact-output micro-optimizations were also measured and rejected. See the [research and evidence note](Docs/vcf-tanh-optimization.md).
- 2026-08-23: Profiled the selectable Fast implementation in one-note and six-note plain/resonant sound generation. Its VCF derivative remains 63.7–66.1% of active samples; the four main-stage nonlinear call sites account for 27.6–29.5% of the complete engine, the four Character/Early sites for 14.9–16.1%, feedback for 3.3–3.6%, and Merson/setup for another 8.3–8.7%. Single/chord work stays nearly equal because all six cards run continuously. A bit-identical equal-headroom reciprocal shortcut was removed after 60 paired runs found only 0.104% median gain against 0.310% MAD. The new opt-in `VCF Fast Early=Cubic` setting instead targets the measured Early sites: its odd monotone C1 transfer bounds the resulting Character multiplier error to 0.0564% at Character 1 and 0.1128% at Character 2, and the final integrated build measures 5.3–7.2% lower whole-engine CPU than Fast-Hermite across idle, one/six-note plain/resonant and full-chorus cases, with all 42 official pairs faster. A stricter four-scenario run measured 6.113% aggregate reduction and 59/60 wins; separate 60-pair checks kept Exact and Fast-Hermite bit-identical to the frozen pre-selector engine with no regression signal. Exact and existing Fast remain the defaults of their respective selectors; Cubic remains hearing-gated. See the [research and evidence note](Docs/vcf-tanh-optimization.md).
- 2026-08-23: Profiled the complete sound generator again after the zoned-table change and removed repeated normalization work from experimental Fast. One reciprocal at each of the seven sanitized Merson nodes now replaces 90 divisions per Character-on VCF card interval; Exact retains its original division expressions, and common invariant hoists preserve its frozen fingerprints. The final native Apple M1 Max paired whole-engine run at 48 kHz/4× measures 35.7–48.3% less thread CPU than Exact across idle, single-note, six-note, resonant and full-chorus cases, with all 42 pairs faster; an immediately prior repetition measured 35.0–47.9%. Single and six-note costs remain close because the six physical cards run continuously. A two-million-value normalization probe changes the ZonedHermite result by at most `1.11e-16`, while its established maximum transfer error remains `1.15546e-8`. Fresh 1×/2×/4× blind bundles render deterministically. Exploratory constrained degree families were scanned here, but their historical timing used different least-squares coefficients; the final C1 Poly9 and Poly5 timings are recorded separately above rather than conflated with those runs. See the [research and evidence note](Docs/vcf-tanh-optimization.md).
- 2026-08-22: Profiled the VCF nonlinearity in actual single-note and six-note sound generation, then replaced Fast's uniform Hermite512 internals with a smaller zoned cubic table while preserving the public `Exact=0` / `Fast=1` state contract and Exact default. At 48 kHz/4× with Character enabled the six powered cards make 103.68 million `tanh` calls/s; native sampling puts exact libm calls at 62–65% of top-of-stack samples in resonant cases. Observed arguments stayed below 2.503, with over 99.5% below 1, so Fast now uses 160 intervals over 0–5 at 1/32 plus a safe coarse tail to 19: 6.75 KiB total and 5 KiB hot. Its million-point maximum error is `1.15546e-8` (`1.15546e-6%` of unit full scale), maximum pointwise relative error is `4.42221e-8`, and independent coefficient extrema keep every derivative in `[0,1]`. Paired whole-engine Apple M1 Max measurements reduce CPU by 24.4–38.8% over Exact across idle, single/six plain, single/six resonant and full-chorus cases; all 42 paired runs win. Fresh deterministic 1×/2×/4× blind bundles use unrevealed balanced mappings. Most fixtures null below −106 dBc, while the intentionally sensitive B15 feedback trajectory diverges strongly and remains the player's hearing gate. Exact fingerprints, state/schema rules, patches, SysEx and randomization remain unchanged. See the [research and evidence note](Docs/vcf-tanh-optimization.md).
- 2026-08-22: Made the VCF's hot nonlinear transfer selectable without changing the established sound by default. `Exact` retains the existing double `std::tanh` path and reproduces every frozen raw-float fingerprint; `Fast (Hermite 512)` uses a bounded, odd 512-interval cubic-Hermite table only inside the Merson derivative, while the cold describing-function reference remains exact. A million-point scan measures `2.017e-8` maximum absolute transfer error and checks monotonicity, every table boundary, bounds, signed zero, near-zero slope, NaN/infinity handling and both-mode hostile-filter recovery. On this Apple M1 Max at 48 kHz / 4x, paired whole-engine thread-CPU runs improve by 1.254x/1.228x/1.533x/1.293x over idle/plain/resonant/full-chorus fixtures (19.4-34.6% less CPU). The new persisted non-automatable `VCF Tanh` host property defaults to Exact, stays outside patches/SysEx/randomization, and restores old sessions to Exact. `YouKnow106RenderTanhAuditions` emits six quality-specific counterbalanced float32 A/B pairs with one shared gain and a separate reveal key; the user's blind preference remains the promotion gate. See the [research and evidence note](Docs/vcf-tanh-optimization.md).
- 2026-08-20: Measured the instrument's tail, and stopped the envelope's own release time reading as it. `getTailLengthSeconds` reports infinity, and the comment above it offered "about 25.55 s" as the finite figure behind that decision. 25.5486 s is real, but it is `envelopeReleaseSeconds(1.0f)` -- the modelled envelope's longest release -- not the instrument's tail: the output coupling takes roughly as long again to settle after it, so a tail reported from the envelope alone would truncate the instrument by half. Measured on the shipping engine with six voices at the longest release, from the moment the last key lifts: with the chorus off, or engaged with Chorus Noise at zero, the output falls below -140 dBFS after 25.41 s and reaches bit-exact zero after 53.3-53.9 s across 44.1 and 48 kHz at 1x and 4x; with the chorus engaged and its noise present it is still not silent after two minutes, which is the modelled analogue floor doing what it is meant to. Infinity therefore stays, unconditionally: it is right whenever the chorus is running, and a value that changed with a patch would have to be re-announced to hosts that cache it. What that costs -- a host cannot idle-suspend an instance whose patch has the chorus off, and 65 of the 128 factory programs store it off -- is now written into the release checklist beside the measurement, so the accept-or-qualify item can be closed on evidence rather than on an estimate.
- 2026-08-20: Named every readout the monitor draws in its own help. The display's tooltip described the voice cards, the voice limit and the LFO/envelope meters, and stopped there -- while the same panel also prints the modelled chassis temperature, the supply rail, the output waveform, the scope's vertical gain and the oversampling the engine actually applied. Two of those print an "Nx" figure and they mean different things: the scope gain, and the applied factor, which can be lower than QUALITY asked for. The tooltip now names all six. This is a content change with room behind it -- the strip's three-line body prints it in full, and the fit sweep covers it.
...
- 2026-08-20: Left no history in the output path after a reset. IC6's output slew integrator was the one mutable node in the output path that `clearOutputPath` did not clear, so `reset()` did not put the engine in one state: what it rendered next depended in part on what it had been rendering before. The leak is small -- the modelled 1.7 V/us slew limit is 3.4 engine units per internal sample at a 192 kHz internal rate, so the integrator collapses within a sample or two of any stop, and the post-stop residual measures -76 dBFS with or without the fix, which is the model's own idle floor rather than the retained node. What it did break is determinism: rendering the same note after `reset()` differed by up to 1.3e-9 at output sample 17 depending on whether a loud six-voice chord or silence preceded the reset, at 44.1 and 48 kHz and 1x/2x. `testResetLeavesNoHistoryInTheOutputPath` renders the same note after a reset from both histories at 44.1/48/96 kHz across 1x/2x/4x and requires the two to be bit-identical. `allNotesOff` is deliberately exempt and says so: it is a panic, not a power cycle, and the six cards stay powered behind their closed VCAs with their oscillator phase and filter charge intact. All ten committed demonstration WAVs re-render byte-identically.
- 2026-08-20: Separated a host transport stop from a power cycle. `AudioProcessor::reset` is what a host issues on every stop and locate -- through `IAudioProcessor::setProcessing(false)` in VST3 and `AudioUnitReset` in Audio Units -- and it forwarded straight to `YouKnow106Engine::reset`, which zeroes the modelled chassis warm-up. The 900 s law that plan step 5 was spent making run to completion at every rate therefore restarted from ambient at every stop, so it never ran in a normal editing session, the panel thermometer snapped back to 25.00 C on each stop, and an offline bounce and a long continuous real-time pass rendered different OTA headroom from the same patch. The engine now has `resetForHostStop`, which clears the same voices, tails and transient controllers but leaves the warm-up timer and the fraction derived from it where they were -- the class of free-running physical state the voice cards' own component trims already outlived a reset. `prepareToPlay` keeps the cold path. `testHostResetKeepsTheModelledChassisWarm` proves the chassis survives a stop, that the stop is still a reset, that the warm-up resumes from where it was, and that preparing the processor still starts it from ambient. This is a decision about what a host reset means rather than a measurement, and it moves a boundary the engine draws elsewhere: `reset()` is written as a power cycle -- the quality-switch comment says a live rate change is not one "because a prepare/reset is" -- so on that reading a host resetting at every transport stop is asking for a power cycle each time. The reading taken here is that a transport stop is not one, because the modelled instrument is not switched off when the player stops the song, and a 900 s warm-up that restarts at every stop never runs at all. `prepare()` remains the cold path, and a rate change, a device change and a fresh instance all go through it.
- 2026-08-20: Retired the panel readouts whenever the instrument stops. The oscilloscope draws the instrument's output, but `processBlockBypassed` cleared the buffer only after `processBlock` had already published the telemetry taken from it, so a bypassed instrument -- silent to the host, and deliberately still rendering behind the mute to keep MIDI state moving -- drew a full-amplitude trace of audio the bypass had thrown away. `releaseResources` cleared none of the readouts at all, so a torn-down instrument kept the last block's voice lamps, envelope meter and frozen trace beside its own STANDBY legend for as long as the window stayed open, and `prepareToPlay` left the previous configuration's readings up until the first new block arrived. The bypass render now silences the buffer before the telemetry is taken from it, and one `clearDisplayTelemetry` retires every published readout from the host reset, the teardown and the prepare alike. `testBypassedTelemetryShowsTheSilenceTheHostReceives` and `testReleaseResourcesRetiresThePanelReadouts` cover both paths.
- 2026-08-20: Fitted the add-on key legends at every supported editor size. The compact keys below the keybed -- PANIC, INIT, the three variation strengths and the patch-file rail -- sized their legend from the key height alone and clamped it to a 13 pt ceiling, so a key that the minimum editor width had already made 11% narrower still held full-size type. "PANIC" then needed 35.34 px of a 35 px cell in the Linux system font and was drawn horizontally condensed; the drawing code's own comment recorded the same overrun for the wider Windows font. The size and the width budget now come from `panel::compactLegendPointSize`/`panel::compactLegendWidth`, which cap the legend by the editor's own scale as well as the key's height, so the legend shrinks with the panel instead of staying behind on a key that no longer fits it. `testCompactKeyLegendsFitAtEverySupportedSize` sweeps all eight compact keys over 2,000 scales spanning the whole supported resize range and requires each legend to fit on the JUCE-free width model, with a fourteen-character legend as the guard that the budget can still say no; the macOS/Linux plug-in suite repeats the check against the real font at the minimum and default sizes through the same two functions the look and feel draws with. Nothing changes at the default size: the rendered editor snapshot is byte-identical before and after.
- 2026-08-20: Put the whole programmer row on one key-face line. A hardware key draws its face at a fraction of its cell height, and that fraction was picked from the cell height with a `> 65` unit threshold meant to lift the sound row's stacked pairs. The VOICE MODE cells are 68 units tall and are not a stacked pair -- they are the left end of the programmer row -- so POLY 1, POLY 2 and UNISON drew their faces and lamps six pixels above the GROUP, BANK, PATCH and DATA keys they share a cell top and cell height with. Measured on the rendered editor: POLY 1's key face occupied rows 294-311 against GROUP A's 300-317, and afterwards both occupy 300-317. The threshold now splits at 80 units, where the shortest stacked cell actually is, and the four literals became named constants. `testProgrammerRowKeysShareOneKeyFace` requires every key in the row to report the same cell and the same key-face fraction, with a stacked sound-row key as the guard that the check is not simply making every key alike.
- 2026-08-20: Ran the chorus interlock only for a real press. CHORUS I and II are mutually exclusive, and the editor implemented that by clearing the sibling parameter from a `ButtonAttachment`'s `onClick`. JUCE calls `onClick` when the attachment moves the lamp too, not only when a player presses the key, so any parameter-driven change -- host automation, a session restore, a program change, a patch dump -- also wrote the other parameter, with a full begin/value/end gesture a latching automation lane would record. Because that only happened while the window was open, the same automation rendered mode I with the editor on screen and mode II with it shut. The two keys now use the `ParameterAttachment` pattern the POLY contacts and the CHORUS OFF key already use -- whose comment names this exact hazard -- so the lamp follows its parameter and only a press moves anything. `testChorusInterlockOnlyRunsForARealPress` covers the host write, the press, the press on the lit key, and the lamps a reopened window shows.
- 2026-08-20: Kept the performance lever's held axis when the other one is released. `keyStateChanged` sprang both axes to zero on any key release while a keyboard gesture was live, but JUCE delivers that callback for every key-up reaching the focused component and does not say which key moved. A player holding Right for pitch bend who pressed and released Up to add vibrato therefore lost the bend with Right still physically down, and with auto-repeat finished nothing brought it back. Both axes are now re-derived from the arrow keys still held, and the gesture ends only when none of them is. The decision is the free-standing `YouKnow106PerformanceLever::axesForHeldKeys`, because a headless suite cannot press real keys; `testPerformanceLeverKeepsTheAxisStillHeld` covers all eight combinations, including both horizontal keys held resting the lever rather than fighting over the sign.
- 2026-08-19: Re-voiced the sub oscillator's mixer coordinate `subMixVolts` 5.0 -> 2.0 against the factory-bank comparison, then **reverted it the same day**; the shipped value is 5.0 and this release changes no sound. Recording the round trip because the measurements it produced are worth keeping. Measured on the shipping engine with both legs at full slider, no saw or pulse, a flat envelope and the filter wide open, the noise leg renders **13.3 dB below** the sub leg, a figure that moves less than 0.1 dB across MIDI 36/48/60 and under 1 dB from cutoff 1.00 to 0.85, so it is the mix constants and not a patch or filter artefact; at A64's own slider values the model renders noise 10.10 dB under sub. That contradiction stands and is recorded under OQ-15. What did not stand was the correction: no circuit mechanism examined accounts for more than about 3 dB of it, the per-leg source resistances (sub 27 kOhm against noise 39 kOhm) push the balance the wrong way by 1.03 dB, a Gaussian noise distribution would push it the wrong way by a further 1.25-7.27 dB, and the reference archive turned out to carry added reverb from an outboard multi-effect. A coordinate moved 7.96 dB on that basis would be a fit to a processed recording wearing a voiced label, which is the failure this project's taxonomy exists to prevent. The 13.3 dB measurement, the measured bank-wide cost of each candidate and the adjudication of both research submissions are in [Docs/open-questions.md](Docs/open-questions.md) under OQ-15, so the next attempt starts from evidence rather than from scratch.
- 2026-08-19: Compacted the half-band decimator's tap table. `downsamplePair` walked all 95 kernel taps and skipped the 46 analytically-zero ones with a per-tap branch, paying 95 coefficient loads and 95 compares to perform 49 stereo multiply-accumulates; the non-zero taps are now gathered once, where the kernel is built, and the hot loop visits only those 49. The retained taps keep their original ascending order, so the accumulation sequence is unchanged and the float result is bit-identical by construction -- the entries dropped are exactly the ones the old branch already declined to accumulate. The isolated loop measures 10.4% faster, while whole-engine render time moves within run-to-run noise, so this is a smaller effect than the decimator's share of the engine is sometimes assumed to be; the `YOUKNOW106_WORK_AUDIT` counters report the same 49 tap visits and 98 stereo MACs per call as before. Verified by a bit-identical demo-corpus re-render (`YouKnow106.RenderDemos` passing, with `git status youknow106/Docs/audio` staying clean across all 10 WAVs), by `YouKnow106.OversamplingAuditContract` passing against the separate `YOUKNOW106_WORK_AUDIT` build that counts those tap visits and MACs, and by a six-scenario FNV fingerprint over 1x/2x/4x at 44.1/48/96 kHz with both chorus lines, dry and wet, which matches the pre-change build exactly in every scenario.
- 2026-08-17: Added regression coverage for `OtaCascade::process`'s own non-finite fallback guards on its scalar `calibration` argument and on the per-node `feedback`/`headroom` fields of its explicit VCF hold control trajectory, none of which had ever fired outside a test since the engine's only production call site always supplies already-sanitised, finite values.
- 2026-08-17: Added regression coverage for `Chorus::correlatedRandomStep`'s own non-finite/out-of-range correlation guard (`rho` falls back to 0.0f for a NaN or infinite `correlation`, and otherwise clamps to `[-1, 1]`), which its only production call site never exercises: `optionalNoise_.commonRandomCorrelation` has no setter reachable from panel, preset or SysEx code and stays fixed at its 1.0f default for the lifetime of every `Chorus`, and the existing correlation fixture in `testChorusNoiseComponents` only ever drove finite in-range values (1.0, -1.0, 0.35) through the friend seam directly. The new test drives NaN and +/-infinity through the seam and confirms each replays sample-for-sample against an explicit rho=0 call from identical seeds, then drives 2.0 and -2.0 and confirms each matches the already-covered rho=+1 (duplicated channels) and rho=-1 (inverted channels) behaviour exactly. Test-only change, so `Chorus` behaviour is unaffected by construction; verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render (`git status youknow106/Docs/audio` stays clean across all 10 WAVs).
- 2026-08-17: Deduplicated `YouKnow106Chorus.cpp`'s eight independent inline copies of the `pi` literal (`3.14159265358979323846`) -- four local `constexpr double pi` re-declarations inside `inputSupportMatrix`/`inputSupportDrive`/`outputSupportMatrix`/`outputSupportDrive` and four bare inline occurrences in `Chorus::deterministicToneStep`, `Chorus::wetOutputCouplingCornerHz` and the two heterodyne-bleed oscillators in `Chorus::process` -- into one file-scope `constexpr double pi` in the anonymous namespace, matching `YouKnow106Engine.cpp`'s existing single shared constant; the one float call site now reads `static_cast<float>(pi)` instead of carrying its own separately-typed literal. Pure textual refactor with the same double-precision arithmetic at every site, so it cannot change engine or chorus output; verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render (`git status youknow106/Docs/audio` stays clean across all 10 WAVs).
- 2026-08-17: Added regression coverage for `Chorus::sallenKeyQ`'s own non-positive shunt-capacitance fallback guard (returning a fixed Q of 0.5 instead of dividing by zero or square-rooting a negative ratio), which both production call sites in `supportChainFor` never exercise directly since they always pass one of four compile-time-positive capacitor constants. Test-only change; verified with `ctest --test-dir youknow106/build-dsp` (all 15 suites passing).
- 2026-08-16: Added regression coverage for `YouKnow106Engine::exactPwmHoldEndpoint`'s own non-finite (or, for the interval, non-positive) fallback guards on its `target`, `intervalSeconds`, `eventPosition` and `eventTarget` arguments, none of which had fired outside a test before.
- 2026-08-16: Added regression coverage for `YouKnow106Engine::exactOnePoleHoldEndpoint`'s seven non-finite (or, for the time constant, non-positive) fallback guards -- the shared solve behind the common VCA, SUB and each voice's own VCA passive hold, whose only production call sites always build every argument from already-finite state, sanitised panel targets and compile-time or precomputed `std::exp` constants, so none of the guards had ever fired outside a test. The new test drives one poisoned argument at a time directly through the function via the engine-suite's existing private-access seam and asserts the exact documented fallback for each -- state to 0.0, target/eventTarget to the already-resolved target, the interval to zero elapsed time, the time constant to 1.0 second, the precomputed decay to a freshly solved exponential, and the event position to 1.0 (an end-of-interval event contributing nothing) -- using zero-difference terms to isolate each guard from the others. Test-only change, so engine behaviour is unaffected by construction; verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing).
- 2026-08-16: Added regression coverage for `YouKnow106Engine::sanitise`'s exact per-field contract -- the existing end-to-end test only confirmed that hostile parameters still render finite audio, which cannot distinguish a correct fallback from a wrong one, because both keep the engine finite. Every one of `sanitise`'s 23 `[0, 1]`-range float fields takes one of two distinct branches: a non-finite input (NaN, +/-infinity) substitutes a named fallback constant, while a finite out-of-range input is clamped instead; every one of those fallback constants happens to equal the field's own default member initialiser. The new test drives all three poison values and both out-of-range directions through each field via a pointer-to-member table and asserts the exact returned value against a default-constructed `EngineParameters`, then separately covers `calibration` (fallback 1.0, ceiling 2.0 rather than 1.0), `masterTuneCents` (fallback 0.0, clamped to +/-50 cents rather than [0, 1]) and the two always-clamped integer fields `keyTranspose` (+/-12) and `polyphony` (1..`maxVoices`). This would catch a fallback constant drifting out of step with its declared default, or the non-finite/clamp branches swapping behaviour, for any single field -- neither of which the existing finite-audio check can see. Test-only change reached through a new `sanitise` passthrough on the engine-suite's existing private-access seam, so engine behaviour is unaffected by construction; verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render (`git status youknow106/Docs/audio` stays clean across all 10 WAVs).
- 2026-08-16: Added regression coverage for `YouKnow106Engine::dcoDivider`'s own non-positive/non-finite frequency guard -- its only production call site, `updateVoiceEnvelopeAndPitch`, always feeds it `midiToHz()` of a finite, panel-derived MIDI note, so the guard's saturate-to-the-widest-divider fallback had no direct coverage even though the existing note-timer test only ever drove strictly positive, finite frequencies through it. The new test asserts that a zero, a negative, a NaN and a +/-infinity frequency all resolve to the documented widest 16'-range divider (65535) rather than zero, a negative count, or a NaN propagating into the note timer. Test-only change, so engine behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render (`git status youknow106/Docs/audio` stays clean across all 10 WAVs).
- 2026-08-16: Added regression coverage for `YouKnow106Engine::pwmDutyCycle`'s own non-finite guards on both its arguments -- `controlVolts` falls back to the nominal 6 V ramp threshold, and `rampAmplitudeScale` falls back to unity, and a non-finite `controlVolts` must fall through to that same fallback rather than take the separate `controlVolts < 0.0f` "Pulse Off pins the comparator high" early return (which is itself guarded by `std::isfinite`). The function's only production call site, `updatePulseComparator`, builds both arguments from panel calibration, per-card trims and the DCO's own render scale, none of which can go non-finite, so neither guard had ever fired outside a test. The new test drives NaN, +infinity and -infinity through each argument independently and confirms every case lands on its documented fallback result, including the -infinity/Pulse-Off distinction. Test-only change, so engine behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing, including a clean `YouKnow106.RenderDemos`).
- 2026-08-16: Added regression coverage for `Chorus::bbdPolyBlepResidual`'s own negative/NaN-distance fallback guard -- the helper's own comment documents that distances are always non-negative in this model, and both production call sites (the past-edge correction, fed a tracked non-negative event age, and the future-edge prediction, fed a distance built up from a `[0, 1)`-range clock phase) only ever pass a non-negative finite value, so the branch had never fired outside a test. The new test drives a negative distance, negative infinity, and NaN through it and confirms each falls back to the documented 0.0, then confirms a call just past the two-sample support still lands on the same zero rather than an accidental wide clamp. Test-only change, so `Chorus` behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing, including a clean `YouKnow106.RenderDemos`).
- 2026-08-16: Added regression coverage for `HighPass::process`'s own non-finite-state guard -- none of the engine's HighPass instances (`voiceBusCoupling_`, `highPass_`, `commonVcaInputCoupling_`, the output-coupling pair, each voice's `moduleCoupling`/`vcaInputCoupling`, and `noiseSourceHighPass_`) can ever hand it a state that has gone non-finite, since each starts at 0.0 and only ever updates through the filter's own finite arithmetic, so the guard had never actually fired outside a test. The new test poisons a standalone filter's state directly and confirms it lands on the documented 0.0 fallback, then confirms the *next* call recovers a finite output instead of continuing to propagate the poison. Test-only change, so `HighPass` behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing, including a clean `YouKnow106.RenderDemos`).
...
- 2026-08-15: Added regression coverage for the JUCE-free panel width model's two untested defensive branches -- a null legend and a button too narrow to leave any drawing room -- verified by `ctest --test-dir youknow106/build-dsp` (all suites, including `YouKnow106.Engine`, passing) and a bit-identical demo-corpus re-render.
- 2026-08-16: Added regression coverage for `YouKnow106SysEx.cpp`'s null-pointer and short-buffer guards -- `patchFromToneBytes`/`toneBytesFromPatch` with a null pointer and `writePatchMessage`/`writeParameterMessage`/`readPatchMessage`/`readParameterMessage` with a null pointer or a buffer/message one byte short of the documented format -- none of which had direct coverage even though the existing framing tests already exercised malformed message *content*; test-only change, so SysEx behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all suites passing).
- 2026-08-16: Added regression coverage for `Chorus::bbdTransfer`'s own non-finite-input guard and its extreme-finite saturation-rail behaviour -- NaN, +/-infinity, and `std::numeric_limits<float>::max()` in both signs -- none of which had direct coverage; the combined-support fixtures that drive hostile input through the full `Chorus::Line::process` never reach it, because the line's own input gate already sanitises non-finite and out-of-range samples to zero before `bbdTransfer` ever sees them. Test-only change, so BBD transfer behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing, including a clean `YouKnow106.RenderDemos`).
- 2026-08-16: Stopped `renderVoice` from re-deriving `dcoLaunchScale` on every DCO cycle wrap crossed inside one internal sample -- a note fast enough to outrun the sample clock can cross dozens of them there, and each one asked the same voice CV/target/period for the identical ratio -- by resolving it lazily on the first wrap and reusing that cached value for the rest, verified bit-identical against the rendered demo corpus and the full 15-contract test suite.
- 2026-08-16: Deduplicated the identical `converterFraction` lambda (`storedControlDacCode(value) / 4064.0f`) that `updateSharedScan`, `performConverterWrite` and `passiveHoldWriteTarget` in `YouKnow106Engine.cpp` each defined separately into one shared `converterDacFraction` helper, with no change to converter-write behaviour, verified bit-identical against the rendered demo corpus and the full 15-contract test suite.
- 2026-08-16: Deduplicated the xorshift32 state-advance step and the 24-bit-state-to-bipolar-float conversion that the shared noise generator, each voice's microscopic filter excitation and the per-card analogue drift wander each hand-wrote inline in `YouKnow106Engine.cpp`, plus `hashBipolar`'s own copy of the latter, into two shared `xorshift32`/`bipolarFromState` helpers, verified bit-identical against the rendered demo corpus and the full 15-contract test suite.
- 2026-08-16: Added regression coverage for `Chorus::rateProportionalNoiseGain`, the causal alternative to the empirical mode-II noise calibration -- no test called this public static function directly before now, only the resulting audio energy through a full engine render; the new test asserts its exact contract (unity at mode I's own rate, the schematic's own mode-rate ratio at mode II's) and its non-positive/NaN-rate fallback guard, none of which process() ever exercises. Test-only change, so chorus behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing) and a bit-identical demo-corpus re-render.
- 2026-08-16: Added regression coverage for `outputReferenceGain`'s non-positive/non-finite Vref fallback and `outputSummerClip`'s non-finite-input guard -- process() only ever calls the former with a positive compile-time compatibility constant and only ever feeds the latter already-sanitised samples, so neither fallback branch had direct coverage before now. Test-only change, so engine output is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing).
- 2026-08-16: Added regression coverage for `Chorus::deterministicToneStep`'s own non-finite-frequency and non-positive/non-finite-sample-rate fallback guard -- its two production call sites (the hum oscillator and each clock-spur oscillator) only ever pass `sampleRate_`, which `prepare()` fixes positive and finite, and frequencies built from configured or clock-derived floats, so the existing hum/clock-spur fixture in `testChorusNoiseComponents` never actually drives a poisoned argument through it. The new test poisons the frequency and the sample rate in turn, confirms each falls back to silence without advancing the caller's phase, then confirms a following clean call still advances normally. Test-only change, so chorus behaviour is unaffected by construction, verified by `ctest --test-dir youknow106/build-dsp` (all 15 suites passing).
...
- 2026-08-18: Fixed six paint defects found by a systematic fit/state review of the polished editor, all of the same family: text drawn with `drawText`, which silently curtails glyphs, into cells that a reachable state or platform font overruns. The monitor's voice readout ellipsised "0 / 6 ACTIVE" to "ACTI…" at the default editor size (the committed screenshot happened to catch the shorter STANDBY state); the PANIC compact legend overran its cell with the wider Windows system font at the minimum size; both now use fitted text. Voice-lamp numerals above 9 curtailed "10".."16" to a misleading "1" at every supported size once VOICES exceeded nine, so the row now numbers the lamps only while the widest numeral actually fits its cell and leaves the count to the readout beside it. The PANIC legend's LED-red colour turned out to be inherited by accident from the icon painter's leftover graphics state; it is now set deliberately, so an icon change cannot silently recolour legends. Compact keys (including the preset chevrons, which are disabled at the ends of the program list) now dim when disabled instead of painting fully enabled. Rotary tick tips are capped inside the component so the left/up/right majors are no longer flat-cut by the clip boundary on knobs past 54 px, and two dead `setFont` calls on the seven-segment patch display (which never reads a font) were removed.
- 2026-08-18: Ran an external sanity validation of the whole factory bank against public hardware recordings: all 128 presets rendered through the exact product path and compared, preset by preset, against synthmania.com's per-patch JUNO-106 recordings on gated 1/3-octave spectral shape, centroid and band balance, with a register-matched second pass and per-preset adjudication of every outlier ([Docs/external-sound-validation.md](Docs/external-sound-validation.md)). Corpus-level agreement is strong -- median shape correlation 0.897, 94/128 at or above 0.80 -- and every flagged preset but one adjudicated as the model rendering exactly what the stored tone bytes dictate, with the delta explained by score content, register or the reference chain; two adjudications (B52's resonant-peak law, A21's harmonic rolloff) independently corroborate the cutoff/resonance calibration at absolute frequencies on real program material. The one engine-side finding -- the noise-versus-sub balance inverting on the noise-forward drum programs against the recording -- is recorded as a dated quantified contradiction under OQ-15 with the family it implicates (A64/A65/A66), not re-voiced from an undocumented MP3 chain. No constant, preset byte or audit gate changed.
