...
RenderMetrics render (mars::MarsEngine& engine, int samples)
{
...
mars::EngineParameters makeParameters (int mode, int chordQuality,
                                         int profile, int vowel)
{
    mars::EngineParameters parameters;
    parameters.profile = static_cast<mars::VoiceProfile> (profile);
    parameters.mode = static_cast<mars::PerformanceMode> (mode);
    parameters.vowel = static_cast<mars::FilterType> (vowel);
    parameters.chordQuality = static_cast<mars::FifthQuality> (chordQuality);
...
int main()
{
    ...
    if (failureCount != 0)
    {
        std::cerr << failureCount << " Mars DSP check(s) failed.\n";
        return EXIT_FAILURE;
    }
