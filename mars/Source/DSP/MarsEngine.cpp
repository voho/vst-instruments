...
void MarsEngine::setParameters(const EngineParameters& p)
{
    const int profile = p.profile == VoiceProfile::Phobos ? 1 : 0;
    const int mode = p.mode == PerformanceMode::Stack ? 1 : (p.mode == PerformanceMode::Fifth ? 2 : 0);
    const int vowel = p.vowel == FilterType::Svf ? 1 : (p.vowel == FilterType::Poles ? 2 : 0);
    const int quality = p.chordQuality == FifthQuality::Locked ? 1 : 0;
...
EngineParameters MarsEngine::snapshotParameters() const noexcept
{
    EngineParameters p;
    p.profile = atomicParameters_.profile.load(std::memory_order_relaxed) == 1 ? VoiceProfile::Phobos : VoiceProfile::Saturn;
    const int mode = atomicParameters_.mode.load(std::memory_order_relaxed);
    p.mode = mode == 1 ? PerformanceMode::Stack : (mode == 2 ? PerformanceMode::Fifth : PerformanceMode::Single);
    const int vowel = atomicParameters_.vowel.load(std::memory_order_relaxed);
    p.vowel = vowel == 1 ? FilterType::Svf : (vowel == 2 ? FilterType::Poles : FilterType::Ladder);
    p.chordQuality = atomicParameters_.chordQuality.load(std::memory_order_relaxed) == 1 ? FifthQuality::Locked : FifthQuality::Free;
...
void MarsEngine::buildSingerIdentities()
{
    for (int i = 0; i < singerCount; ++i)
    {
...
int MarsEngine::voicesForMode(const EngineParameters& p) const noexcept
{
    if (p.mode == PerformanceMode::Fifth)
        return 6;
    if (p.mode == PerformanceMode::Stack)
        return p.choirSize <= 6 ? 4 : (p.choirSize <= 10 ? 8 : 12);
    return 1;
}

int MarsEngine::chordMidiForSinger(int root, int singer, const EngineParameters& p) const noexcept
{
    const int third = p.chordQuality == FifthQuality::Locked ? 3 : 4;
    ...
    const int low = p.profile == VoiceProfile::Phobos ? 35 : 47;
    const int high = p.profile == VoiceProfile::Phobos ? 79 : 91;
    while (note < low)
        note += 12;
    while (note > high)
        note -= 12;
...
void MarsEngine::makeRoomFor(int required)
{
    int free = 0;
...
void MarsEngine::noteOn(int midiNote, float velocity)
{
    ...
    ++generation_;
    const float modeTrim = total == 1 ? 0.88f : (p.mode == PerformanceMode::Fifth ? 0.61f : 0.72f);
    ...
    for (int singer = 0; singer < total; ++singer)
    {
        ...
        if (slot < 0)
            break;
        const int soundingMidi = p.mode == PerformanceMode::Fifth ? chordMidiForSinger(midiNote, singer, p) : midiNote;
...
void MarsEngine::initialiseVoice(Voice& voice, int rootMidi, int soundingMidi, int singerIndex,
...
void MarsEngine::silenceVoice(Voice& voice) noexcept
{
    voice.active = false;
...
float MarsEngine::midiToHz(int midiNote) noexcept
{
    return 440.0f * std::exp2((static_cast<float>(midiNote) - 69.0f) / 12.0f);
}

void MarsEngine::updateResonator(Resonator& resonator, float frequency, float bandwidth) const noexcept
{
...
void MarsEngine::updateVoiceControl(Voice& voice, const EngineParameters& p)
{
    ...
    {
        const float base = p.profile == VoiceProfile::Saturn
        ...
        const float bw = (p.profile == VoiceProfile::Saturn ? femaleBw[formant] : maleBw[formant])
    ...
    const float pan = std::clamp(singer.pan * p.spread * (voice.singer == 0 && p.mode == PerformanceMode::Single ? 0.18f : 1.0f), -1.0f, 1.0f);
    voice.panLeft = std::sqrt(0.5f * (1.0f - pan));
    voice.panRight = std::sqrt(0.5f * (1.0f + pan));
}

float MarsEngine::tableLookup(const std::array<float, tableSize>& table, float phase) const noexcept
{
...
float MarsEngine::sine(float phase) const noexcept
{
    return tableLookup(sineTable_, phase);
}

float MarsEngine::randomBipolar(Voice& voice) noexcept
{
...
void MarsEngine::updateRoom(float inputLeft, float inputRight, float amount,
...
int MarsEngine::getActiveVoiceCount() const
{
