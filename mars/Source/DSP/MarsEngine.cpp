...
void MarsEngine::updateProcessingRate() noexcept
{
    ...
    // The Ornstein-Uhlenbeck drift constants depend only on the processing
    // rate; computing them here instead of on every card control tick removes
    // four transcendentals per card per tick from the hot path.
                               ...
                               float &outputRight) noexcept
{
    ...
    // unobservable, so both bucket-brigade lines and the compander can rest.
    // Their held charge resumes decaying when the wet gain next rises from
    // zero, refilled with live signal within one line delay.
