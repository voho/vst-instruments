...
VST3="${ARTIFACT_DIR}/VST3/Mars.vst3"
AU="${ARTIFACT_DIR}/AU/Mars.component"
APP="${ARTIFACT_DIR}/Standalone/Mars.app"
...
VST3_ARCHS="$(lipo -archs "${VST3}/Contents/MacOS/Mars")"
AU_ARCHS="$(lipo -archs "${AU}/Contents/MacOS/Mars")"
APP_ARCHS="$(lipo -archs "${APP}/Contents/MacOS/Mars")"
...
ditto "${VST3}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Mars.vst3"
ditto "${AU}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Mars.component"
ditto "${APP}" "${PACKAGE_ROOT}/Applications/Mars.app"

NOTICE_ROOT="${PACKAGE_ROOT}/Library/Application Support/Mars/Documentation"
...
mkdir -p "${NOTICE_ROOT}"
ditto "${PROJECT_DIR}/LICENSE" "${NOTICE_ROOT}/Mars-LICENSE.txt"
...
for bundle in \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Mars.vst3" \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Mars.component" \
    "${PACKAGE_ROOT}/Applications/Mars.app"; do
    bundle_notice_root="${bundle}/Contents/Resources/Documentation"
    mkdir -p "${bundle_notice_root}"
    ditto "${PROJECT_DIR}/LICENSE" "${bundle_notice_root}/Mars-LICENSE.txt"
    ditto "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
...
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/Mars.vst3"
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/Mars.component"
sign_bundle "${PACKAGE_ROOT}/Applications/Mars.app"

ZIP_PATH="${DIST_DIR}/Mars-${VERSION}-macOS-${ARTIFACT_ARCH}.zip"
PKG_UNSIGNED="${DIST_DIR}/Mars-${VERSION}-unsigned.pkg"
PKG_FINAL="${DIST_DIR}/Mars-${VERSION}-macOS-${ARTIFACT_ARCH}.pkg"

# Leave exactly one Mars release set in dist, avoiding stale version or
# architecture names in wildcard-driven CI publication.
rm -f "${DIST_DIR}/Mars-"*-macOS-*.zip
rm -f "${DIST_DIR}/Mars-"*-macOS-*.pkg
rm -f "${DIST_DIR}/Mars-"*-unsigned.pkg

COPYFILE_DISABLE=1 pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier audio.mars.synth.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
