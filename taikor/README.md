...
- 2026-08-17: Deduplicated the absolute voice-lifetime hard cap - `static_cast<std::uint64_t> (maximumTailSeconds * sampleRate_)`, the twelve-second ceiling that bounds every voice and physical bank's `maximumSamples` - previously rebuilt identically at five call sites (`buildVoiceModes`'s post-strike deadline, `ensurePhysicalDrum`'s fresh-bank deadline, `trigger()`'s post-schedule extension and its direct-path deadline, and `applyTensionShift`'s lengthened-lifetime clamp) into one shared `maximumTailSamples()` helper. A pure code-motion change - the same double multiply and cast, now computed once per call site instead of typed out five times - with no effect on any resolved drum or rendered audio, confirmed by `ctest --test-dir taikor/build-dsp` (all tests pass) and a full 25-file demo re-render sha256-compared byte-for-byte against the committed WAVs.
- 2026-08-17: Added direct regression coverage for `mountingLossAt`'s own domain guard: `resolveDrumGeometry` only ever derives `mountCorner` as a fixed constant divided by `drum.radius`, itself clamped to `[radiusFloor, radiusCeiling]` (0.008 m .. 3.75 m), so the corner it ever hands the function sits between roughly 7 Hz and 3.3 kHz - never at or below the `std::max(mountCorner, 1.0f)` floor - and the guard was previously exercised nowhere at all, direct or indirect, unlike the fourth-order shelf formula it protects. The new test pins the zero/negative/sub-floor corner against the same floored 1 Hz shelf, checks the 0/0 case the floor exists to avoid (a zero corner with a zero frequency) resolves to the finite unbent shelf rather than NaN, and pins an ordinary corner against the documented closed form; confirmed as a real regression by temporarily replacing the floored divisor with the raw `mountCorner` and watching the four new assertions fail before reverting. Test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Deduplicated the DC blocker's and drive stage's shared reset, previously written out identically in `reset()`, `allSoundsOff()` and `process()`'s idle freeze, into one shared `resetOutputStagePath` helper, with no change to any resolved drum or rendered audio.
- 2026-08-16: Added direct regression coverage for `stiffnessStretch`'s own domain guard: `resolveDrumGeometry` always feeds it a strictly positive `stiffnessBatter`/`stiffnessResonant` (a positive modulus and thickness cubed over a positive tension and radius squared), so the `!(stiffness > 0.0f)` fallback to an ideal, unstretched membrane was never reached from `Source/` and, unlike its ordinary branch, was never asserted directly - only inferred through `testHeadStiffnessOpensTheModalRatios`'s resolved-drum readouts. The new test pins the zero/negative/NaN fallback, the (0,1) mode's own no-op case, and an ordinary higher mode against the documented closed form. Test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Added direct regression coverage for `UiMath::rowLayout`'s cell-size floor on a row too narrow for its own columns: the existing tests only asserted the floor for a zero extent and for zero columns, both of which collapse `available` to zero or negative before the `available > columns` comparison is even reached. A positive extent that is merely smaller than its column count - nine pixels of usable space shared across twelve columns, say - takes the same `cellSize = 1` floor through that comparison directly, and integer-divides to a zero-width cell without it; the new case also checks that the centring `origin` clamps to zero rather than the negative value nine pixels of usable space and twenty-three pixels of demanded cells naturally produces. No call site in `PluginEditor.cpp` reaches this branch - the editor's own minimum width (1024 px) is more than an order of magnitude wider than any row's column count needs - so it was previously exercised nowhere at all. Test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Extracted the far-field microphone spread term - `1 / (1 + micDistance / 0.12)`, written out identically in `observeMode` and in `buildVoiceModes` - into one shared `propagatingSpreadFor` helper, alongside the near-field and proximity terms the same two functions already share through `nearFieldAttenuation` and `proximityLift`. A pure code-motion change with no effect on any resolved drum or rendered audio, confirmed by the DSP test suite and bit-identical demo renders (sha256-compared against the committed WAVs) before and after.
- 2026-08-16: Added direct regression coverage for `columnStiffnessFactor`'s own domain guards: its `!(x > 0.0f)` fallback to the uncorrected low-frequency limit (`1.0`, reached at zero, a negative `x`, or NaN - the sole call site can land exactly on zero when `volumeBranchOmega`'s eigenvalue has collapsed to non-positive and it returns `0.0` rather than a frequency) and its quarter-wave floor (`0.0` at and beyond `x = pi/2`, where the column is past the one branch on which a lumped stiffness has a meaning). `testTheCavityIsAColumnNotAnInfiniteSpring` already exercised the function end-to-end through a resolved drum's `cavityStiffnessFactor` across the ordinary range, including configurations where the factor reaches zero, but nothing asserted either domain guard - or the plain `x cot x` formula - directly, and the continuity of the truncation at the quarter-wave itself was undocumented in the test suite; test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
...
- 2026-08-16: Extracted the impulse-invariant two-pole coefficient solve - the same `omega`/pole-radius/`a1`/`a2`/`sin(omega)` arithmetic written out separately in `configureResonator` and in `applyTensionShift`'s per-mode retune (the latter's copy carried a comment promising it matched the former by hand) - into one shared `polePairFor` helper, both call sites keeping their own gain scaling and out-parameters on top of it. A pure code-motion change with no effect on any resolved drum or rendered audio, confirmed by the DSP test suite and bit-identical demo renders before and after.
- 2026-08-16: Added direct regression coverage for `UiMath::semitonesBetween`'s invalid-input guard: only its `frequencyHz == 0` branch was previously asserted, leaving a negative frequency, a zero or negative `referenceHz`, and a NaN frequency untested even though the same `! (frequencyHz > 0.0f) || ! (referenceHz > 0.0f)` guard covers all of them; test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Added direct regression coverage for `parametersForOctave`'s two documented identities: an Octave Body of 0 (and the negative/NaN inputs that clamp to it before its `! (body > 0.0f)` guard is taken) must return the parameter block untouched at any octave, and octave 0 must return it untouched at any Octave Body, because `getDrumDescription(0)` is both the reference and the drum being compared against so every delta the transform would add is exactly zero. Every existing test reached both identities only indirectly, by rendering a full drum through `resolveDrumFor`, and never asserted either one directly or checked that a case away from both endpoints actually moves one of the five fields the transform owns; test-only change (a `TaikoEngineTestAccess::parametersForOctave` wrapper was added alongside the existing `sanitise` one, since the method itself is private), verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Added direct regression coverage for `UiMath::smoothStep`'s zero-span branch: the existing test only exercised `value >= edge0` (returning 1.0) when `edge0 == edge1`, leaving the `value < edge0` side of that same ternary (returning 0.0) unasserted; the function isn't called anywhere else in `Source/`, so that half of the branch was previously exercised nowhere at all. Test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Extracted the four-field mode start-up (`omega`, `liveOmega`, `decayRate`, `appliedPalmDecay`) written out identically across `buildVoiceModes`'s axisymmetric, circumferential and shell loops into a shared `beginMode` helper, a pure code-motion change confirmed bit-identical by the DSP test suite and demo renders.
- 2026-08-16: Added direct regression coverage for `applyCollisionRetention`'s own defensive fallback: when a mode's `poleRadius`, `resonator.b0` or `liveOmega` has collapsed, the function cannot recover velocity from the live pole coordinates and instead retains a plain backward difference between `q[n]` and `q[n-1]`. `configureResonator` and `applyTensionShift` both keep those three coordinates strictly positive for every mode either one builds, so no ordinary mode ever took this branch, and `testCollisionChangesVelocityNotDisplacement` had only ever driven the function with a healthy pole; test-only change, verified with `ctest --test-dir taikor/build-dsp` (all tests pass, including the new assertions) and `git status taikor/Docs/audio` clean since no engine code changed.
- 2026-08-16: Removed `renderVoice`'s pre-reciprocal-contact fallback: a scheduled, non-reciprocal `sin^1.5` Hertz arch that read `voice.contacts[voice.nextContact]` and only ran when `nonlinearContact` was false and a contact had come due. Both conditions were unreachable from any call site - every playable `StrikeProfile` (Don, Ka, Tsu, Don Rim) has `membraneGain > 0`, so `trigger()` always sets `nonlinearContactActive` true before the voice is first rendered, and `trigger()` sets `nextContact` equal to `contactCount` in that same call, so the "a contact has come due" condition the fallback needed was already false as well - leftover scaffolding from before the reciprocal IMP-2 bachi solve replaced it. `voice.contactLength` and `voice.contactReference` lost their only remaining reader with it and are removed too; `contacts[]`/`contactCount` stay, since `trigger()` still uses them to size a voice's retirement schedule for a flam or a press roll, as does `contactAmplitude`/`contactNoiseAmplitude` (now documented as set directly by `trigger()` rather than re-read per contact) feeding the live nonlinear branch. Provably a no-op on any reachable voice; verified with `ctest --test-dir taikor/build-dsp` (all tests pass) and a full 25-file demo re-render, `git status taikor/Docs/audio` clean, confirming bit-exact output.
