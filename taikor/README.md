...
| --- | --- | ---: | ---: | ---: |
| `01-stroke-vocabulary.wav` | All four strokes on the o-daiko, in keyboard order | 4.2 s | −11.7 dBFS | +8.7 dB |
| `02-the-four-drums.wav` | A Don on each of the four drums: o-daiko, nagado-daiko, okedo, shime | 6.3 s | −22.2 dBFS | +19.2 dB |
| `03-the-playing-grid.wav` | The whole grid: four strokes on each of the four drums | 10.1 s | −11.5 dBFS | +8.5 dB |
| `04-drums-ka.wav` | An edge-of-head stroke on each of the four drums | 3.9 s | −18.7 dBFS | +15.7 dB |
| `05-drums-rim-shot.wav` | A rim shot on each of the four drums | 5.1 s | −10.7 dBFS | +7.7 dB |
| `06-odaiko-phrase.wav` | A phrase on the o-daiko | 10.0 s | −11.4 dBFS | +8.4 dB |
| `07-chudaiko-phrase.wav` | The same phrase on the nagado-daiko | 6.7 s | −16.6 dBFS | +13.6 dB |
| `08-okedo-phrase.wav` | The same phrase on the okedo-daiko | 5.1 s | −9.9 dBFS | +6.9 dB |
| `09-shime-phrase.wav` | The same phrase on the shime-daiko | 4.2 s | −21.1 dBFS | +18.1 dB |
| `10-velocity-dynamics.wav` | One stroke from a ghost note to a full-arm hit | 5.9 s | −15.9 dBFS | +12.9 dB |
| `11-rolls-and-presses.wav` | Press rolls, a played flam and a roll accelerating into a rim shot | 5.4 s | −7.4 dBFS | +4.4 dB |
| `12-bachi-hardness.wav` | Felt beater through to a hard oak bachi | 4.8 s | −13.6 dBFS | +10.6 dB |
| `13-strike-position.wav` | The same stroke walked from centre to rim | 4.6 s | −16.0 dBFS | +13.0 dB |
| `14-head-tension.wav` | Slack head through to fully tacked | 5.5 s | −16.6 dBFS | +13.6 dB |
| `15-head-material.wav` | Thin synthetic film through to thick cowhide | 5.5 s | −21.2 dBFS | +18.2 dB |
...
| `18-body-depth.wav` | Shallow body through to deep | 5.5 s | −20.4 dBFS | +17.4 dB |
| `19-head-damping.wav` | Open head through to heavily damped | 6.3 s | −20.4 dBFS | +17.4 dB |
| `20-octave-body.wav` | The keyboard as one drum retuned four times, then as the four drums | 7.8 s | −19.3 dBFS | +16.3 dB |
| `21-mic-distance.wav` | The close pair from 3 cm out to 40 cm, on the head and then on the body | 8.5 s | −8.3 dBFS | +5.3 dB |
...
| `24-pitch-wheel.wav` | The wheel pressing the head sharp and flat | 4.3 s | −21.2 dBFS | +18.2 dB |
| `25-ensemble-piece.wav` | A longer piece moving between all four drums of the grid | 10.4 s | −11.2 dBFS | +8.2 dB |
| `26-polar-strikes.wav` | Fixed left, centre and right strikes, then one circuit around the head | 8.6 s | −21.3 dBFS | +18.3 dB |
| `27-performer-ensemble.wav` | Phase-locked P1 copies, then the same phrase layered as P1-P4 | 5.7 s | −12.8 dBFS | +9.8 dB |
<!-- peaks-table-end -->
