...
void patchShaperVoice(EngineParameters& p)
{
    p.filterPathA = 0.0f;
    p.shaperPathA = 0.8f;
    p.shaperMode = ghost::ShaperMode::Free;
    p.shaperRate = 0.6f;
}
...
{
    p.filterPathA = 0.8f;
    p.modSource = ghost::ModSource::LfoTriangle;
    p.modXTo = ghost::ModXDestination::FilterUL;
    p.lfoRate = 0.55f;
}
...
void patchShaperToOscB(EngineParameters& p)
{
    ...
    p.shaperMode = ghost::ShaperMode::Free;
    p.shaperRate = 0.5f;
    p.shaperYTo = ghost::ShaperYDestination::OscB;
}

void patchGlide(EngineParameters& p)
{
    p.filterPathA = 0.8f;
    p.glideMode = ghost::GlideMode::On;
    p.glide = 0.4f;
}
...
const std::array<TravelCase, 30>& travelCases()
{
    static const std::array<TravelCase, 30> cases {{
        ...
        { "lowerOnly", [](EngineParameters& p, float v) { p.lowerOnly = v; },
          [](EngineParameters& p) {
              p.filterPathA = 0.8f;
              p.lowerMode = ghost::LowerFilterMode::HighPass;
          },
          EventStyle::HeldNote, false, false },
        { "resonance", [](EngineParameters& p, float v) { p.resonance = v; },
          [](EngineParameters& p) {
              p.filterPathA = 0.8f;
              p.upperResonance = ghost::UpperResonanceMode::Variable;
              p.cutoff = 0.5f;
          },
          EventStyle::HeldNote, false, false },
...
int main(int argc, char** argv)
{
    ...
    std::printf("Ghost zipper audit: latching residual vs a sample-accurate"
                " reference, %.0f Hz\n", sampleRate);
