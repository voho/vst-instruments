...
struct YouKnowTestAccess
{
    ...
    static Wiring q1Wiring(YouKnowEngine& engine, double hostRate)
    {
        engine.sampleRate_ = std::clamp(
            hostRate, YouKnowEngine::minimumSupportedSampleRate,
                               ...
                               bool legacyUnclamped = false)
    {
        ...
        if (legacyUnclamped)
        {
            ...
            engine.noiseSourceLowPassG_ = std::tan(
                productionPi * YouKnowEngine::noiseSourceLowPassHz()
...
int selfTest()
{
    ...
    {
        const auto wiring = YouKnowTestAccess::wiring(row.hostRate, row.hq);
    ...
    youknow::YouKnowEngine denseEngine;
    denseEngine.prepare(8000.0, 64, false);
    int denseCells = 0;
    for (int rate = 8000; rate <= 12000; ++rate)
    {
        const auto wiring = YouKnowTestAccess::q1Wiring(denseEngine, rate);
    ...
    {
        const auto wiring = YouKnowTestAccess::wiring(rate, false);
        const auto actual = YouKnowTestAccess::impulse(rate, false, 4096);
    ...
    {
        const auto blockOne = YouKnowTestAccess::publicRun(rate, false, 1);
        const auto blockPrime = YouKnowTestAccess::publicRun(rate, false, 37);
    ...
    {
        const auto run = YouKnowTestAccess::publicRun(
            row.hostRate, row.hq, 37);
        const auto legacy = YouKnowTestAccess::publicRun(
...
int main(int argc, char** argv)
{
    if (argc == 2 && std::string_view(argv[1]) == "--self-test")
        return selfTest();
    std::cerr << "usage: YouKnowNoiseSourceQualityAudit --self-test\n";
    return 1;
}
