...
struct YouKnowTestAccess
{
    ...
    {
        return YouKnowEngine::voiceBusCouplingCornerHz(mode);
    }

    static float legCorner(HighPassMode mode) noexcept
    {
        return YouKnowEngine::highPassCornerHz(mode);
    }

    static float lowGain(HighPassMode mode) noexcept
    {
        return YouKnowEngine::highPassShelfGain(mode);
    }

    static float highGain(HighPassMode mode) noexcept
    {
        return YouKnowEngine::highPassHighGain(mode);
    }
    ...
    {
        YouKnowEngine::HighPass preserved;
        preserved.reset();
        ...
        YouKnowEngine::HighPass resetMutation;
        resetMutation.reset();
...
using Mode = youknow::HighPassMode;
...
int main(int argc, char** argv)
{
    if (argc == 2 && std::string_view(argv[1]) == "--self-test")
        return selfTest();
    std::cerr << "usage: YouKnowHighPassNetworkAudit --self-test\n";
    return 2;
}
