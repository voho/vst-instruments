...
inline constexpr auto pitchBend    = "pitchBend";
...
namespace colour
{
inline constexpr std::uint32_t faceplate     = 0x252722u; // worn charcoal
inline constexpr std::uint32_t faceplateHigh = 0x383b35u; // control surfaces
inline constexpr std::uint32_t faceplateLow  = 0x181b18u; // recessed surfaces
inline constexpr std::uint32_t accentMuted   = 0xb8322au; // red screen print
inline constexpr std::uint32_t accent        = 0x71a7b8u; // faded blue accent
inline constexpr std::uint32_t control       = 0xd8d2beu; // aged ivory caps
inline constexpr std::uint32_t controlShadow = 0x696e64u;
inline constexpr std::uint32_t led           = 0x68c57au; // green panel lamps
inline constexpr std::uint32_t ledDim        = 0x34513au;
inline constexpr std::uint32_t text          = 0xf2eee0u;
inline constexpr std::uint32_t textDim       = 0xc4c0b1u;
inline constexpr std::uint32_t blueBar       = 0x236f89u;
inline constexpr std::uint32_t redBar    = 0xb8322au;
inline constexpr std::uint32_t slot          = 0x0e1012u; // slider track
inline constexpr std::uint32_t scope         = slot; // dark monitor/group fill

// Neutral edges and headings keep the accent reserved for interaction.
inline constexpr std::uint32_t edge          = 0x75776du;
inline constexpr std::uint32_t heading       = 0xfff9e8u;
} // namespace colour
...
inline constexpr float presetHeight = 48.0f;
inline constexpr float panelHeight = 440.0f;
inline constexpr float keyboardHeight = 156.0f;
...
// headings share one centred typographic style while retaining a quiet hierarchy.
...
// Add-on keys share these default-size text, icon and padding budgets, scaled
// with the editor. A readable minimum protects the smallest supported window.
...
//
// The height sets the size and the editor scale sets its ceiling. Text and
// its available space grow together above the default size.
...
[[nodiscard]] float compactLegendWidth(float buttonWidth, float buttonHeight,
                                       bool hasIcon,
                                       float editorScale = defaultEditorScale) noexcept;
