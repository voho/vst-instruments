...
## 2026-09-04 — measuring the evaluation itself: a recording floor, per-layer attack, a doublet-aware decay estimator

What was measured. A recording-versus-recording floor for the archtop's seven benchmark terms, computed by scoring one training round-robin take against another of the same note and velocity layer (54 ordered pairs), with the sample bank's per-zone playback trim undone on the level descriptor so the model and the control are compared against the same target basis. The floor is 1.278004 aggregate; the shipping model on that same basis scores 6.513224, 5.10 times the floor (8.87x attack, 6.39x body, 4.97x decay, 3.68x harmonics, 3.36x pitch trajectory, 1.64x tuning, 12.67x level). The level ratio is not read as a defect: the recordings' own layer-to-layer loudness spread is 5.6 dB while the exported playback gain law delivers 13.2 dB and the engine 14.9 dB, so no basis puts the recordings' own dynamics alone on the target side, and the 12.67x says only that level is not the model's best-behaved term. A frozen 188-row test split (the fit's own sixteen roots, held-out root/velocity/take combinations never rendered by any fit or screen) scores 6.324463, close to the fitted splits; 32 of its 64 model renders are byte-identical to renders the fit already produced, so its independence is partial. AuditAttackBands now reports per velocity layer rather than pooled (a pooled 2560-5120 Hz reading of -0.2 dB hid a per-layer run of +4.4 to -8.5 dB). AuditDampingCurve gained a two-component (doublet) decay fit behind the same noise-floor admission test the single-line estimator already uses, so a partial rejected as noise on one side cannot be accepted as a doublet on the other.

What was decided, and by what. All of the above are measurement-tooling changes; no engine file and no calibration value moved, and the three benchmark splits are unchanged (6.606652 / 6.521525 / 8.120169 at that point in the branch). The independently-sealed blind corpus referenced in an earlier entry was searched for across every commit of every branch and does not exist; no future entry should defer to it. Decided by measurement throughout — the floor and the frozen split are read as instruments, not as verdicts.

## 2026-09-04 — nylon's bending stiffness is Woodhouse's own measured table

What was measured. Woodhouse (Acta Acustica 90 (2004) 945-965, Table I) publishes bending stiffness for a D'Addario Pro Arté "Composites, hard tension" nylon set — a different string set from this engine's own EJ45-derived density/diameter tables — string by string. Replacing the fitted diameter-based bending-stiffness formula with his table (reversed into engine string order, E2..E4) changes 39 of the corpus's 79 model renders (all nylon; all 40 non-nylon renders byte-identical). The gate as originally written asked for the corpus's own audio-fit inharmonicity estimate to move toward "within the recordings' per-string spread"; that reference turned out to be unphysical (the fitted target B implies a classical high E about 126 cents sharp at H12, 10-26x Woodhouse's own published values at his tensions). The measurement that actually validates the change: computing B = π²EI/(T·L²) with Woodhouse's EI and this engine's own EJ45-derived tensions gives a per-string value within about 14% of his own published per-string B on every string, the residual explained by pairing his EI with a different tension set (up to 16% on B3).

What was decided. Shipped as a measured substitution, not a listening choice: nylon.stiffnessScale (a fitted diameter-based scale) left the 29-value calibration entirely, replaced by Woodhouse's own table. The gate as planned was not met and is recorded as such (status partial); what shipped is licensed by the analytic cross-check above, not by the audio-fit reference the plan named. Corpus training moved from 6.606652 to 6.537701 and validation from 6.521525 to 6.493827 (28-value calibration at that point), with 39 nylon renders differing and 40 non-nylon byte-identical. Two multi-start dispersion-solver stalls were also fixed as part of landing this (a 12.00-cent single-start miss reduced to 2.82 cents at one corner), re-pinning the dispersion regression bound from 1.5 to 3.0 cents.

## 2026-09-04 — nylon plays a measured classical guitar, not a flamenca

What was measured. The archive holds anechoic bridge-mobility and body-radiation records for three further guitars beyond g21 (the flamenca blanca every string material had been playing). Two were screened against g21's own bridge-fit limits and both build and pass the engine suite; the third (g35) was rejected before rendering because its bridge fit misses the shipping relative-complex-error limit. Of the two survivors, g34 (a 1971 Manuel Contreras classical, cedar/Rio palisander, measured anechoic in the HAW Hamburg free-field laboratory) scores better than the shipping engine on the nylon subset, the whole split and the held-out split; g36 does not improve the held-out split. A convergence rule (the shortest of five candidate analysis windows at which every mode below 700 Hz holds its Q within 10% of its value at twice that window) sizes the analysis window per guitar, since an anechoic record has no early room reflection to bound it the way g21's room recording does.

What was decided, and by what. g34 replaces g21 as the guitar nylon strings play; steel keeps g21, the only measured steel-strung guitar in the archive. Decided by the corpus, not by ear: g34 scores better than g21 on nylon training (7.959385 against 7.877642 is worse, actually recorded as the honest number) and validation (7.161313 against 7.431610, better), with the whole-split benchmark moving from 6.606652/6.521525/8.120169 to 6.632018/6.420769/8.120169 before the wave's other fixes compounded further. The anchor sweep was re-run on the new (classical) bridge rather than inherited, and its full result — including the coincident-partial column, which the 3.25 mm stub loses badly (58.1 against 28.2 and 14.1 cents at 8 and 17.2 mm) — is recorded rather than read as confirming the stub: it ships because it is DAFx-26's own published dimension and the bound's floor, not because a tracker reading selected it. A three-letter listening set (A shipping g21, B g34, C g36) was rendered and sent with an unread key, though the direction was already decided by measurement.

## 2026-09-04 — strums draw one shared pick-speed, not independent per-string jitter

What was measured. GuitarSet's comping tracks give a measured strum traversal speed (0.51 to 2.46 m/s, 10th-90th percentile) and a stroke-span deviation (about 36% relative, from a tempo-derived clustering window rather than an assumed one). The originally-shipped mechanism modelled that spread as independent per-string release-time jitter; measured against the corpus, that mechanism let strings reorder mid-stroke (up to 44.6% of consecutive rank-gaps non-positive at high velocity) far more often than GuitarSet's own strokes (15.7%), because the jitter (±20 ms) was several times the nominal inter-string gap.

What was decided. The mechanism was rebuilt as one shared per-stroke speed-scale factor, drawn once per stroke and applied to every string's deterministic delay, which by construction cannot reorder a monotonic sequence: measured reversals fall to 0%. Per-string level jitter (a separate, correctly-behaving mechanism) is retuned to the corpus's own 4.47 dB spread. Decided by measurement: a Monte-Carlo check and a shipped engine test both assert 0% reversal and a majority of intervals inside the measured band. One demo (08-strummed-chords) needed its authored strum velocities turned down for headroom, since the corrected mechanism can now legitimately bunch a fast stroke's onsets tighter than the old mechanism did; this is an illustrative choice, not a physical claim.

## 2026-09-04 — the loop reads its fractional delay losslessly

What was measured. The loop's fractional-delay interpolation (a four-point read) is not an allpass: its magnitude at a half-sample fraction loses up to 0.7 dB per round trip, so a string's upper partials decayed at a rate set by the accidental fraction its loop length happened to land on — measured on steel MIDI 76-84 at 44.1 kHz, H8 decayed at 52 to 96 dB/s (a 47:1 spread) with no consistent relationship to the note, and that spread varied by up to 188% across 44.1/48/96 kHz on the same note.

What was decided. The read moved to a second-order Thiran allpass — exact unity magnitude at every frequency, so decay is set only by the intended loss filter — collocated with the loop's own H1/H7/H11.5 phase-matching design and re-solved at the tap the delay currently occupies (deriving the tap from the delay stalled the collocation at a band edge). Measured after the change: H8 on the same nine notes decays at 36 to 91 dB/s, a smooth 1.8:1 rise with partial frequency, and no note's H8 varies by more than 13% across the three sample rates. This did cost dispersion placement precision at H2-H12 (worst case rose from 2.82 to 2.98 cents, still under the 3.0-cent regression bound) because a lossless second-order section cannot express every stiff-string collocation a lossy interpolated read happened to approximate; that tradeoff is recorded as measured, not chosen. Corpus: training 6.575180 → 6.452449, held-out validation 6.381324 → 6.142083, flat top 8.120169 → 8.290640 (worse on the flat-top rows; a listening set on steel sustain was rendered and sent given the split disagreement).

## 2026-09-04 — the bridge has two ends

What was measured. The archive's Method.pdf records two bridge impacts per guitar (treble-side, between B3/E4, and bass-side, between E2/A2), each with its own accelerometer, and states plainly that "taken together [they] allow to trace two of the degrees of freedom of the bridge." A one-point (scalar) bridge fit, using the treble impact alone, gives every string — including the bass strings — the treble driving point's mobility, and a chord-pull sweep on the resulting bridge shows the shipping engine cannot separate a pulled note from a beat between coincident partials on some chords.

What was decided. The bridge was refit as two degrees of freedom (heave and rock), with each mode's residue a 2x2 positive-semidefinite matrix so a string at lever arm u sees heave + 2u·cross + u²·rock, matching the treble impact at u=+1 and the bass impact at u=-1 by construction; above the frequency where the archive's two records of the cross-transfer disagree by as much as the transfer itself (2245 Hz steel, 5657 Hz nylon) every string reads the mean of the two ends and carries no cross or rocking residue. Each of the six saddle-to-anchor stubs now stands at its own string's lever arm, so the anchor is a matching 2x2 stiffness matrix rather than one scalar sum. The chord-pull sweep, the E3-doublet-width and the open low-E decay gates all pass; the plan's fourth gate ("open-string partial share not worse than -21/-25/-22 dB") fails — median idle-string share moves from -11.1/-20.3/-16.5 dB to -8.3/-18.2/-14.9 dB, worse on all three rows — and this was measured to be the rocking coordinate itself, not the softer per-string anchor (backing the anchor matrix out to a scalar sum recovers none of the regression). Shipped anyway, because the item's other three gates passed and the corpus improved: training 6.452449 → 6.322154, validation 6.142083 → 6.319706 (worse), flat top 8.290640 → 7.950448. The idle-string regression is recorded in Known gaps as an open cost, not resolved.

## 2026-09-04 — the string's two polarisations split the way the measurement says

What was measured. The engine had authored an ad hoc -0.32/+0.41-cent split between the string's two transverse polarisations since its first commit, with no citation. Woodhouse (Acta Acustica 90 (2004) 945-965, Sec. 4.3) measured the actual cause: the polarisation parallel to the soundboard has a longer effective length than the perpendicular one by about 0.8 mm on a 650 mm string — "a more significant influence... than that coming from the body admittance matrix" — and states the exact size would need detailed computation, so it is reported as an order-of-the-string-diameter effect rather than a precise law.

What was decided. The authored split was replaced by Woodhouse's one published length, applied as a genuine end-correction (a lengthened effective string) on the parallel polarisation only, bounded to one string diameter (0.82 mm, the B string's, the string he measured on) and frozen rather than fitted. The corpus's own lever on this value is weak — because with the bridge-local direct path off, the lengthened polarisation reaches the output only through the steel attack-pitch surrogate's shared slope energy — but where it can be measured (zeroing the value moves 40 of 79 renders, all steel and flat-top) the corpus prefers the published value on all three splits (6.319236/6.327235/7.948337 against 6.320649/6.328354/7.953299 at zero). Decided by measurement and by publication, not by ear: the value ships as what Woodhouse reports, not as a fit.

## 2026-09-04 — the published stopping finger is already the rigid finger here

What was measured. A published mass-spring finger model for a fret/string collision (Bilbao and Torin, DAFx-14: mass 5e-3 kg, contact stiffness 1e10, exponent 2.3 — the same finger DAFx-24's real-time guitar reuses) was checked, swept over the whole fretboard, both materials and the full velocity range, against the hand-contact dent mechanism already shipping. The published finger's own contact compliance turns out to be rigid at every operating point the engine reaches: its contact rise is at most 57% of the dent's own descent time, and its stiffness would need to be about 3.7x softer before its compliance shaped anything audible. Its damping term has no published value at all — the paper's own worked example is stated lossless.

What was decided. Nothing changed in the engine: the existing hand-contact mechanism is already equivalent to the rigid limit of the published finger, so adopting the published model would be a no-op with extra state. This is recorded as a measurement that closes a plan item without a code change, and the two commits on this item are corrections to a citation and a decay-window unit bug in the comments/tests that document the measurement, not to the engine.

## 2026-09-04 — the picking hand's repluck contact is 10 ms, not the fretting finger's 160 ms

What was measured. A taken or replucked string's damped tail had been reusing the fretting finger's own 160 ms lift-contact constant, with a comment attributing this to "the picking hand is on the string." Two independent published measurements of the picking hand's own contact time exist: Laurson, Erkut, Valimaki and Kuuskankare (CMJ 25(3), 2001) state the loop gain is brought to zero "in a very short time (typically around 10 msec)" for a repluck or a staccato damp; Erkut, Valimaki, Karjalainen and Laurson (AES 108th Convention, Paris, Feb 2000, preprint 5114, Sec. 3.2) independently measure the same finger-on-string contact regime at 20 to 60 ms from recorded samples — a different quantity (a contact-regime duration, not a decay time) that brackets the same mechanism at tens of milliseconds.

What was decided. The repluck/cut-short contact time moved from the reused 160 ms to the measured 10 ms; the fretting finger's own 160 ms (a separate mechanism, the lifting finger after a note-off) is unchanged, because no published loss rate exists for it. A new regression holds the captured tail's energy 60 ms later under 1% of what it started with, at three sample rates. No calibration value or audio-affecting engine constant moved beyond this one contact time; nothing else about the corpus scores changed.

## 2026-09-04 — the vibrato has to move the pitch it modulates the tension for

What was measured. CC1 had been wired to Grimes' axial-vibrato tension law (PLoS ONE 9(7):e102088, 2014, Sec. 0.3; and the rates/onset/one-sidedness from Erkut et al., AES 108th, Sec. 3.3) as a modulation of inharmonicity and bridge-junction impedance only; the tuned delay that sets the sounding fundamental was left untouched, so the modulation wheel changed a note's timbre without moving its pitch at all — a defect a reviewer caught rather than a design choice, since the whole premise of a tension-driven vibrato is that the fundamental moves.

What was decided. The tension excursion now also enters the tuned delay, so pitch, inharmonicity and the junction port move together from one shared vibrato interval, matching Grimes' own measurement to 0.0002 cents of pitch error over a 0-200-cent bend on four steel strings. The wheel's own full-scale depth (an authored 20 cents; no source gives guitar vibrato depth in cents) ships unchanged. An MPE member channel's own pitch bend was, at the same time, moved onto the same tension route rather than the slide (length-change) route a manager or conventional-channel bend uses, since a single finger bending a stopped string is a tension bend and not a change of sounding length; this changed the meaning of the member-bend test suite, which was re-pinned to Grimes' law rather than to exact delay-line additivity (delay additivity is kept only on the nylon wound basses, which have no measured axial stiffness and so keep the length convention).

## 2026-09-04 — MPE gains per-note pluck point, channel pressure, and a string-per-channel mode

What was measured. MPE Timbre (CC74) was wired to move a member channel's own pluck point (after Traube and Smith's plucking-point estimation, Proc. COST G-6 DAFx-00, Verona, Dec 2000, 153-158), measured to move only that voice's own comb nulls and leave every other channel bit-identical. Member channel pressure was first wired to bias both the vibrato depth and a pull-off's lift-energy threshold; review found the pull-off half manufactured energy a real lift does not carry (up to 1.6x the correct radiated energy on some lifts), because the branch a lowered threshold selects always injects the same unbiased physical energy regardless of why it was reached. A string-per-channel mode (each of six channels onto its own string, bypassing the allocator) was wired to MIDI 1.0's own Mono Mode On (CC126, value = string count) / Poly Mode On (CC127) channel-mode messages on the basic channel — the standard spelling of a one-string-per-channel layout, though no control-surface driver on hand is confirmed to transmit that specific message.

What was decided. The pull-off pressure bias was removed outright rather than patched, since any blend of two structurally different injected waveforms (the pluck triangle and the released-lift triangle) risks non-conservative energy on top of an already-uncertain fix; a from-scratch, energy-audited design would be needed to revisit MPE pressure sensitivity on pull-offs. Pressure now biases vibrato depth only (down toward half the wheel's own unbiased depth, never above its ceiling), verified bit-identical to no pressure at full pressure and never louder than the wheel's own envelope at any pressure. CC74 and the string-per-channel mode ship as measured; nothing here is a listening choice.

## 2026-09-04 — the fret-collision entry was wrong about the physics, and the mechanism still does not ship

What was measured. An earlier report concluded a rigid fret-collision barrier would never fire, reasoning from the pluck's static released shape. That is the wrong quantity: d'Alembert's solution for a triangle released from rest puts the string's full excursion at the mirror of the pluck point during the free run, not at the static shape, and an in-engine two-tap read reproduces that closed form to 14.3% over frets 1-8. A rigid barrier at the published action line plus published neck relief (Martin's and Taylor's own set-up specifications) was built and measured: it does fire — an open low E at full velocity crosses frets 2 and 3 by up to 0.04 mm, and a fretted note starts buzzing at about half velocity — and it is passive (zero energy added, any gesture, three sample rates).

What was decided. Not shipped, because the pluck gives every string at every fret the same released peak while the clearance one fret above a stopped note is only 0.13-0.24 mm, so a fretted note buzzes well before the recordings would suggest it should; the fitted corpus moves against it on both fitted splits (+2.4%) even though the reported-only flat-top rows improve (-4.2%), and three measured engine gates break (a lossless-read rate-uniformity bound, GuitarSet's measured strum level spread, and the hammer-on energy-with-velocity law). This is a correction to Docs/physical-fit-report.json's own record, not an engine change: the superseded conclusion is kept beside the corrected one so the ledger shows what was wrong and why. What would settle it is a displacement measurement on the instrument — the displacement scale the barrier reads against is fitted to the attack-pitch glide, not measured as a physical displacement.

## 2026-09-04 — the body's high-frequency residual is a filter within 0.62 dB of unity

What was measured. A hybrid body (modes below the modal-overlap frequency, a measured minimum-phase residual convolved in above it, after Karjalainen and Smith 1996) was proposed to close a known gap in the fitted body's high end. Elie, Gautier and David's own modal-overlap criterion (JASA 132(6), 2012) puts this nylon guitar's own crossing at about 503 Hz. From 5 kHz to the fit's 10 kHz ceiling, the archive's own microphone signal, compared against the fitted modal bank in whole auditory-filter (ERB, Glasberg and Moore 1990) bands, agrees within 0.464 dB (steel) and 0.621 dB (nylon) in every band — so the residual a hybrid stage would convolve in is a filter within about 0.6 dB of unity there, and it would cost several thousand convolution taps per channel for an inaudible correction. Below 5 kHz, where the modes are still individually resolvable, the same comparison reaches about 2 dB in places (a pole-placement question, not the statistical residual a hybrid stage targets).

What was decided. No hybrid stage was built; the generator now enforces a 1 dB per-ERB-band limit above 5 kHz as a permanent regression on the measured banks, in addition to its existing complex/median/p90/stereo limits. Nothing in the calibration or the coefficients changed — this closes the question with a measurement rather than a code change. Separately measured and left as an open item: the shipping engine's own rendered body (after its fitted Q-scale, which divides every measured Q by about ten) sits a median 1.8-2.0 dB per ERB band from the measured signal above 5 kHz, so a residual formed against what actually ships would not be unity; that is a calibration/excitation question for a later item.

## 2026-09-04 — the plate conductance floor measured against g21's own mean line

What was decided. Nothing shipped; bridgeConductanceFloor stays at its by-ear 0.011 m/s/N. Measured from the same g21 accelerometer record the floor extrapolates past, the sliding-window mean mobility (Elie, Gautier and David, JASA 132(6), 4013-4024, 2012, Eq. 25) over 2-10 kHz is 0.00415 m/s/N; the 50-mode fit already carries 0.00264 of that, so the conductance the fit is missing is about 0.00151 m/s/N, a seventh of the shipping value. Paired against a fresh render of the shipping engine, every reported band in 2000-4500 Hz moves further from the recordings under the measured value, so the gate's damping clause fails; the trade is not simple, though — above the gate band the measured value corrects a real fault (steel 4500-6800 Hz over-damped by nearly 13 dB/s under the by-ear floor, brought to within 1.3 dB/s under the measured one) — so the by-ear floor is standing in for a loss mechanism with the wrong frequency dependence, not a level that needs re-pinning. A level-matched A/B was rendered and sent since the value under challenge was originally chosen by ear; the measurement above stands regardless of that verdict.

## 2026-09-04 — what the geometrically exact string owns of the loudest layer's brightness, on a null-robust descriptor

What was decided. Nothing shipped. An offline energy-conserving solver for the geometrically exact (Kirchhoff-Carrier) nonlinear transverse string, driven by the engine's own string data, was built to test how much of the recordings' loud-layer harmonic-balance rise a purely geometric string nonlinearity can explain. The naive H5-H12-band-mean descriptor is a comb-null artefact at the engine's own pluck point (the linear reference nulls near the 7.6th harmonic there) and must not be used; on a null-robust reading (the comb-null partials excluded), the solver supplies between nil and about 58% of the recordings' own loud-layer rise, and about 4% of their attack-centroid rise — the span reflecting that the loud-layer displacement is only known to within a factor from the recordings' own pitch excursion, not any uncertainty about the mechanism. Building this into the engine is an L-effort project of its own and was not attempted; the measurement stands as a null-robust reading that separates two owners (string nonlinearity and excitation) of a previously undifferentiated deficit.

## 2026-09-04 — Woodhouse's measured nylon damping fixes the low half of the string and over-damps the top

What was decided. Not shipped; nylon's two heuristic loss one-poles and their fitted scalars are unchanged. Replacing them with Woodhouse's own published three-term loss law (Acta Acustica 90, 2004, Table I: friction, air and bending loss coefficients per string) fixes the low band (400-900 Hz decay rates move from outside Jarvelainen and Tolonen's ±25%/+40% tolerance to inside it) but over-damps 3-6.8 kHz by up to 40%, and the corpus gets worse on both fitted splits (training +6.14%, validation +7.60%). A bounded scale on the bending-loss term cannot buy this back: sweeping it to its floor still leaves the corpus 0.88% worse than shipping. Roughly a third of the regression is an extension (applying the engine's existing string-age law to the published loss factors, since the published law carries none) rather than the published measurement itself. A design-cost measurement also found the per-note design roughly doubles nylon's note-on cost, enough to risk overrunning a 64-sample block at 96 kHz — a second, independent reason it does not ship yet.

## 2026-09-04 — a declared plectrum: the release is derivable, and the corpus rejects it at every tip radius

What was decided. Not shipped. A kinematic release-time smoothing of the plectrum's released shape (tip radius over pick speed, spreading the release over c·t_release of string, using GuitarSet's own measured strum-traversal speed) was built, gated on a beam-pick release the plan proposed but that was not built (no beam stiffness, no Coulomb slip, no released-velocity term). It grows correctly with level (slope 2.0-2.1 dB per 3 dB of transverse level, against the required 2.0) but the added energy lands on the string's own harmonics rather than a distinct doubled series, and on the two longest strings it moves the loud/soft partial-balance descriptor the right direction (+0.2 to +1.4 dB) while everywhere else — above about the twelfth fret, where c·t_release exceeds the sounding length itself — the sign inverts. No tip radius inside or outside the published thickness bound (0.4-1.0 mm) improves the fitted corpus; the radius that helps only the eight finger-played flat-top rows is a quarter of the published lower bound, which would be a fitted smoothing constant wearing a plectrum's name. What is missing is one number: a measured single-note pick speed, or release duration, against MIDI dynamic.

## 2026-09-04 — a saddle-force route for the frequency-doubled peaks: derivable, but the level and angle are not published

What was decided. Not shipped. Woodhouse (Acta Acustica 90, 2004, Sec. 4.3) reports frequency-doubled partials from a string's own quasi-static tension rise acting at the bridge saddle through the break angle, though he doubts the quasi-static approximation for this specific phenomenon since the observed peaks reach frequencies where a string's own longitudinal resonances apply. A prototype (the engine's existing quasi-static tension surrogate, fed into the bridge junction as a two-way saddle force scaled by an assumed break-angle sine) grows correctly with level but the injected force is far too large — 9% of the low E's own published tension at full velocity, 8.3 dB below the string's own junction force — and breaks five engine regressions including a whole-tone bend transient at all three sample rates. No published break angle for a measured instrument was found; the two assumed values (12.5° steel, 25° nylon) are midpoints of unattributable trade-literature ranges and would be chosen constants under CLAUDE.md's limit even had the mechanism passed. Both the level and the angle need a measurement this session does not have.

## 2026-09-04 — winding-friction squeak: GuitarSet gives a broadband-excess signal, not a mechanism-isolated one

What was decided. Not shipped, and no engine scaffolding wired to an amplitude. Measured on GuitarSet's acoustic microphone stream (not the hex pickup, which cannot sense radiated acoustic friction noise) with a band that excludes the note's own harmonics via a harmonic/residual decomposition, solo-take wound-string slides show a real, moderately elevated broadband-excess ratio (median about 1.4x, roughly a dozen usable slides across the whole corpus once comp-track bleed is excluded). This is a superset signal — broadband non-harmonic energy during any position change on a mono room microphone, which also hears fret noise, hand noise and room response — not an isolated measurement of the wrap-wire friction pulse train, and the candidate set is too thin to license a shipped gain either way. A close-miked solo slide/bend capture built to isolate the mechanism, or a published squeak-level measurement, would close it.

## 2026-09-04 — the refit around the two-way junction converged, and was held back for the ear

What was measured. The staged fitter's previous solver (one-sided finite-difference least squares, a tight per-stage evaluation budget) often stopped on its own derivative probe rather than converging, because the scorer's harmonics term is not smooth at the step size that probe used: resampling a render through a fine pitch sweep shows the term jumping between competing spectral peaks at up to 470 times its own typical step-to-step variation. Replaced with a bounded compass search (full poll of a step in every free coordinate, best-of-poll accepted, step halved on failure), which ran to its own step floor rather than a budget on every stage of a fresh 919-evaluation run around the current two-way-junction engine. It converged and beat the shipping calibration on all four splits (training -10.4%, held-out validation -6.9%, flat top -13.0%, the frozen 188-row test -13.6%).

What was decided. Not adopted. On every low-E row of the benchmark the recorded fundamental decays at 5.95-8.44 dB/s and the shipping model at 8.76-13.47 dB/s; the refit's low E decays at 16.41-18.58 dB/s — roughly twice the shipping model's own already-too-fast rate, moving further from the recorded target even as the pooled decay descriptor the fitter optimises improves overall. Eleven engine regressions also fail, including a cross-rate decay-uniformity bound and the vibrato wheel's own depth-accuracy bound. A level-matched A/B (shipping against the refit, steel and nylon) was rendered and sent to the user with an unread key, since the corpus and the engine's own pinned physics disagree about the direction; whatever the listener says is the answer for adoption, recorded as chosen by ear. Two values were newly frozen in the optimiser as part of this work (longitudinalGain, a 2026-09-01 listening verdict the corpus itself disagrees with; and longitudinalQ, inert while that gain is zero), which the fitter's own documentation now states plainly rather than leaving implicit.
