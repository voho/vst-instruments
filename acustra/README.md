...
| `02-nylon-fingerstyle.wav` | A fingertip nylon arpeggio with overlapping held notes | 4.2 s | −19.4 dBFS | +16.4 dB |
...
| `06-playing-behaviours.wav` | A chord change over a ringing chord, CC2 bridge-hand damping, then two natural harmonics above the fretted range | 10.7 s | −11.9 dBFS | +8.9 dB |
| `07-fretting-hand.wav` | Hammer-ons at two velocities and a pull-off under CC68, then one fretted note released three ways by its note-off velocity | 8.8 s | −13.9 dBFS | +10.9 dB |
...
| `09-recuerdos-de-la-alhambra.wav` | Tarrega, Recuerdos de la Alhambra, bars 1-12: a nylon tremolo over a thumb arpeggio | 31.6 s | −13.7 dBFS | +10.7 dB |
| `10-lagrima.wav` | Tarrega, Lagrima, bars 1-8: a sung nylon melody over held bass | 26.0 s | −16.7 dBFS | +13.7 dB |
<!-- peaks-table-end -->
...
string is only taken while the picking hand is on it, and it rides in the
junction as a second wave on its string. Measured on a
...
note alone. The fitted 17 mm length that shipped before was a shield, not a
dimension: between the body's modes the measured mobility is mass-like, and
a spring to ground in series with it resonates, so at 17 mm the strings saw
an effective conductance of 0.31 at 233 Hz and 0.047 at 102 Hz where the
bridge's own is 0.004 and 0.005, and a B string in an E chord was pulled 23
cents sharp for half a second. Every length moves that artefact rather than
removing it (the stub's sits at 306, 602 and 668 Hz), and no spring at all
exposes the flamenco body's own 83 and 91 Hz modes; the stub is the length
at which chords pull least (Known gaps), chosen on that measurement.
...
| --- | ---: | ---: | ---: | ---: | ---: |
| Training | 83 | 8.768535 | 6.441362 | 6.319236 | −27.93% |
| Development validation | 24 | 8.145301 | 6.427690 | 6.327235 | −22.32% |
| Flat top (reported only) | 8 | 9.309315 | 7.428581 | 7.948337 | −14.62% |
...
- The two-way junction and the stub anchor are calibrated around a
  ...
  against the recordings' 6, and the benchmark reads 6.607, 6.522 and 8.120
  against the previous engine's 6.835, 6.824 and 5.655, better on the picked
  splits and 44% worse on those eight rows. The string losses and the bridge
  mobility want refitting around the new termination; the fitter freezes the
  by-ear values, so that refit is a separate job.
