...
| --- | --- | ---: | ---: | ---: |
| `01-steel-sustain-range.wav` | Steel sustain from open E2 to B5, one held pluck at a time | 9.5 s | −16.1 dBFS | +13.1 dB |
| `02-nylon-fingerstyle.wav` | A fingertip nylon arpeggio with overlapping held notes | 4.2 s | −22.1 dBFS | +19.1 dB |
| `03-shape-material-anchors.wav` | One chord: Parlor/Jumbo, then Cedar/Maple anchor settings | 13.7 s | −8.2 dBFS | +5.2 dB |
| `04-string-age.wav` | The same steel phrase with fresh strings, then fully aged strings | 6.5 s | −12.2 dBFS | +9.2 dB |
| `05-alternate-tunings.wav` | Drop D, DADGAD and Open G chords | 12.3 s | −10.9 dBFS | +7.9 dB |
| `06-playing-behaviours.wav` | A chord change over a ringing chord, CC2 bridge-hand damping, then two natural harmonics above the fretted range | 10.7 s | −12.1 dBFS | +9.1 dB |
<!-- peaks-table-end -->
...
| --- | ---: | ---: | ---: | ---: |
| Training | 83 | 8.768535 | 6.441362 | −26.54% |
| Development validation | 24 | 8.145301 | 6.427690 | −21.09% |
| Flat top (reported only) | 8 | 9.309315 | 7.428581 | −20.20% |
...
- The model's steel attack reads far darker than the steel references — over
  the first 350 ms its spectral centroid sits about 685 cents below theirs —
  but not for want of a top end. Band by band over that window, each render
  normalised to its own total, the model carries 5.3 dB more energy over
  160--320 Hz and 1.5 dB more over 320--640 Hz, 3.9 dB less over 640--1280 Hz
  and 3.2 dB less over 1280--2560 Hz, 3.6 dB more over 2560--5120 Hz and
  1.4 dB less over 5--10 kHz. The centroid is dragged
...
- The shared gain on the body's 85--145 Hz modes trades the two materials
  against each other rather than helping both. Swept from the 5.43 it was
  fitted to before the saddle anchor changed, down to 3.0 and then 1.5,
  nylon's 80--160 Hz excess went from +2.5 dB to -0.7 and -2.9 while steel's
  160--320 Hz excess grew from +4.6 dB to +5.1 and +5.3, since taking energy
  out of the bottom raises every other band's share. The aggregate preferred
  the fitted value throughout. The refit has since moved that gain to 4.07 on
  its own, and nylon's 80--160 Hz excess with it, to +0.9 dB; the trade the
  sweep found has not been re-run around the new value.
- Nylon's excess is elsewhere and narrower: 6.7 dB over 640--1280 Hz and
  7.1 dB over 1280--2560 Hz, with the rest of the band within 3.4 dB. The two
...
- The training corpus and a flat-top guitar want opposite instruments, and the
  fitted calibration follows the corpus. Six parameters swept one at a time,
  all three splits rescored: against the fitted corpus the model wants less
  bass, a brighter body, more string loss and shorter sustain, and against the
  eight flat-top rows it wants the opposite of all four. Moved together the
  five that matter put training 13.3% and development validation 13.9% worse
  while the flat-top rows go 30% closer, from 7.428581 to 5.194350. That last
  number is not a held-out reading — the five values were chosen by looking at
  those eight rows, which are spent as a selection set for the candidate — and
  no single move improves all three splits, so the fit is at a real local
  optimum on the corpus it was given and the distance to a flat-top is not
  closed by fitting harder. Two A/B listening sets are rendered to
  `tmp/acustra-listening/` and not committed, each with its key.
- The body sweeps in detail. Swept one at a time and rescored on
  all three splits: the 85--145 Hz body gain has a shallow minimum near its
  fitted value on training and development validation while the flat-top rows
  fall monotonically past 9.0; the body residue tilt is best near +1.9 dB per
  octave on both fitted splits and at −2.0 on the flat-top rows, 20% apart;
  and the bridge mobility scale improves training all the way to its 0.25
  lower bound — 6.1266 training and 6.2146 development validation, 4.7% and
  2.6% better than the shipping build — while the flat-top rows are worst
  there. That last direction is a real improvement the staged optimiser never
  reaches and it is deliberately not taken: it buys training score by scaling a
  measured bridge three times away from its measurement, in the direction that
  moves the only flat-top evidence the wrong way.
- Three listening sets are outstanding and nothing waits on anything else. Two
  are rendered and waiting in `tmp/acustra-listening/`, uncommitted and with
  their keys unread: `2026-08-31-archtop-or-flattop`, the whole
  which-instrument question above, and `2026-08-31-body-weight`, its narrower
  two-parameter version. The third is not rendered: upper-band damping, where
  the gated loss and the damping audit disagree and the two materials want
  opposite corrections above 4.5 kHz. All three are cases where following the
  references may mean following the wrong instrument.
