...
| --- | --- | ---: | ---: | ---: |
| `01-steel-sustain-range.wav` | Steel sustain from open E2 to B5, one held pluck at a time | 9.5 s | −13.0 dBFS | +10.0 dB |
| `02-nylon-fingerstyle.wav` | A fingertip nylon arpeggio with overlapping held notes | 4.2 s | −18.7 dBFS | +15.7 dB |
| `03-shape-material-anchors.wav` | One chord: Parlor/Jumbo, then Cedar/Maple anchor settings | 13.7 s | −6.8 dBFS | +3.8 dB |
| `04-string-age.wav` | The same steel phrase with fresh strings, then fully aged strings | 6.5 s | −10.0 dBFS | +7.0 dB |
| `05-alternate-tunings.wav` | Drop D, DADGAD and Open G chords | 12.3 s | −9.4 dBFS | +6.4 dB |
...
their axial area from the published core diameter. One resonator per string is
driven by DAFx-26's `EA/(2L)` times the mean square slope, through the same
...
the resonator passes are sum and difference phantom partials rather than an
added tone, and it grows faster than the note that made it: 5.4 dB of extra
1.5--4 kHz energy at a quarter velocity against 12.7 dB near full. It radiates
