...
- [Capture types](Docs/audio/12-capture-types.wav) — the same steel performance
  through stereo, treble and bass microphones, ideal saddle piezo, magnetic
  pickup, upper microphone, then loaded piezo. Native relative levels are
  retained; pickup sensitivities are normalized model units, not measurements
  of hardware gain.
...
absorber, with its first room reflections 12 ms or more away. **Upper mic**
exposes the third position, 20 cm from the treble microphone toward the upper
bout and also 10 cm above the top. Each individual microphone is mono; the
...
optimal pole bank. The retained responses use the previous common causal
windows: 3000 samples for g21 and 12000 for g34 at 48 kHz, with the final tenth
faded. The g34 window came from the earlier low-frequency Q-convergence check;
g21's room reflection already falls inside its shorter window. These are
approximations of windowed measurements, not identified intrinsic body decays.
...
Loaded piezo adds the electrical high-pass response derived from a measured
450 pF acoustic-guitar bridge pickup and its installed 2 MΩ preamp input,
documented for an Adamas SMT in Zollner's
...
The analog response is `sRC / (1 + sRC)`, with a 176.84 Hz corner; a passive
bilinear discretization retains its frequency warping. The filter advances
while unheard so changing capture preserves its ringing history. This models
the reported electrical loading only: the mechanical sensor response,
preamp EQ and absolute sensitivity are not identified, and it is not a full
Adamas emulation. Native impulse responses match the derived complex transfer
within 1.16×10⁻⁶ across 8–384 kHz. No matched piezo recording benchmark is
available yet.

For host automation, the original five-value Capture parameter retains its
normalized mapping. **Upper mic** selects the additional microphone when enabled;
turning it off restores the underlying Capture choice. **Piezo loading** affects
only the saddle-piezo choice. The editor combines these into one seven-choice
menu. Old sessions default absent switches off.
Thirty-six fixed-construction phrases across the six previous captures, both
string materials and 44.1/48/96 kHz remain byte-identical to the previous engine.
The 47 native renders used by the existing 83-row attack benchmark are also
byte-identical, carrying forward its mixed descriptor results without a new fit.
...
- Capture choices provide measured microphone positions, ideal pickup
  observables and one measured piezo/preamp electrical load. The piezo's
  mechanical sensor response and sensitivity, magnetic field map, coil
  resonance, and named microphone electronics remain unidentified. GuitarSet's
...
- Velocity still barely reaches attack brightness with the selected
  ...
  morph and Q limits. Its measured Q values still describe the retained windows,
  not intrinsic guitar damping: in the earlier g21 window audit, extending
  62.5 ms to 250 ms sharpened the 91 Hz mode from Q 7 to 19 and the 209 Hz mode
  from 15 to 39. Earlier scalar candidates retaining those decays worsened the
