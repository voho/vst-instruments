...
- 2026-08-17: Added direct regression coverage for `setPitchBend()`'s own clamp to +/-48 semitones, which every existing bend in the suite (up to 22 semitones, plus the separate NaN-fallback case) stayed well inside of and which production never reaches either, since `PluginProcessor::dispatchMidiData()` scales the MIDI wheel by a fixed +/-2 semitones; a finite bend far past either side of the clamp is now asserted to land on exactly the four-octave ratio the clamp promises, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-17: Replaced `setParameters()`'s and `voicesForMode()`'s two identical copies of the "clamp the requested ensemble size into 2..singerCount" expression with one shared `effectiveChoirSize()` helper, so the rule that a choir can only ever ask for as many singers as the engine actually holds twelve identities for has a single definition instead of two that could silently drift apart; no change to engine output, confirmed by `ctest --test-dir vocalor/build-dsp` (both tests pass, including `testEnsembleSizeIsExact()`'s existing out-of-range-request coverage) and a byte-for-byte identical re-render of the demo audio (`git status vocalor/Docs/audio` stays clean).
- 2026-08-17: Gave the Dynamics knob its own accessible description instead of the generic "Adjust dynamics" every other `VocalorKnob` gets, stating the mod-wheel/channel-pressure takeover this control is documented to have (the `dynamics` host parameter owns the level only until the first CC 1 or channel-pressure message, and CC 121 hands it back) -- a behaviour the README's MIDI mapping table and `EngineParameters::dynamics`'s own doc comment already state but that a screen-reader user driving the plug-in had no way to discover, since every other continuous control's description was the generic template; UI-only change, no engine or parameter behaviour touched, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-17: Added direct regression coverage for each of `formantResponseDbFromCoefficients()`'s five guard conditions in isolation (a null `a1`, `a2` or `scale` pointer, a non-positive formant count, and a non-positive sample rate), which the existing coverage only ever exercised as one combined case with every term invalid at once -- exactly the gap `formantResponseCoefficients()`'s own guard tests, right above this one in the suite, were already written to avoid. Temporarily dropping the count and sample-rate terms from the guard (leaving the null-pointer checks in place) did not make either new assertion fail: an empty formant count leaves the response loop's accumulators at zero by construction, and a zero sample rate drives the per-formant angle to NaN, which the loop's own `denominator > 0.0f` check treats as `continue` -- both incidentally reproduce the guard's -120 dB fallback without it, so the explicit early return is what pins that value as a contract rather than being the only thing standing between the caller and a NaN. Dropping the sample-rate sign check alone (a negative rate, which does not hit either coincidence) did make its assertion fail as expected. Test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-17: Added direct regression coverage for `prepare()`'s own sanitisation of the sample rate it is handed: a NaN or infinite value falls back to 48 kHz, and any other value is clamped into the 8 kHz-192 kHz range every formant ceiling, room delay tap and mip guard is derived from. `PluginProcessor::prepareToPlay()` forwards the host's rate straight through with no clamp of its own, and every existing `prepare()` call across the suite (including `testSampleRateInvariance()`'s own sweep) already passed an in-range rate, so neither the fallback nor the clamp had ever been exercised directly. The new test asserts all four cases (NaN, infinity, a below-floor and an above-ceiling rate) land on the expected sanitised rate, plus that an engine prepared with a NaN rate still renders normally afterwards. Review caught that the "renders normally" check only required a non-zero sample, which a NaN or infinite sample also satisfies (`NaN != 0.0f` is true), so a broken sanitisation path producing non-finite output could still have passed; the check now also requires every rendered sample in both channels to be finite. Test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-17: Added direct regression coverage for `roomSizeScale()`'s and `glideTimeSeconds()`'s shared `clampUnit()` fallback, which the existing coverage only ever exercised inside the valid 0..1 knob range (0, 0.5 and 1) and never against a below-range, above-range or non-finite input. A negative room size or glide, a value above 1, and a NaN are now each asserted to land on the same result as the nearest in-range endpoint (0 for the non-finite case, matching `clampUnit()`'s own documented fallback), so a future change that dropped or narrowed that clamp -- both parameters are host automatable and a DAW can send out-of-range automation -- would be caught directly instead of only by a stray NaN/negative propagating into the room or portamento DSP; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass), confirmed the new assertions actually fail by temporarily removing the `clampUnit()` calls from both functions (`ctest` then reported 6 failed checks) before reverting, and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Simplified `makeRoomFor()`'s free-voice-slot count to reuse `countActiveVoices()` instead of re-walking the voice array with duplicate logic of its own.
- 2026-08-16: Added direct regression coverage for `noteOn()`'s non-positive-velocity guard being safe on a never-prepared engine, not just on an already-prepared one. That guard is the very first statement in `noteOn()` and routes straight to `noteOff()` before the ordinary body's own `prepared_` check, and `noteOff()` has no `prepared_` guard of its own; the existing pre-prepare coverage only ever called `noteOn()` with a normal velocity (hitting the `prepared_` check directly), and the existing non-positive-velocity coverage only ever ran on an already-prepared engine, so the specific combination -- and whether a subsequent `prepare()`/`reset()` still leaves the engine able to accept a note afterwards -- had never been asserted; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Added direct regression coverage for `resetControllers()`'s documented promise to leave the sustain pedal alone (CC121, Reset All Controllers, does not report the physical position of a switch), which the existing sustain-pedal test never exercised together with a controller reset and the existing `resetControllers()` test only used to check the mod-wheel-to-host handback, never a pedal-held note. The new case holds a note under the pedal through its note-off, calls `resetControllers()` and asserts the note is still sounding, then confirms `setSustainPedal(false)` still releases it normally afterwards through its full release tail; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Stopped `process()` from scanning every one of `placementHold_`'s twelve singer slots on every block just to decide whether the engine can take its all-silent idle path, when that scan's result is only ever read inside the very condition that also requires zero active voices and a decayed room tail; the scan is now folded into the condition itself so it short-circuits before running at all whenever any voice is sounding (the common case) and exits at the first held singer otherwise, instead of always visiting all twelve; no change to engine output, confirmed by `ctest --test-dir vocalor/build-dsp` (both tests pass) and a byte-for-byte identical re-render of the demo audio (`git status vocalor/Docs/audio` stays clean).
- 2026-08-16: Added direct regression coverage for `tunedFirstFormant()`'s third defensive fallback, the non-finite ceiling guard, which the prior direct-coverage pass (below) exercised for a non-positive base formant and a non-positive/non-finite fundamental but not for `ceilingHz` itself; production only ever passes `chunkMaxF1_` times a finite formant-shift scale, so no engine-level test reaches it either. A NaN and an infinite ceiling are now both asserted to fall back to the vowel's own F1, which collapses the whole tracking range to `baseHz` regardless of how high the fundamental has climbed; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Added direct regression coverage for `chordMidiForSinger()`'s three chord-voicing shapes (an upward two-octave stack for a root below 48, a downward one for a root above 72, and the symmetric shape used between them) plus its register clamp and singer-index clamp, none of which had ever been exercised in isolation because every chord-mode render elsewhere in the suite plays root 60, which only reaches the middle shape; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Added regression coverage for the out-of-range index clamp `factoryPreset()`/`factoryPresetName()` are documented to apply, which every existing call site (including `PluginProcessor::setCurrentProgram()`, which already clamps its own index first) had only ever exercised inside the valid `0..count-1` range, leaving the defensive clamp itself unasserted for a negative, a large-negative, an exactly-past-the-end and a large-positive index; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
...
- 2026-08-15: Resolved radiatedPowerTarget()'s per-formant signed peak gain (polarity, cascade amplitude and peak normaliser) once per call instead of re-deriving it inside tractPowerAt() on every one of the up to 72 frequency probes a single call makes, since none of those three terms depends on the probe frequency; no change to engine output, confirmed by re-running the DSP test suite and a byte-for-byte identical re-render of the demo audio.
- 2026-08-16: Replaced five independent copies of the `p.vowel == Vowel::Ooh ? 1 : (p.vowel == Vowel::Uuh ? 2 : 0)` AAH/OOH/UUH-to-index mapping (in setParameters(), updateChunkState(), updateVowelDrift(), updateVoiceControl() and publishDisplayState()) with one shared `vowelIndexOf()` helper, so the vowel-to-preset-index rule now has a single definition instead of five that could silently drift apart; no change to engine output, confirmed by re-running the DSP test suite and a byte-for-byte identical re-render of the demo audio.
- 2026-08-16: Added regression coverage for the defensive fallback branches in VocalorMath's display helpers that had none — `normalisedLogFrequency`/`logFrequencyForNormalised` on a non-positive or inverted min/max range, `linearToDecibels` and `decibelsToMeterPosition` on a non-finite input and on an inverted floor/ceiling, `smoothingCoefficient` on a non-positive time constant or update interval, and `meterFollow` on a non-finite target (its non-finite-current path was already covered); no source change, verified by the DSP test suite passing and Docs/audio staying untouched.
- 2026-08-16: Added regression coverage for the remaining untested guard branches in VocalorMath's vowel-space and formant-bank helpers — `formantShiftRatio` on a non-finite semitone value, `cardinalVowelPosition`/`cardinalVowelName` and `presetVowelPosition` on an out-of-range index (both a negative and a past-the-end one), the null-output no-op in `formantsForVowelPoint`/`formantsForPresetVowel`, and the null-pointer/non-positive-count/non-positive-sample-rate no-op shared by `parallelFormantCoefficients`/`parallelFormantAmplitudes` and by `formantResponseCoefficients`; no source change, verified by the DSP test suite passing and Docs/audio staying untouched.
- 2026-08-16: Added direct regression coverage for `tunedFirstFormant()` (the singer's-formant-style F1 tracking a fundamental too high for its own vowel), which was previously exercised only indirectly through the engine-level testFormantTuningAtHighPitch() and had neither its normal engagement curve nor its two defensive fallbacks (a non-positive base formant, a non-finite or non-positive fundamental) asserted directly; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and a byte-for-byte identical re-render of the demo audio.
- 2026-08-16: Rebuilt `formantResponseDb()` to delegate into `formantResponseCoefficients()` and `formantResponseDbFromCoefficients()` instead of keeping its own copy of the same per-formant pole/gain derivation, so the bit-identity between the one-shot probe and the cached-coefficient path the editor uses is now guaranteed by construction rather than by two implementations happening to agree; no change to engine output, confirmed by the DSP test suite (including its existing direct-vs-cached-coefficient equality check) passing and a byte-for-byte identical re-render of the demo audio.
- 2026-08-16: Added regression coverage for `dynamicResponse()`'s own input clamp, which had only ever been exercised at the knob's 0 and 1 endpoints — a value below 0, a value above 1, and a non-finite dynamics level all now assert against the same empty/full response the endpoints already produce; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Added direct regression coverage for `justIntonationOffsetCents()`'s negative-input pitch-class wrap, which the existing suite only ever called with non-negative semitone counts or the exact octave `-12` (whose result is unchanged whether or not the wrap runs) — a genuinely negative, non-octave argument reaches it in production whenever `VoiceEngine::noteOff()` releases the current bass: it begins releasing that root's voices and then recomputes `intonationRoot_` from the remaining non-releasing voices before the release tail has faded, so the departing root's own `voice.midiNote - intonationRoot_` goes negative for the rest of its release. Five negative inputs (`-1`, `-5`, `-7`, `-13`, `-24`) are now asserted to land on the same offset as their positive same-pitch-class equivalents, which a naive `semitonesAboveRoot % 12` (C++'s `%` keeps the left operand's sign) would not; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Deduplicated the per-formant pole derivation that `parallelFormantCoefficients()` and `formantResponseCoefficients()` each computed independently into one shared helper, so the Nyquist-relative clamp bounds and pole-radius/angle formulas can no longer drift apart between the two.
- 2026-08-16: Added regression coverage for `VoiceEngine::noteOn()`'s own guard that routes a non-positive or non-finite velocity to `noteOff()` instead of starting a voice. `PluginProcessor::dispatchMidiData()` already turns a raw MIDI note-on with a velocity byte of 0 into an explicit `noteOff()` before the engine ever sees it, so that guard was invisible from the plugin's own MIDI path; but `enqueueUiMidiEvent()`/`dispatchUiMidiEvents()`, which is what the editor's on-screen keyboard and the standalone's own MIDI input go through, forward whatever velocity the UI produced straight into `engine.noteOn()` with no such pre-filtering, and nothing in the suite had ever called it with a velocity of exactly 0 or a negative one. The new test asserts that `noteOn()` with velocities `0`, `-0.4` and `-1.0` renders bit-identically to `noteOff()` on the same note over a full release tail, and that the same call on a pitch nothing is sounding is a safe silent no-op; test-only change, verified with `ctest --test-dir vocalor/build-dsp` (both tests pass) and confirming `git status vocalor/Docs/audio` stays clean.
- 2026-08-16: Extracted the "which band-limited mip level is still alias-safe for this fundamental" search into one `mipTableLevelForFrequency()` helper, which `updateVoiceControl()` (choosing the oscillator's own running mip) and `radiatedPowerTarget()` (choosing its independent analysis-rate mip, deliberately from the note's own f0 rather than the oscillator's vibrato/jitter-affected one) had each run as an identical permissible-harmonic-count loop; no change to engine output, confirmed by the DSP test suite passing and a byte-for-byte identical re-render of the demo audio.
