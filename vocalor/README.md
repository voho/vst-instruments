...
A real-time **vocal and choir instrument**: VST3, Audio Unit and Standalone for
macOS, plus Linux and Windows builds.

![Vocalor](Docs/screenshots/vocalor-standalone.png)

Vocalor turns MIDI notes into original sung vowels, from a single singer to an
ensemble or a major/minor chord. Three preset vowels anchor a **continuous
vowel space** that can be morphed, automated and played from a draggable pad,
and the whole vocal tract can be lengthened or shortened independently of pitch.
The engine is procedural and runs locally: it clones no named singer, loads no
recordings, and contacts no service while rendering audio.

An LF-style glottal source drives a source-filter tract with legato phrasing
across solo, ensemble and chord modes. Vowel transitions run on articulator
timescales, F1 tracks the fundamental once pitch climbs past it, the singer's
formant is a resonance cluster rather than a fixed boost, and a nasal branch
allows humming.

> The screenshot was captured from the 1.0 Standalone application and shows the
> earlier layout. Automated editor-rendering tests exercise the current one.

## Audio demos

Ten takes, rendered by [`Tools/RenderDemos.cpp`](Tools/RenderDemos.cpp) through
the shipping engine, so they cannot drift from what the plug-in does: a solo
legato phrase, the preset vowels on both voice profiles, the vowel space and
formant shift on held notes, the twelve-singer ensemble, chord mode locking
onto just intervals, a choir swelled on the Dynamics control, tension and
breath closing into a hum, the room at both sizes, and one soprano line through
the register-aware upper tract.
...
Init Soprano pairs the fresh 38 % Drift default with Vibrato at exactly 0 %: a
straight tone with bounded aperiodic pitch and slow articulation motion, not a
perfect oscillator. Programs that want an intentional sung gesture opt into
Vibrato explicitly, and on those sounds Drift also varies vibrato rate, depth,
contour and onset.
...
- **Word building.** Vocalor is not a lyric sequencer and ships no sample data,
  so synthesizing plosives, fricatives and stops from scratch would be a
  different instrument. The nasal branch is included because it is a
  tract-geometry change the existing model was one pole-zero pair away from,
  not because it is a first step toward words.
...
**Deferred, with the reason.** Each is a documented mechanism the engine does
not implement, recorded so a later pass knows what it is buying and what blocks
it.
...
- **Subglottal pole-zero pair.** A tracheal branch contributes 5–12 dB of
  ...
  a chromatic run uneven in a way Vocalor's is not. Perhaps twenty lines, but it
  needs the open-quotient weighting to be per voice.
- **Nonlinear source-filter coupling and open-phase F1 modulation.** Titze's
  ...
  destabilisation at an f₀–F1 crossing is the one place the literature says
  instability is not a defect
  ([JASA 123(5):2733](https://pubmed.ncbi.nlm.nih.gov/18529191/)). Deferred
  because it interacts with the tuned-F1 region, and because a within-period
  bandwidth change means resolving the resonators at something other than the
  chunk rate.
- **Key-centre memory and intonation drift.** A held common tone is retuned by
  up to 15.6 cents when the bass moves, because the intonation root
  re-references rather than being carried
  ([Howard, *Journal of Voice*](https://www.sciencedirect.com/science/article/abs/pii/S0892199705001657)).
  Worth doing, but it means accumulating state across note events — the
  determinism question again.
- **Voice type as a function of sounding pitch.** The voice profile is one
  global switch, so a two-octave chord is sung by one tract length. The vowel
  tables, bandwidths, cluster centre and F1 ceiling are all resolved per chunk
  from that one boolean, so blending it with the sounding note is the same
  per-voice restructuring as per-singer phonation.
- **Pitch scoop direction.** A constant-sign onset gesture is a mannerism; the
  correct behaviour depends on interval direction, dynamic and attack type, and
  the engine does not know the previous sounding pitch at note-on outside the
  legato path.
...
- **The wavetable ladder.** Nothing audible rides on it — energy above 11.2 kHz
  is 111.1 dB down at MIDI 51 and 143.3 dB down at MIDI 53. Recorded so a future
  step that brightens the source knows to check it, and that the step is 32 dB.

## Release history

### 1.4

- Publishes the control whose immutable host identifier is `instability` as
  **Drift**, broadening it from vibrato-cycle variation into a
  natural-performance macro.

### 1.3

- Appended the `instability` control, adding cycle-to-cycle variation to what
  had been a straight tone.

### 1.2

- Added Dynamics, Intonation and Nasal, and regrouped the control row into
  voice character, performance, space and master.

### 1.1

- Added the vowel-space pad, the live vocal-tract analyser with output meters,
  and the phrasing and room-geometry controls.

### 1.0

- First release: the LF-style glottal source, the source-filter tract with
  three anchored vowels and a continuous space between them, formant shifting
  independent of pitch, solo/ensemble/chord modes with up to twelve singer
  identities, and the room.
...
cmake -S . -B build-dsp -DCMAKE_BUILD_TYPE=Release \
  -DVOCALOR_BUILD_PLUGIN=OFF -DBUILD_TESTING=ON
...
`-DVOCALOR_JUCE_PATH=/path/to/JUCE`):
...
terms — see `THIRD_PARTY_NOTICES.md`. No recordings, sample libraries or
third-party voice data are included.
