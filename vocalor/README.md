...
from 11.68 to 6.36 dB and the largest adjacent-note change from 7.37 to 4.36 dB.
...
only 0.10 dB on a held C5. Instability-zero sessions retain the old 0.0020-per-
cent, in-phase source-and-shelf path exactly; its fixed-rate audio-envelope test
still reads 2.994 dB on C5 and 3.163 dB on C6 at full Vibrato. Fresh sounds use
...
and its coefficient moves toward 0.0011 per cent with a 0.18 ceiling. The two
presence shelves share one spectral gesture instead of accidentally applying
the complete gain twice on top of the direct-source gain.
...
and 1.371 dB in the upper band. The solo demo's 50/44 % setting measures
0.922/2.154 dB, and the full 100/100 % corner remains bounded at
2.933/5.845 dB. The upper band therefore follows the intended roughly 2:1
