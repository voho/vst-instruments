...
void VoiceEngine::updateVoiceControl(Voice& voice, const EngineParameters& p)
{
    ...
    // There are two cascaded presence shelves. Driving each by the full gain
    // makes their high-band modulation square, then the voiced drive below
    // applies it a third time. Fade each shelf toward half the linear depth as
    // the natural model comes in: together they contribute one laryngeal
    // gesture and the source contributes the other, the intended 2:1 law.
    const float shelfDepth = (1.0f - 0.5f * naturalAmount)
        * laryngeal * amplitudeShape;
    const float vibratoShelfGainTarget = 1.0f + shelfDepth;
...
void VoiceEngine::renderVoice(Voice& voice, const EngineParameters& p, int count)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        // The natural vibrato path shares one spectral gesture across the two
        // shelves. With the direct-source gesture below, the band above the
        // corner approaches twice the fundamental's movement; Instability zero
        // alone retains the old three-copy law for exact session recall.
