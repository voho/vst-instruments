...
Vocalor's original source code is covered by the repository's `LICENSE` file.
The build downloads or consumes the separate dependency below; that dependency
is not relicensed by Vocalor.
...
distributing Vocalor with JUCE therefore requires either compliance with the
...
No third-party neural-network model, voice recording, or sample library is
