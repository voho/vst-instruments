...
ARTIFACT_DIR="${BUILD_DIR}/YouKnow201_artefacts/${CONFIG}"
...
VST3="${ARTIFACT_DIR}/VST3/YouKnow201.vst3"
AU="${ARTIFACT_DIR}/AU/YouKnow201.component"
APP="${ARTIFACT_DIR}/Standalone/YouKnow201.app"
...
VST3_ARCHS="$(lipo -archs "${VST3}/Contents/MacOS/YouKnow201")"
AU_ARCHS="$(lipo -archs "${AU}/Contents/MacOS/YouKnow201")"
APP_ARCHS="$(lipo -archs "${APP}/Contents/MacOS/YouKnow201")"
...
ditto "${VST3}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/YouKnow201.vst3"
ditto "${AU}" "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/YouKnow201.component"
ditto "${APP}" "${PACKAGE_ROOT}/Applications/YouKnow201.app"

NOTICE_ROOT="${PACKAGE_ROOT}/Library/Application Support/YouKnow201/Documentation"
...
mkdir -p "${NOTICE_ROOT}"
ditto "${PROJECT_DIR}/LICENSE" "${NOTICE_ROOT}/YouKnow201-LICENSE.txt"
...
for bundle in \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/YouKnow201.vst3" \
    "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/YouKnow201.component" \
    "${PACKAGE_ROOT}/Applications/YouKnow201.app"; do
    bundle_notice_root="${bundle}/Contents/Resources/Documentation"
    mkdir -p "${bundle_notice_root}"
    ditto "${PROJECT_DIR}/LICENSE" "${bundle_notice_root}/YouKnow201-LICENSE.txt"
    ditto "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
...
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/VST3/YouKnow201.vst3"
sign_bundle "${PACKAGE_ROOT}/Library/Audio/Plug-Ins/Components/YouKnow201.component"
sign_bundle "${PACKAGE_ROOT}/Applications/YouKnow201.app"

ZIP_PATH="${DIST_DIR}/YouKnow201-${VERSION}-macOS-${ARTIFACT_ARCH}.zip"
PKG_UNSIGNED="${DIST_DIR}/YouKnow201-${VERSION}-unsigned.pkg"
PKG_FINAL="${DIST_DIR}/YouKnow201-${VERSION}-macOS-${ARTIFACT_ARCH}.pkg"

# Leave exactly one YouKnow201 release set in dist, avoiding stale version or
# architecture names in wildcard-driven CI publication.
rm -f "${DIST_DIR}/YouKnow201-"*-macOS-*.zip
rm -f "${DIST_DIR}/YouKnow201-"*-macOS-*.pkg
rm -f "${DIST_DIR}/YouKnow201-"*-unsigned.pkg

COPYFILE_DISABLE=1 pkgbuild \
    --root "${PACKAGE_ROOT}" \
    --identifier cz.protocodus.youknow201.pkg \
    --version "${VERSION}" \
    --install-location / \
    "${PKG_UNSIGNED}"
