...
YouKnow201's original source code is covered by the project's `LICENSE` file.
The build downloads or consumes the separate dependency below; that dependency
is not relicensed by YouKnow201.
...
included in every distributed bundle and installer package alongside YouKnow201's
...
distributing YouKnow201 with JUCE therefore requires either compliance with the
